# Graph Report - v3  (2026-07-20)

## Corpus Check
- 2341 files · ~2,085,973 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 19500 nodes · 37949 edges · 986 communities (696 shown, 290 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 580 edges (avg confidence: 0.75)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cd9e2c16`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_TypeScript Type Utilities|TypeScript Type Utilities]]
- [[_COMMUNITY_Mesh Networking Contracts|Mesh Networking Contracts]]
- [[_COMMUNITY_Deployment Domain Schemas|Deployment Domain Schemas]]
- [[_COMMUNITY_Admin Auth Plugin|Admin Auth Plugin]]
- [[_COMMUNITY_HTTPTraefik Router Builder|HTTP/Traefik Router Builder]]
- [[_COMMUNITY_Declarative Routes|Declarative Routes]]
- [[_COMMUNITY_Traefik Configuration Repository|Traefik Configuration Repository]]
- [[_COMMUNITY_Permission & Role Checks|Permission & Role Checks]]
- [[_COMMUNITY_Docker Container Inspection|Docker Container Inspection]]
- [[_COMMUNITY_Mesh Core & Orchestration|Mesh Core & Orchestration]]
- [[_COMMUNITY_Docker Container Actions|Docker Container Actions]]
- [[_COMMUNITY_Docker Module Wiring|Docker Module Wiring]]
- [[_COMMUNITY_Mesh Topology Services|Mesh Topology Services]]
- [[_COMMUNITY_Docker Runtime Activity Projection|Docker Runtime Activity Projection]]
- [[_COMMUNITY_Mesh Cluster Repository|Mesh Cluster Repository]]
- [[_COMMUNITY_Deployment Pipeline Service|Deployment Pipeline Service]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_cn|cn]]
- [[_COMMUNITY_SystemMeshTopologyService|SystemMeshTopologyService]]
- [[_COMMUNITY_types.ts|types.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_AuthCoreService|AuthCoreService]]
- [[_COMMUNITY_platform-domain.builder.ts|platform-domain.builder.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_mesh-connection-registry.ts|mesh-connection-registry.ts]]
- [[_COMMUNITY_mesh-contracts.ts|mesh-contracts.ts]]
- [[_COMMUNITY_docker.large.mock.ts|docker.large.mock.ts]]
- [[_COMMUNITY_query-builder.ts|query-builder.ts]]
- [[_COMMUNITY_DockerRuntimeRunnerService|DockerRuntimeRunnerService]]
- [[_COMMUNITY_DeploymentTriggerInput|DeploymentTriggerInput]]
- [[_COMMUNITY_DeploymentService|DeploymentService]]
- [[_COMMUNITY_stream.ts|stream.ts]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_reporting.ts|reporting.ts]]
- [[_COMMUNITY_docker.repository.ts|docker.repository.ts]]
- [[_COMMUNITY_docker-entity-domain.service.ts|docker-entity-domain.service.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_BaseEventService|BaseEventService]]
- [[_COMMUNITY_ProjectService|ProjectService]]
- [[_COMMUNITY_middlewares.ts|middlewares.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_build-tools.ts|build-tools.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_CoreEventSyncService|CoreEventSyncService]]
- [[_COMMUNITY_LocalDatabaseService|LocalDatabaseService]]
- [[_COMMUNITY_permission-engine.ts|permission-engine.ts]]
- [[_COMMUNITY_VariableRegistry|VariableRegistry]]
- [[_COMMUNITY_deployment-queue-lifecycle.service.ts|deployment-queue-lifecycle.service.ts]]
- [[_COMMUNITY_context-filter-debug.ts|context-filter-debug.ts]]
- [[_COMMUNITY_mesh-query-builder-types.ts|mesh-query-builder-types.ts]]
- [[_COMMUNITY_docker-runtime-activity-domain.service.ts|docker-runtime-activity-domain.service.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_core-event-sync.service.ts|core-event-sync.service.ts]]
- [[_COMMUNITY_DockerRuntimeEventsSourceService|DockerRuntimeEventsSourceService]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_docker-image-detail-modal.tsx|docker-image-detail-modal.tsx]]
- [[_COMMUNITY_requireAuth|requireAuth]]
- [[_COMMUNITY_docker-container-runtime-mesh.service.ts|docker-container-runtime-mesh.service.ts]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_DockerRepository|DockerRepository]]
- [[_COMMUNITY_ServiceDomainMappingRepository|ServiceDomainMappingRepository]]
- [[_COMMUNITY_cn|cn]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_InitializationService|InitializationService]]
- [[_COMMUNITY_BaseTriggerService|BaseTriggerService]]
- [[_COMMUNITY_isObjectLike|isObjectLike]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_common.ts|common.ts]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_PlatformRole|PlatformRole]]
- [[_COMMUNITY_GlobalDatabaseService|GlobalDatabaseService]]
- [[_COMMUNITY_TraefikFileSystemService|TraefikFileSystemService]]
- [[_COMMUNITY_auth-core.service.ts|auth-core.service.ts]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_github.module.ts|github.module.ts]]
- [[_COMMUNITY_DeploymentMeshService|DeploymentMeshService]]
- [[_COMMUNITY_deployment.service.spec.ts|deployment.service.spec.ts]]
- [[_COMMUNITY_runtime-configuration.schema.ts|runtime-configuration.schema.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_mdx-components.tsx|mdx-components.tsx]]
- [[_COMMUNITY_MeshControlEnvelope|MeshControlEnvelope]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_standard-schema-helpers.ts|standard-schema-helpers.ts]]
- [[_COMMUNITY_map.tsx|map.tsx]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_route-builder.ts|route-builder.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_mesh-entity.ts|mesh-entity.ts]]
- [[_COMMUNITY_deployment.ts|deployment.ts]]
- [[_COMMUNITY_AnalyticsService|AnalyticsService]]
- [[_COMMUNITY_dependency-graph.mock.ts|dependency-graph.mock.ts]]
- [[_COMMUNITY_standard-operations.ts|standard-operations.ts]]
- [[_COMMUNITY_MeshTrustStrictModeService|MeshTrustStrictModeService]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_contracts.ts|contracts.ts]]
- [[_COMMUNITY_data-table.tsx|data-table.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_AppLifecycleService|AppLifecycleService]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_.error|.error]]
- [[_COMMUNITY_SetupStepTracker|SetupStepTracker]]
- [[_COMMUNITY_vitest|vitest]]
- [[_COMMUNITY_makeRoute.tsx|makeRoute.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_user.repository.ts|user.repository.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_shared.ts|shared.ts]]
- [[_COMMUNITY_PushRepository|PushRepository]]
- [[_COMMUNITY_deployment-provider-builder-runner-state-machine.service.ts|deployment-provider-builder-runner-state-machine.service.ts]]
- [[_COMMUNITY_ServiceService|ServiceService]]
- [[_COMMUNITY_traefik-middleware-library.service.ts|traefik-middleware-library.service.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_isRecord|isRecord]]
- [[_COMMUNITY_InternalErrorInsightService|InternalErrorInsightService]]
- [[_COMMUNITY_file-upload-link.ts|file-upload-link.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_docker-create-container-modal.tsx|docker-create-container-modal.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_RouteRegistryService|RouteRegistryService]]
- [[_COMMUNITY_docker-runtime-stream-orchestrator.service.ts|docker-runtime-stream-orchestrator.service.ts]]
- [[_COMMUNITY_manager.ts|manager.ts]]
- [[_COMMUNITY_TraefikSyncService|TraefikSyncService]]
- [[_COMMUNITY_auth-module-definition.ts|auth-module-definition.ts]]
- [[_COMMUNITY_TraefikVariableResolverService|TraefikVariableResolverService]]
- [[_COMMUNITY_ZodStandardOperations|ZodStandardOperations]]
- [[_COMMUNITY_DockerContainerLogEntry|DockerContainerLogEntry]]
- [[_COMMUNITY_setup-wizard.tsx|setup-wizard.tsx]]
- [[_COMMUNITY_helpers.ts|helpers.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_DockerRuntimeEvent|DockerRuntimeEvent]]
- [[_COMMUNITY_scan.schema.ts|scan.schema.ts]]
- [[_COMMUNITY_schemas.ts|schemas.ts]]
- [[_COMMUNITY_generate-hooks.ts|generate-hooks.ts]]
- [[_COMMUNITY_Body|Body]]
- [[_COMMUNITY_platform-domain.schema.ts|platform-domain.schema.ts]]
- [[_COMMUNITY_Startup Architecture v2 — Sub-App Trigger Chain|Startup Architecture v2 — Sub-App Trigger Chain]]
- [[_COMMUNITY_shared-api-runtime.ts|shared-api-runtime.ts]]
- [[_COMMUNITY_startup-architecture|startup-architecture.mdx]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_SystemMeshClusterRepository|SystemMeshClusterRepository]]
- [[_COMMUNITY_auth.ts|auth.ts]]
- [[_COMMUNITY_organization.repository.ts|organization.repository.ts]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_traefik-config-builder.ts|traefik-config-builder.ts]]
- [[_COMMUNITY_DockerContainerRuntimeService|DockerContainerRuntimeService]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_RoleConfigCollection|RoleConfigCollection]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_configuration-core.module.ts|configuration-core.module.ts]]
- [[_COMMUNITY_DockerContainerRuntimeMeshService|DockerContainerRuntimeMeshService]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_command.tsx|command.tsx]]
- [[_COMMUNITY_TestController|TestController]]
- [[_COMMUNITY_docker-containers-orchestrator.service.ts|docker-containers-orchestrator.service.ts]]
- [[_COMMUNITY_docker-container-runtime.service.ts|docker-container-runtime.service.ts]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_.key|.key]]
- [[_COMMUNITY_DeploymentRepository|DeploymentRepository]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_standard|standard]]
- [[_COMMUNITY_peerDependencies|peerDependencies]]
- [[_COMMUNITY_DetailedOutputBuilder|DetailedOutputBuilder]]
- [[_COMMUNITY_tasks|tasks]]
- [[_COMMUNITY_terminal-viewer.tsx|terminal-viewer.tsx]]
- [[_COMMUNITY_include|include]]
- [[_COMMUNITY_types.ts|types.ts]]
- [[_COMMUNITY_crud.ts|crud.ts]]
- [[_COMMUNITY_StatementConfig|StatementConfig]]
- [[_COMMUNITY_StatementConfig|StatementConfig]]
- [[_COMMUNITY_route-method-meta.ts|route-method-meta.ts]]
- [[_COMMUNITY_init.ts|init.ts]]
- [[_COMMUNITY_TraefikEventService|TraefikEventService]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_usePermissions.ts|usePermissions.ts]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_TraefikConfigBuilder|TraefikConfigBuilder]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_DashboardSidebar.tsx|DashboardSidebar.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_PermissionBuilder|PermissionBuilder]]
- [[_COMMUNITY_traefik.module.ts|traefik.module.ts]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_endpoints.ts|endpoints.ts]]
- [[_COMMUNITY_Startup v2 Implementation Blueprint — Sub-App Trigger Chain|Startup v2 Implementation Blueprint — Sub-App Trigger Chain]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_docker.mock.ts|docker.mock.ts]]
- [[_COMMUNITY_StatementConfigCollection|StatementConfigCollection]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_ApiKeyRepository|ApiKeyRepository]]
- [[_COMMUNITY_project.module.ts|project.module.ts]]
- [[_COMMUNITY_ProjectDomainRepository|ProjectDomainRepository]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_schemas.ts|schemas.ts]]
- [[_COMMUNITY_platform-domain.builder.ts|platform-domain.builder.ts]]
- [[_COMMUNITY_types.ts|types.ts]]
- [[_COMMUNITY_.nowMs|.nowMs]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_DeploymentQueueProcessor|DeploymentQueueProcessor]]
- [[_COMMUNITY_client.tsx|client.tsx]]
- [[_COMMUNITY_mock-hooks.ts|mock-hooks.ts]]
- [[_COMMUNITY_schemas.ts|schemas.ts]]
- [[_COMMUNITY_makeRoute.tsx|makeRoute.tsx]]
- [[_COMMUNITY_peerDependencies|peerDependencies]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_service.schema.ts|service.schema.ts]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_RolesConfig|RolesConfig]]
- [[_COMMUNITY_RolesConfig|RolesConfig]]
- [[_COMMUNITY_server.ts|server.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_router-hooks.types.ts|router-hooks.types.ts]]
- [[_COMMUNITY_cluster.ts|cluster.ts]]
- [[_COMMUNITY_mesh-type-utils.ts|mesh-type-utils.ts]]
- [[_COMMUNITY_mesh-filter.ts|mesh-filter.ts]]
- [[_COMMUNITY_error|error]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_ProviderSchemaService|ProviderSchemaService]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_domain.module.ts|domain.module.ts]]
- [[_COMMUNITY_utils.test.ts|utils.test.ts]]
- [[_COMMUNITY_platform-domain.schema.ts|platform-domain.schema.ts]]
- [[_COMMUNITY_Phase 1 Dead Code Removal 🧹|Phase 1: Dead Code Removal 🧹]]
- [[_COMMUNITY_🎯 AI Coding Agent Instructions|🎯 AI Coding Agent Instructions]]
- [[_COMMUNITY_docker-registry-detail-modal.tsx|docker-registry-detail-modal.tsx]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_AbstractDomainEventStreamService|AbstractDomainEventStreamService]]
- [[_COMMUNITY_drizzle-filter.utils.ts|drizzle-filter.utils.ts]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_Current Architecture & Next Steps for Sub-App Orchestration|Current Architecture & Next Steps for Sub-App Orchestration]]
- [[_COMMUNITY_platform-capabilities-graphs|platform-capabilities-graphs.mdx]]
- [[_COMMUNITY_app-error.ts|app-error.ts]]
- [[_COMMUNITY_Phase 2 Mock→Real Data Migration 🔗|Phase 2: Mock→Real Data Migration 🔗]]
- [[_COMMUNITY_routing-resources.schema.ts|routing-resources.schema.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_utils|utils]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_entrypoint.prod.ts|entrypoint.prod.ts]]
- [[_COMMUNITY_ServiceContextBuilder|ServiceContextBuilder]]
- [[_COMMUNITY_SystemMeshConfigService|SystemMeshConfigService]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_migration.schema.ts|migration.schema.ts]]
- [[_COMMUNITY_calendar-date-picker.tsx|calendar-date-picker.tsx]]
- [[_COMMUNITY_Startup v2 — Complete NestJS DI Orchestration Graph|Startup v2 — Complete NestJS DI Orchestration Graph]]
- [[_COMMUNITY_vitest.shared-postgres.e2e.ts|vitest.shared-postgres.e2e.ts]]
- [[_COMMUNITY_startup-architecture-analysis|startup-architecture-analysis.mdx]]
- [[_COMMUNITY_mesh-subscription-manager.ts|mesh-subscription-manager.ts]]
- [[_COMMUNITY_LoadBalancerController|LoadBalancerController]]
- [[_COMMUNITY_3.1 Fix `as unknown as` Violations (80+)|3.1 Fix `as unknown as` Violations (80+)]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_toolbar.tsx|toolbar.tsx]]
- [[_COMMUNITY_StatementsConfig|StatementsConfig]]
- [[_COMMUNITY_StatementsConfig|StatementsConfig]]
- [[_COMMUNITY_domain.types.ts|domain.types.ts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_.handleEvent|.handleEvent]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_app.module.ts|app.module.ts]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_apinest|api:nest]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_All|All]]
- [[_COMMUNITY_ConfigurationResolverService|ConfigurationResolverService]]
- [[_COMMUNITY_mesh-doc-compliance-matrix.e2e-spec.ts|mesh-doc-compliance-matrix.e2e-spec.ts]]
- [[_COMMUNITY_list.ts|list.ts]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_progressive-module-initialization|progressive-module-initialization.mdx]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_shared.ts|shared.ts]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_RoleConfigCollection|RoleConfigCollection]]
- [[_COMMUNITY_StatementConfigCollection|StatementConfigCollection]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_load-balancer.service.ts|load-balancer.service.ts]]
- [[_COMMUNITY_LoadBalancerService|LoadBalancerService]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_components.json|components.json]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_cache-operations.ts|cache-operations.ts]]
- [[_COMMUNITY_📋 Exhaustive Feature Catalog|📋 Exhaustive Feature Catalog]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_auth-core.service.ts|auth-core.service.ts]]
- [[_COMMUNITY_GitHubService|GitHubService]]
- [[_COMMUNITY_5.1 Address 14 TODOFIXME Items|5.1 Address 14 TODO/FIXME Items]]
- [[_COMMUNITY_fleet-latency-map.tsx|fleet-latency-map.tsx]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_state.schema.ts|state.schema.ts]]
- [[_COMMUNITY_configuration-resolver.service.ts|configuration-resolver.service.ts]]
- [[_COMMUNITY_BaseStateMachineService|BaseStateMachineService]]
- [[_COMMUNITY_retry.schema.ts|retry.schema.ts]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_next.config.ts|next.config.ts]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_dropdown-menu.tsx|dropdown-menu.tsx]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_RoleConfig|RoleConfig]]
- [[_COMMUNITY_RoleConfig|RoleConfig]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_merge-hooks.ts|merge-hooks.ts]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_update-agent-context.sh|update-agent-context.sh]]
- [[_COMMUNITY_nextjs.json|nextjs.json]]
- [[_COMMUNITY_diagnose-build.ts|diagnose-build.ts]]
- [[_COMMUNITY_config.ts|config.ts]]
- [[_COMMUNITY_rule-validator.ts|rule-validator.ts]]
- [[_COMMUNITY_server.tsx|server.tsx]]
- [[_COMMUNITY_nest-cli.json|nest-cli.json]]
- [[_COMMUNITY_AbstractDockerContainerService|AbstractDockerContainerService]]
- [[_COMMUNITY_CoreEventServiceQueryBuilder|CoreEventServiceQueryBuilder]]
- [[_COMMUNITY_catalogs|catalogs]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_template.schema.exhaustive.test.ts|template.schema.exhaustive.test.ts]]
- [[_COMMUNITY_DockerImageAutoScanListenerService|DockerImageAutoScanListenerService]]
- [[_COMMUNITY_mesh-test.module.ts|mesh-test.module.ts]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_decorators.ts|decorators.ts]]
- [[_COMMUNITY_MiddlewareCheck|MiddlewareCheck]]
- [[_COMMUNITY_add.ts|add.ts]]
- [[_COMMUNITY_PermissionBuilder|PermissionBuilder]]
- [[_COMMUNITY_entity.ts|entity.ts]]
- [[_COMMUNITY_Migration Plan — Complete Application Perfection|Migration Plan — Complete Application Perfection]]
- [[_COMMUNITY_permission.repository.ts|permission.repository.ts]]
- [[_COMMUNITY_LocalInitializationService|LocalInitializationService]]
- [[_COMMUNITY_InternalBaseMeshService|InternalBaseMeshService]]
- [[_COMMUNITY_Migration Plan — Global → Scoped Docker|Migration Plan — Global → Scoped Docker]]
- [[_COMMUNITY_❌ Not Working  Stub  Dead Features|❌ Not Working / Stub / Dead Features]]
- [[_COMMUNITY_🛡️ Complete Anti-Pattern Audit Report|🛡️ Complete Anti-Pattern Audit Report]]
- [[_COMMUNITY_hooks.ts|hooks.ts]]
- [[_COMMUNITY_.params|.params]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_ContainerDockerodeNormalizer|ContainerDockerodeNormalizer]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_🧪 Features Using Mock Data|🧪 Features Using Mock Data]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_test-copilot-setup.ts|test-copilot-setup.ts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_CliAuthService|CliAuthService]]
- [[_COMMUNITY_runtime-configuration-accessor.ts|runtime-configuration-accessor.ts]]
- [[_COMMUNITY_RuntimeRuleDsl|RuntimeRuleDsl]]
- [[_COMMUNITY_DockerFileEntry|DockerFileEntry]]
- [[_COMMUNITY_server.tsx|server.tsx]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_test.json|test.json]]
- [[_COMMUNITY_devDependencies|devDependencies]]
- [[_COMMUNITY_Implementation Order (Topological)|Implementation Order (Topological)]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_components.json|components.json]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_context-menu.tsx|context-menu.tsx]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_config.js|config.js]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_init|init]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_MIGRATION-PLAN-OVERVIEW|MIGRATION-PLAN-OVERVIEW.md]]
- [[_COMMUNITY_2. API Backend — Core Infrastructure|2. API Backend — Core Infrastructure]]
- [[_COMMUNITY_🔍 Code Perfection Standards|🔍 Code Perfection Standards]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_development-session.ts|development-session.ts]]
- [[_COMMUNITY_vocabulary.schema.ts|vocabulary.schema.ts]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_QueryBuilder|QueryBuilder]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_LocalEventOutboxDispatcherService|LocalEventOutboxDispatcherService]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_compilerOptions|compilerOptions]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_invalidation.ts|invalidation.ts]]
- [[_COMMUNITY_service.service.ts|service.service.ts]]
- [[_COMMUNITY_migration-journal.service.ts|migration-journal.service.ts]]
- [[_COMMUNITY_fleet-rollout-planner.service.ts|fleet-rollout-planner.service.ts]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_AppLoadingScreen.tsx|AppLoadingScreen.tsx]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_form.tsx|form.tsx]]
- [[_COMMUNITY_environment.schema.ts|environment.schema.ts]]
- [[_COMMUNITY_provider-runner.schema.ts|provider-runner.schema.ts]]
- [[_COMMUNITY_test|test]]
- [[_COMMUNITY_ServiceDependencyGraphPanel.tsx|ServiceDependencyGraphPanel.tsx]]
- [[_COMMUNITY_domain.ts|domain.ts]]
- [[_COMMUNITY_Phase 4 Architecture Cleanup 🏗️|Phase 4: Architecture Cleanup 🏗️]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_SystemMetricsService|SystemMetricsService]]
- [[_COMMUNITY_load-balancer.controller.ts|load-balancer.controller.ts]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_jest|jest]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_utils.ts|utils.ts]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_react.ts|react.ts]]
- [[_COMMUNITY_place-autocomplete.tsx|place-autocomplete.tsx]]
- [[_COMMUNITY_case-utils.ts|case-utils.ts]]
- [[_COMMUNITY_chart.tsx|chart.tsx]]
- [[_COMMUNITY_builder.ts|builder.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_configuration-definition.service.ts|configuration-definition.service.ts]]
- [[_COMMUNITY_Vulnerability Scanning — Per-Project Architecture|Vulnerability Scanning — Per-Project Architecture]]
- [[_COMMUNITY_Dashboard View|Dashboard View]]
- [[_COMMUNITY_Individual Report Summaries|Individual Report Summaries]]
- [[_COMMUNITY_mesh-observable-types.ts|mesh-observable-types.ts]]
- [[_COMMUNITY_docker-common.module.ts|docker-common.module.ts]]
- [[_COMMUNITY_fleet.module.ts|fleet.module.ts]]
- [[_COMMUNITY_DependencyTemplateConfig|DependencyTemplateConfig]]
- [[_COMMUNITY_fleet-rollback.service.ts|fleet-rollback.service.ts]]
- [[_COMMUNITY_nest-cli.json|nest-cli.json]]
- [[_COMMUNITY_app.module.ts|app.module.ts]]
- [[_COMMUNITY_🔴 Anti-Pattern Catalog (Always Reject)|🔴 Anti-Pattern Catalog (Always Reject)]]
- [[_COMMUNITY_makeRoute.tsx|makeRoute.tsx]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_check.ts|check.ts]]
- [[_COMMUNITY_tsconfig.test.json|tsconfig.test.json]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_configuration.schema.ts|configuration.schema.ts]]
- [[_COMMUNITY_peerDependencies|peerDependencies]]
- [[_COMMUNITY_Sub-App Architecture|Sub-App Architecture]]
- [[_COMMUNITY_filtering.ts|filtering.ts]]
- [[_COMMUNITY_drift-reconciliation.service.ts|drift-reconciliation.service.ts]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_orpc|orpc]]
- [[_COMMUNITY_2026-01-01-unified-info-files.migration.ts|2026-01-01-unified-info-files.migration.ts]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_react-library.json|react-library.json]]
- [[_COMMUNITY_exclude|exclude]]
- [[_COMMUNITY_MeshResourceSubQueryBuilder|MeshResourceSubQueryBuilder]]
- [[_COMMUNITY_lint|lint]]
- [[_COMMUNITY_turbo.json|turbo.json]]
- [[_COMMUNITY_progress-tasks.tsx|progress-tasks.tsx]]
- [[_COMMUNITY_🚩 Category 1 Dead Code — Unused Files|🚩 Category 1: Dead Code — Unused Files]]
- [[_COMMUNITY_Auth — Subagent 13 APIBackend Layer|Auth — Subagent 1/3: API/Backend Layer]]
- [[_COMMUNITY_Auth — Subagent 23 WebFrontend Layer|Auth — Subagent 2/3: Web/Frontend Layer]]
- [[_COMMUNITY_common.sh|common.sh]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_entrypoint.dev.ts|entrypoint.dev.ts]]
- [[_COMMUNITY_1. API Backend — Product Modules|1. API Backend — Product Modules]]
- [[_COMMUNITY_BoundedEventLog|BoundedEventLog]]
- [[_COMMUNITY_fleet-failure-containment.service.ts|fleet-failure-containment.service.ts]]
- [[_COMMUNITY_input-group.tsx|input-group.tsx]]
- [[_COMMUNITY_openapi.ts|openapi.ts]]
- [[_COMMUNITY_database|database]]
- [[_COMMUNITY_testing|testing]]
- [[_COMMUNITY_ui|ui]]
- [[_COMMUNITY_tsconfig.test.json|tsconfig.test.json]]
- [[_COMMUNITY_DeploymentLoadBalancerSyncAdapter|DeploymentLoadBalancerSyncAdapter]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_tsconfig.test.json|tsconfig.test.json]]
- [[_COMMUNITY_DockerRuntimeActivityProjectorService|DockerRuntimeActivityProjectorService]]
- [[_COMMUNITY_VariableNumber|VariableNumber]]
- [[_COMMUNITY_exclude|exclude]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_build|build]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_exclude|exclude]]
- [[_COMMUNITY_bun|bun]]
- [[_COMMUNITY_Data Model — Scoped Docker Entities|Data Model — Scoped Docker Entities]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_tsconfig.test.json|tsconfig.test.json]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_schemas.ts|schemas.ts]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_tsconfig.json|tsconfig.json]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_pagination.ts|pagination.ts]]
- [[_COMMUNITY_detect-changed-packages.ts|detect-changed-packages.ts]]
- [[_COMMUNITY_test.controller.ts|test.controller.ts]]
- [[_COMMUNITY_sorting.ts|sorting.ts]]
- [[_COMMUNITY_SetupStateService|SetupStateService]]
- [[_COMMUNITY_exclude|exclude]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_entrypoint.dev.ts|entrypoint.dev.ts]]
- [[_COMMUNITY_dev|dev]]
- [[_COMMUNITY_next|next]]
- [[_COMMUNITY_docker-compose|docker-compose.mdx]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_Startup v2 — Complete Flow Per Case|Startup v2 — Complete Flow Per Case]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_TestHookProvider|TestHookProvider]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_generate|generate]]
- [[_COMMUNITY_docker-compose|docker-compose.mdx]]
- [[_COMMUNITY_add-upstream.ts|add-upstream.ts]]
- [[_COMMUNITY_prepare-render.ts|prepare-render.ts]]
- [[_COMMUNITY_index.tsx|index.tsx]]
- [[_COMMUNITY_FencingTokenIssuer|FencingTokenIssuer]]
- [[_COMMUNITY_StateMachine|StateMachine]]
- [[_COMMUNITY_StateMachineBuilder|StateMachineBuilder]]
- [[_COMMUNITY_service-dag.service.ts|service-dag.service.ts]]
- [[_COMMUNITY_.reportMeshLoad|.reportMeshLoad]]
- [[_COMMUNITY_Auth — Subagent 33 PackagesContracts Layer|Auth — Subagent 3/3: Packages/Contracts Layer]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_debug-file-watcher.ts|debug-file-watcher.ts]]
- [[_COMMUNITY_tsconfig.scripts.json|tsconfig.scripts.json]]
- [[_COMMUNITY_deploy.sh|deploy.sh]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_tsconfig.test.json|tsconfig.test.json]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_.testMethod|.testMethod]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_use-session.ts|use-session.ts]]
- [[_COMMUNITY_8. Detailed File Contents for Context|8. Detailed File Contents for Context]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_peerDependencies|peerDependencies]]
- [[_COMMUNITY_Container Lifecycle — Scoped|Container Lifecycle — Scoped]]
- [[_COMMUNITY_node-startup-check.command.ts|node-startup-check.command.ts]]
- [[_COMMUNITY_Command|Command]]
- [[_COMMUNITY_🔍 UIUX Standards|🔍 UI/UX Standards]]
- [[_COMMUNITY_🔴 NestJS Server Patterns|🔴 NestJS Server Patterns]]
- [[_COMMUNITY_FakeClock|FakeClock]]
- [[_COMMUNITY_🔍 Developer Experience Standards|🔍 Developer Experience Standards]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_query-builder.test.ts|query-builder.test.ts]]
- [[_COMMUNITY_nest-cli.json|nest-cli.json]]
- [[_COMMUNITY_ORPC Auth Patterns — Public vs Protected Routes|ORPC Auth Patterns — Public vs Protected Routes]]
- [[_COMMUNITY_Dead Code, Deprecated APIs & Enhancement Audit Report|Dead Code, Deprecated APIs & Enhancement Audit Report]]
- [[_COMMUNITY_Deployment — API Layer (Subagent 13)|Deployment — API Layer (Subagent 1/3)]]
- [[_COMMUNITY_Service — Subagent 23 WebFrontend Layer|Service — Subagent 2/3: Web/Frontend Layer]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_Docker — Contracts Layer (Subagent 33)|Docker — Contracts Layer (Subagent 3/3)]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_standardize-package-names.ts|standardize-package-names.ts]]
- [[_COMMUNITY_Auth — Complete Analysis (Subagents 1-3 combined)|Auth — Complete Analysis (Subagents 1-3 combined)]]
- [[_COMMUNITY_setup.state.service.ts|setup.state.service.ts]]
- [[_COMMUNITY_mock-node-server.js|mock-node-server.js]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_types.ts|types.ts]]
- [[_COMMUNITY_fix_file|fix_file]]
- [[_COMMUNITY_resource-graph.ts|resource-graph.ts]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_base.ts|base.ts]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_peerDependencies|peerDependencies]]
- [[_COMMUNITY_search.ts|search.ts]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_3. Current Architecture|3. Current Architecture]]
- [[_COMMUNITY_5. Key Files & Their Roles|5. Key Files & Their Roles]]
- [[_COMMUNITY_generate_label|generate_label]]
- [[_COMMUNITY_create-new-feature.sh|create-new-feature.sh]]
- [[_COMMUNITY_next.config.ts|next.config.ts]]
- [[_COMMUNITY_layout.tsx|layout.tsx]]
- [[_COMMUNITY_run-declarative-routing.ts|run-declarative-routing.ts]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_deployer-version.ts|deployer-version.ts]]
- [[_COMMUNITY_Startup v2 — Complete Bootstrap Flow|Startup v2 — Complete Bootstrap Flow]]
- [[_COMMUNITY_tailwind.config.mts|tailwind.config.mts]]
- [[_COMMUNITY_openapi.template.ts|openapi.template.ts]]
- [[_COMMUNITY_New Architecture Image Project Membership|New Architecture: Image Project Membership]]
- [[_COMMUNITY_RoleBuilder|RoleBuilder]]
- [[_COMMUNITY_BaseConfig|BaseConfig]]
- [[_COMMUNITY_BaseConfig|BaseConfig]]
- [[_COMMUNITY_exports|exports]]
- [[_COMMUNITY_API Surface — Scoped Docker Endpoints|API Surface — Scoped Docker Endpoints]]
- [[_COMMUNITY_Function|Function]]
- [[_COMMUNITY_2.4 Service Pages (8 pages + 1 component)|2.4 Service Pages (8 pages + 1 component)]]
- [[_COMMUNITY_build.ts|build.ts]]
- [[_COMMUNITY_Service — Subagent 13 APIBackend Layer|Service — Subagent 1/3: API/Backend Layer]]
- [[_COMMUNITY_Service — Subagent 33 ContractsEntities Layer|Service — Subagent 3/3: Contracts/Entities Layer]]
- [[_COMMUNITY_glob|glob]]
- [[_COMMUNITY_unnamed|./mdx]]
- [[_COMMUNITY_.nextjs|./nextjs]]
- [[_COMMUNITY_files|files]]
- [[_COMMUNITY_paginate.ts|paginate.ts]]
- [[_COMMUNITY_schemas.ts|schemas.ts]]
- [[_COMMUNITY_initialize.schema.ts|initialize.schema.ts]]
- [[_COMMUNITY_node.schema.ts|node.schema.ts]]
- [[_COMMUNITY_Docker — API Layer (Subagent 13)|Docker — API Layer (Subagent 1/3)]]
- [[_COMMUNITY_🔍 Proposing Structural Changes|🔍 Proposing Structural Changes]]
- [[_COMMUNITY_styling|styling]]
- [[_COMMUNITY_compatibility.ts|compatibility.ts]]
- [[_COMMUNITY_post-setup-hints.tsx|post-setup-hints.tsx]]
- [[_COMMUNITY_InlineRoleEntry|InlineRoleEntry]]
- [[_COMMUNITY_ResourceBuilder|ResourceBuilder]]
- [[_COMMUNITY_RoleBuilderWithMeta|RoleBuilderWithMeta]]
- [[_COMMUNITY_schemas.js|schemas.js]]
- [[_COMMUNITY_Handoff Sub-App Orchestration Refactor|Handoff: Sub-App Orchestration Refactor]]
- [[_COMMUNITY_express|express]]
- [[_COMMUNITY_10. Implementation Plan|10. Implementation Plan]]
- [[_COMMUNITY_nanoid|nanoid]]
- [[_COMMUNITY_@nestjsconfig|@nestjs/config]]
- [[_COMMUNITY_1.4 Dead Web Frontend Files|1.4 Dead Web Frontend Files]]
- [[_COMMUNITY_Deployment — Contracts Layer (Subagent 33)|Deployment — Contracts Layer (Subagent 3/3)]]
- [[_COMMUNITY_@nestjsplatform-express|@nestjs/platform-express]]
- [[_COMMUNITY_Project — Complete Analysis Report (Subagents 1-3 combined)|Project — Complete Analysis Report (Subagents 1-3 combined)]]
- [[_COMMUNITY_Project — Subagent 33 ContractsEntities Layer|Project — Subagent 3/3: Contracts/Entities Layer]]
- [[_COMMUNITY_@octokitrest|@octokit/rest]]
- [[_COMMUNITY_Service — Complete Analysis Report (Subagents 1-3 combined)|Service — Complete Analysis Report (Subagents 1-3 combined)]]
- [[_COMMUNITY_Docker — Web Layer (Subagent 23)|Docker — Web Layer (Subagent 2/3)]]
- [[_COMMUNITY_@orpcnest|@orpc/nest]]
- [[_COMMUNITY_@orpcshared|@orpc/shared]]
- [[_COMMUNITY_🔴 Error Handling|🔴 Error Handling]]
- [[_COMMUNITY_@repoauth|@repo/auth]]
- [[_COMMUNITY_@repocontracts-common|@repo/contracts-common]]
- [[_COMMUNITY_🔴 Type Safety — Zero Tolerance for Assertions|🔴 Type Safety — Zero Tolerance for Assertions]]
- [[_COMMUNITY_🔴 React  Next.js Patterns|🔴 React / Next.js Patterns]]
- [[_COMMUNITY_@scalarnestjs-api-reference|@scalar/nestjs-api-reference]]
- [[_COMMUNITY_🔴 Performance — SSE  High-Frequency Streams|🔴 Performance — SSE / High-Frequency Streams]]
- [[_COMMUNITY_@thallespnestjs-better-auth|@thallesp/nestjs-better-auth]]
- [[_COMMUNITY_patch-shared-api-runtime.ts|patch-shared-api-runtime.ts]]
- [[_COMMUNITY_test-interceptor.ts|test-interceptor.ts]]
- [[_COMMUNITY_next-env.d.ts|next-env.d.ts]]
- [[_COMMUNITY_eslint|eslint]]
- [[_COMMUNITY_🔴 Pre-Work Investigation Protocol|🔴 Pre-Work Investigation Protocol]]
- [[_COMMUNITY_@tailwindcsspostcss|@tailwindcss/postcss]]
- [[_COMMUNITY_@typesreact|@types/react]]
- [[_COMMUNITY_eslint|eslint]]
- [[_COMMUNITY_@nestjscli|@nestjs/cli]]
- [[_COMMUNITY_@repoconfig-typescript|@repo/config-typescript]]
- [[_COMMUNITY_source-map-support|source-map-support]]
- [[_COMMUNITY_tsconfig-paths|tsconfig-paths]]
- [[_COMMUNITY_@typesnode|@types/node]]
- [[_COMMUNITY_🔍 Deep Debugging Protocol|🔍 Deep Debugging Protocol]]
- [[_COMMUNITY_2. Tech Stack & Tooling|2. Tech Stack & Tooling]]
- [[_COMMUNITY_7. The New Architecture|7. The New Architecture]]
- [[_COMMUNITY_index|index.mdx]]
- [[_COMMUNITY_page.tsx|page.tsx]]
- [[_COMMUNITY_ClientAuthProviders.tsx|ClientAuthProviders.tsx]]
- [[_COMMUNITY_🚩 Category 4 Technical Debt & Anti-Patterns|🚩 Category 4: Technical Debt & Anti-Patterns]]
- [[_COMMUNITY_eslint-config-prettier|eslint-config-prettier]]
- [[_COMMUNITY_jest|jest]]
- [[_COMMUNITY_@nestjstesting|@nestjs/testing]]
- [[_COMMUNITY_source-map-support|source-map-support]]
- [[_COMMUNITY_supertest|supertest]]
- [[_COMMUNITY_ts-jest|ts-jest]]
- [[_COMMUNITY_ts-node|ts-node]]
- [[_COMMUNITY_tsconfig-paths|tsconfig-paths]]
- [[_COMMUNITY_@typesjest|@types/jest]]
- [[_COMMUNITY_typescript|typescript]]
- [[_COMMUNITY_next-env.d.ts|next-env.d.ts]]
- [[_COMMUNITY_Scoped Docker Architecture — Overview|Scoped Docker Architecture — Overview]]
- [[_COMMUNITY_1. 💀 Dead Core Modules (Superseded)|1. 💀 Dead Core Modules (Superseded)]]
- [[_COMMUNITY_clsx|clsx]]
- [[_COMMUNITY_cmdk|cmdk]]
- [[_COMMUNITY_5. 💀 Dead Web Frontend Files|5. 💀 Dead Web Frontend Files]]
- [[_COMMUNITY_date-fns-tz|date-fns-tz]]
- [[_COMMUNITY_Deployment — Web Layer (Subagent 23)|Deployment — Web Layer (Subagent 2/3)]]
- [[_COMMUNITY_leaflet-draw|leaflet-draw]]
- [[_COMMUNITY_🔴 Dependency Inversion (DI Tokens)|🔴 Dependency Inversion (DI Tokens)]]
- [[_COMMUNITY_🔴 Engineering Mindset & Operating Posture|🔴 Engineering Mindset & Operating Posture]]
- [[_COMMUNITY_lucide-react|lucide-react]]
- [[_COMMUNITY_🔴 ORPC Contract Patterns|🔴 ORPC Contract Patterns]]
- [[_COMMUNITY_🔴 Testing|🔴 Testing]]
- [[_COMMUNITY_🔴 Logging|🔴 Logging]]
- [[_COMMUNITY_🔴 Refactor Incrementalism & Atomic Refactors|🔴 Refactor Incrementalism & Atomic Refactors]]
- [[_COMMUNITY_11. Files to Create, Modify, Delete|11. Files to Create, Modify, Delete]]
- [[_COMMUNITY_12. Rules & Constraints|12. Rules & Constraints]]
- [[_COMMUNITY_13. AppModule Dependencies (Critical)|13. AppModule Dependencies (Critical)]]
- [[_COMMUNITY_react-dom|react-dom]]
- [[_COMMUNITY_react-leaflet|react-leaflet]]
- [[_COMMUNITY_9. Sub-App Definitions (What to Register)|9. Sub-App Definitions (What to Register)]]
- [[_COMMUNITY_@repoenv|@repo/env]]
- [[_COMMUNITY_slug.utils.ts|slug.utils.ts]]
- [[_COMMUNITY_🚩 Category 6 Orphaned Apps & Packages|🚩 Category 6: Orphaned Apps & Packages]]
- [[_COMMUNITY_💡 Enhancement Recommendations|💡 Enhancement Recommendations]]
- [[_COMMUNITY_6. ❌ Not Working — Infrastructure Gaps|6. ❌ Not Working — Infrastructure Gaps]]
- [[_COMMUNITY_@tanstackreact-virtual|@tanstack/react-virtual]]
- [[_COMMUNITY_@typescircular-json|@types/circular-json]]
- [[_COMMUNITY_7. ❌ Not Working — Architectural Gaps|7. ❌ Not Working — Architectural Gaps]]
- [[_COMMUNITY_8. 📋 Planned  Blocked Features|8. 📋 Planned / Blocked Features]]
- [[_COMMUNITY_zod|zod]]
- [[_COMMUNITY_🔴 Type-Driven Development — Make Illegal States Unrepresentable|🔴 Type-Driven Development — Make Illegal States Unrepresentable]]
- [[_COMMUNITY_sw.ts|sw.ts]]
- [[_COMMUNITY_🔴 Monorepo Package Boundaries|🔴 Monorepo Package Boundaries]]
- [[_COMMUNITY_🔴 No Bridges, No Compatibility Layers|🔴 No Bridges, No Compatibility Layers]]
- [[_COMMUNITY_4. What the App Does (Domain)|4. What the App Does (Domain)]]
- [[_COMMUNITY_6. The Problem|6. The Problem]]
- [[_COMMUNITY_@darraghoreslint-plugin-nestjs-typed|@darraghor/eslint-plugin-nestjs-typed]]
- [[_COMMUNITY_build.sh|build.sh]]
- [[_COMMUNITY_idle-monitor.sh|idle-monitor.sh]]
- [[_COMMUNITY_scanner-entrypoint.sh|scanner-entrypoint.sh]]
- [[_COMMUNITY_scanner-wrapper.sh|scanner-wrapper.sh]]
- [[_COMMUNITY_MeshOrchestrationService|MeshOrchestrationService]]
- [[_COMMUNITY_eslint-plugin-mdx|eslint-plugin-mdx]]
- [[_COMMUNITY_eslint-plugin-only-warn|eslint-plugin-only-warn]]
- [[_COMMUNITY_eslint-plugin-react|eslint-plugin-react]]
- [[_COMMUNITY_eslint-plugin-react-hooks|eslint-plugin-react-hooks]]
- [[_COMMUNITY_eslint-plugin-react-refresh|eslint-plugin-react-refresh]]
- [[_COMMUNITY_entrypoint.prod.ts|entrypoint.prod.ts]]
- [[_COMMUNITY_knip.config.ts|knip.config.ts]]
- [[_COMMUNITY_@parcelwatcher|@parcel/watcher]]
- [[_COMMUNITY_chokidar|chokidar]]
- [[_COMMUNITY_eslint-config-prettier|eslint-config-prettier]]
- [[_COMMUNITY_@eslintjs|@eslint/js]]
- [[_COMMUNITY_globals|globals]]
- [[_COMMUNITY_tailwindcss|tailwindcss]]
- [[_COMMUNITY_typescript-eslint|typescript-eslint]]
- [[_COMMUNITY_@typescript-eslinteslint-plugin|@typescript-eslint/eslint-plugin]]
- [[_COMMUNITY_@typescript-eslintparser|@typescript-eslint/parser]]
- [[_COMMUNITY_@typescriptnative-preview|@typescript/native-preview]]
- [[_COMMUNITY_@vercelstyle-guide|@vercel/style-guide]]
- [[_COMMUNITY_vitest|vitest]]
- [[_COMMUNITY_tailwind.config.mts|tailwind.config.mts]]
- [[_COMMUNITY_count.ts|count.ts]]
- [[_COMMUNITY_sortBy.ts|sortBy.ts]]
- [[_COMMUNITY_shared.ts|shared.ts]]
- [[_COMMUNITY_🚩 Category 3 Type Safety Violations|🚩 Category 3: Type Safety Violations]]
- [[_COMMUNITY_eslint|eslint]]
- [[_COMMUNITY_@eslinteslintrc|@eslint/eslintrc]]
- [[_COMMUNITY_next-env.d.ts|next-env.d.ts]]
- [[_COMMUNITY_🚩 Category 5 Unused Dependencies (Production Mode)|🚩 Category 5: Unused Dependencies (Production Mode)]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_@repoconfig-vitest|@repo/config-vitest]]
- [[_COMMUNITY_Appendices|Appendices]]
- [[_COMMUNITY_🔴 Ripple Effect Analysis|🔴 Ripple Effect Analysis]]
- [[_COMMUNITY_🔴 DRY — Don't Repeat Yourself|🔴 DRY — Don't Repeat Yourself]]
- [[_COMMUNITY_🔴 YAGNI — You Aren't Gonna Need It|🔴 YAGNI — You Aren't Gonna Need It]]
- [[_COMMUNITY_🔴 Separation of Concerns|🔴 Separation of Concerns]]
- [[_COMMUNITY_🔴 Error Handling Philosophy|🔴 Error Handling Philosophy]]
- [[_COMMUNITY_🔴 The Boy Scout Rule|🔴 The Boy Scout Rule]]
- [[_COMMUNITY_🔴 Scream Test for Architecture|🔴 "Scream" Test for Architecture]]
- [[_COMMUNITY_🔴 Git Workflow|🔴 Git Workflow]]
- [[_COMMUNITY_🔴 Composition & OpenClosed Principle|🔴 Composition & Open/Closed Principle]]
- [[_COMMUNITY_🔴 Documentation Awareness|🔴 Documentation Awareness]]
- [[_COMMUNITY_🔴 MCP-First Workflow|🔴 MCP-First Workflow]]
- [[_COMMUNITY_🔴 Zod-as-Source-of-Truth for All External Data|🔴 Zod-as-Source-of-Truth for All External Data]]
- [[_COMMUNITY_AccessDeniedError|AccessDeniedError]]
- [[_COMMUNITY_build|build]]
- [[_COMMUNITY_compile|compile]]
- [[_COMMUNITY_start|start]]
- [[_COMMUNITY_testcoverage|test:coverage]]
- [[_COMMUNITY_testui|test:ui]]
- [[_COMMUNITY_testwatch|test:watch]]
- [[_COMMUNITY_dev|dev]]
- [[_COMMUNITY_dockerbuild|docker:build]]
- [[_COMMUNITY_dockerdev|docker:dev]]
- [[_COMMUNITY_generatedirectustypes|generate:directus:types]]
- [[_COMMUNITY_better-auth|better-auth]]
- [[_COMMUNITY_better-sqlite3|better-sqlite3]]
- [[_COMMUNITY_chalk|chalk]]
- [[_COMMUNITY_@nestjsbull|@nestjs/bull]]
- [[_COMMUNITY_@repologger|@repo/logger]]
- [[_COMMUNITY_@repotype-guards|@repo/type-guards]]
- [[_COMMUNITY_web-push|web-push]]
- [[_COMMUNITY_yaml|yaml]]
- [[_COMMUNITY_zod|zod]]
- [[_COMMUNITY_cluster-migration-state.ts|cluster-migration-state.ts]]
- [[_COMMUNITY_schema-version.ts|schema-version.ts]]
- [[_COMMUNITY_seed-version.ts|seed-version.ts]]
- [[_COMMUNITY_babel-plugin-react-compiler|babel-plugin-react-compiler]]
- [[_COMMUNITY_class-variance-authority|class-variance-authority]]
- [[_COMMUNITY_@hookformresolvers|@hookform/resolvers]]
- [[_COMMUNITY_leaflet.fullscreen|leaflet.fullscreen]]
- [[_COMMUNITY_@n0safeenvcli|@n0safe/envcli]]
- [[_COMMUNITY_next|next]]
- [[_COMMUNITY_check-prerequisites.sh|check-prerequisites.sh]]
- [[_COMMUNITY_setup-plan.sh|setup-plan.sh]]
- [[_COMMUNITY_drizzle-kit|drizzle-kit]]
- [[_COMMUNITY_@nextbundle-analyzer|@next/bundle-analyzer]]
- [[_COMMUNITY__BaseMeshServiceClass|_BaseMeshServiceClass]]
- [[_COMMUNITY_deploymentStorageTypeKnownSchema|deploymentStorageTypeKnownSchema]]
- [[_COMMUNITY_next-sitemap|next-sitemap]]
- [[_COMMUNITY_@orpctanstack-query|@orpc/tanstack-query]]
- [[_COMMUNITY_react-day-picker|react-day-picker]]
- [[_COMMUNITY_react-use|react-use]]
- [[_COMMUNITY_react-window|react-window]]
- [[_COMMUNITY_@repoapi-contracts|@repo/api-contracts]]
- [[_COMMUNITY_hasAuthCookie|hasAuthCookie]]
- [[_COMMUNITY_deploymentListConfigSchemas|deploymentListConfigSchemas]]
- [[_COMMUNITY_deploymentStreamListConfigSchemas|deploymentStreamListConfigSchemas]]
- [[_COMMUNITY_coreEventStreamListConfigSchemas|coreEventStreamListConfigSchemas]]
- [[_COMMUNITY_meshEventStreamListConfigSchemas|meshEventStreamListConfigSchemas]]
- [[_COMMUNITY_projectListConfigSchemas|projectListConfigSchemas]]
- [[_COMMUNITY_templateListConfigSchemas|templateListConfigSchemas]]
- [[_COMMUNITY_userListConfigSchemas|userListConfigSchemas]]
- [[_COMMUNITY_meshTrustStrictRolloutPlanInputSchema|meshTrustStrictRolloutPlanInputSchema]]
- [[_COMMUNITY_@repocontracts-common|@repo/contracts-common]]
- [[_COMMUNITY_inviteClient|inviteClient]]
- [[_COMMUNITY_{ statement organizationStatement, ac organizationAc, roles organizationRoles, schemas organizationSchemas, rolesConfig organizationRolesConfig, roleMeta organizationRoleMeta, }|{ statement: organizationStatement, ac: organizationAc, roles: organizationRoles, schemas: organizationSchemas, rolesConfig: organizationRolesConfig, roleMeta: organizationRoleMeta, }]]
- [[_COMMUNITY_{ statement platformStatement, ac platformAc, roles platformRoles, schemas platformSchemas, rolesConfig platformRolesConfig, roleMeta platformRoleMeta, }|{ statement: platformStatement, ac: platformAc, roles: platformRoles, schemas: platformSchemas, rolesConfig: platformRolesConfig, roleMeta: platformRoleMeta, }]]
- [[_COMMUNITY_{ statement projectStatement, ac projectAc, roles projectRoles, schemas projectSchemas, rolesConfig projectRolesConfig, roleMeta projectRoleMeta, }|{ statement: projectStatement, ac: projectAc, roles: projectRoles, schemas: projectSchemas, rolesConfig: projectRolesConfig, roleMeta: projectRoleMeta, }]]
- [[_COMMUNITY_{      statement organizationStatement,      ac organizationAc,      roles organizationRoles,      schemas organizationSchemas,     rolesConfig organizationRolesConfig,     roleMeta organizationRoleMeta, }|{ 
    statement: organizationStatement, 
    ac: organizationAc, 
    roles: organizationRoles, 
    schemas: organizationSchemas,
    rolesConfig: organizationRolesConfig,
    roleMeta: organizationRoleMeta,
}]]
- [[_COMMUNITY_{      statement platformStatement,      ac platformAc,      roles platformRoles,      schemas platformSchemas,     rolesConfig platformRolesConfig,     roleMeta platformRoleMeta, }|{ 
    statement: platformStatement, 
    ac: platformAc, 
    roles: platformRoles, 
    schemas: platformSchemas,
    rolesConfig: platformRolesConfig,
    roleMeta: platformRoleMeta,
}]]
- [[_COMMUNITY_{     statement projectStatement,     ac projectAc,     roles projectRoles,     schemas projectSchemas,     rolesConfig projectRolesConfig,     roleMeta projectRoleMeta, }|{
    statement: projectStatement,
    ac: projectAc,
    roles: projectRoles,
    schemas: projectSchemas,
    rolesConfig: projectRolesConfig,
    roleMeta: projectRoleMeta,
}]]
- [[_COMMUNITY_deploymentEnvironmentSchema|deploymentEnvironmentSchema]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_DetailedBrand|DetailedBrand]]
- [[_COMMUNITY_index.ts|index.ts]]
- [[_COMMUNITY_@repodeclarative-routing|@repo/declarative-routing]]
- [[_COMMUNITY_@repologger|@repo/logger]]
- [[_COMMUNITY_serwist|serwist]]
- [[_COMMUNITY_sonner|sonner]]
- [[_COMMUNITY_@tanstackreact-devtools|@tanstack/react-devtools]]
- [[_COMMUNITY_@tanstackreact-table|@tanstack/react-table]]
- [[_COMMUNITY_@repoconfig-eslint|@repo/config-eslint]]
- [[_COMMUNITY_@repoconfig-prettier|@repo/config-prettier]]
- [[_COMMUNITY_@repoconfig-typescript|@repo/config-typescript]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_meshEventStreamListConfigSchemas|meshEventStreamListConfigSchemas]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Inject|Inject]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Inject|Inject]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_Req|Req]]
- [[_COMMUNITY_Res|Res]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_Implement|Implement]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_Get|Get]]
- [[_COMMUNITY_Post|Post]]
- [[_COMMUNITY_Sse|Sse]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Global|Global]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_Implement|Implement]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Implement|Implement]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Implement|Implement]]
- [[_COMMUNITY_Controller|Controller]]
- [[_COMMUNITY_Sse|Sse]]
- [[_COMMUNITY_UseGuards|UseGuards]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_Option|Option]]
- [[_COMMUNITY_bcrypt|bcrypt]]
- [[_COMMUNITY_bull|bull]]
- [[_COMMUNITY_drizzle-orm|drizzle-orm]]
- [[_COMMUNITY_express-list-routes|express-list-routes]]
- [[_COMMUNITY_mime-types|mime-types]]
- [[_COMMUNITY_@nestjscore|@nestjs/core]]
- [[_COMMUNITY_nestjs-flub|nestjs-flub]]
- [[_COMMUNITY_@nestjsswagger|@nestjs/swagger]]
- [[_COMMUNITY_@octokitapp|@octokit/app]]
- [[_COMMUNITY_Module|Module]]
- [[_COMMUNITY_AuthenticatedUser|AuthenticatedUser]]
- [[_COMMUNITY_BeforeHook|BeforeHook]]
- [[_COMMUNITY_decorator|decorator]]
- [[_COMMUNITY_Hook|Hook]]
- [[_COMMUNITY_OptionalAuth|OptionalAuth]]
- [[_COMMUNITY_UserRoles|UserRoles]]
- [[_COMMUNITY_Catch|Catch]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Inject|Inject]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Inject|Inject]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Injectable|Injectable]]
- [[_COMMUNITY_Optional|Optional]]

## God Nodes (most connected - your core abstractions)
1. `requireAuth()` - 222 edges
2. `vitest` - 207 edges
3. `cn()` - 182 edges
4. `isRecord()` - 155 edges
5. `DockerRepository` - 131 edges
6. `standard` - 124 edges
7. `scripts` - 101 edges
8. `DockerContainerResolutionService` - 94 edges
9. `cn()` - 89 edges
10. `SystemMeshTopologyService` - 86 edges

## Surprising Connections (you probably didn't know these)
- `createORPCClientWithCookies()` --indirect_call--> `AppContract`  [INFERRED]
  apps/web/src/lib/orpc/index.ts → packages/contracts/api/index.ts
- `buildSharedApiRuntimeContext()` --indirect_call--> `AppContract`  [INFERRED]
  apps/api/src/e2e/utils/shared-api-runtime.ts → packages/contracts/api/index.ts
- `defaultOrpcContextBuilder()` --calls--> `isRecord()`  [EXTRACTED]
  apps/api/src/core/modules/auth/plugin-utils/middleware/middleware-converter.ts → packages/utils/type-guards/src/index.ts
- `buildMiddlewareContext()` --calls--> `isRecord()`  [EXTRACTED]
  apps/api/src/core/modules/auth/plugin-utils/middleware/orpc-middleware-proxy.ts → packages/utils/type-guards/src/index.ts
- `toProjectRuntimeConfig()` --calls--> `isRecord()`  [EXTRACTED]
  apps/api/src/core/modules/configuration/services/runtime-configuration-accessor.ts → packages/utils/type-guards/src/index.ts

## Import Cycles
- 3-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-where.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 3-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder-types.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 3-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-executor.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 3-file cycle: `packages/utils/orpc/src/builder/core/route-builder.ts -> packages/utils/orpc/src/builder/output/proxy.ts -> packages/utils/orpc/src/builder/output/builder.ts -> packages/utils/orpc/src/builder/core/route-builder.ts`
- 4-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder-types.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-where.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 4-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-observable-types.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder-types.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 4-file cycle: `apps/api/src/core/modules/mesh/mesh-type-utils.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-executor.ts -> apps/api/src/core/modules/mesh/services/system-mesh-resource-discovery/query/mesh-query-builder-types.ts -> apps/api/src/core/modules/mesh/mesh-type-utils.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/relations.schema.ts -> packages/contracts/entities/src/entities/docker/registries/index.ts -> packages/contracts/entities/src/entities/docker/registries/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/images/index.ts -> packages/contracts/entities/src/entities/docker/images/relations.schema.ts -> packages/contracts/entities/src/entities/docker/registries/index.ts -> packages/contracts/entities/src/entities/docker/registries/relations.schema.ts -> packages/contracts/entities/src/entities/docker/images/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/registries/index.ts -> packages/contracts/entities/src/entities/docker/registries/relations.schema.ts -> packages/contracts/entities/src/entities/docker/stacks/index.ts -> packages/contracts/entities/src/entities/docker/stacks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/registries/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/relations.schema.ts -> packages/contracts/entities/src/entities/docker/volumes/index.ts -> packages/contracts/entities/src/entities/docker/volumes/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/images/index.ts -> packages/contracts/entities/src/entities/docker/images/relations.schema.ts -> packages/contracts/entities/src/entities/docker/volumes/index.ts -> packages/contracts/entities/src/entities/docker/volumes/relations.schema.ts -> packages/contracts/entities/src/entities/docker/images/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/networks/index.ts -> packages/contracts/entities/src/entities/docker/networks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/volumes/index.ts -> packages/contracts/entities/src/entities/docker/volumes/relations.schema.ts -> packages/contracts/entities/src/entities/docker/networks/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/stacks/index.ts -> packages/contracts/entities/src/entities/docker/stacks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/volumes/index.ts -> packages/contracts/entities/src/entities/docker/volumes/relations.schema.ts -> packages/contracts/entities/src/entities/docker/stacks/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/relations.schema.ts -> packages/contracts/entities/src/entities/docker/stacks/index.ts -> packages/contracts/entities/src/entities/docker/stacks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/images/index.ts -> packages/contracts/entities/src/entities/docker/images/relations.schema.ts -> packages/contracts/entities/src/entities/docker/stacks/index.ts -> packages/contracts/entities/src/entities/docker/stacks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/images/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/networks/index.ts -> packages/contracts/entities/src/entities/docker/networks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/stacks/index.ts -> packages/contracts/entities/src/entities/docker/stacks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/networks/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/inspect.schema.ts -> packages/contracts/entities/src/entities/docker/images/index.ts -> packages/contracts/entities/src/entities/docker/images/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/inspect.schema.ts -> packages/contracts/entities/src/entities/docker/networks/index.ts -> packages/contracts/entities/src/entities/docker/networks/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`
- 4-file cycle: `packages/contracts/entities/src/entities/docker/containers/index.ts -> packages/contracts/entities/src/entities/docker/containers/relations.schema.ts -> packages/contracts/entities/src/entities/docker/images/index.ts -> packages/contracts/entities/src/entities/docker/images/relations.schema.ts -> packages/contracts/entities/src/entities/docker/containers/index.ts`

## Communities (986 total, 290 thin omitted)

### Community 0 - "TypeScript Type Utilities"
Cohesion: 0.05
Nodes (9): DomainAdapter, ProjectDomainRepository, Injectable, OrganizationDomainController, Controller, Implement, ProjectDomainController, Controller (+1 more)

### Community 1 - "Mesh Networking Contracts"
Cohesion: 0.01
Nodes (163): meshConnectPeerContract, meshConsumeJoinGrantContract, meshContract, meshControlEnvelopePublishContract, meshControlEnvelopePublishOps, meshDisconnectPeerContract, meshEventStreamByIdOps, meshEventStreamListConfig (+155 more)

### Community 2 - "Deployment Domain Schemas"
Cohesion: 0.01
Nodes (172): Deployment, DeploymentCompiledPlan, deploymentCompiledPlanSchema, DeploymentCompiledPlanSnapshot, DeploymentCompiledPlanSnapshotByRunResult, deploymentCompiledPlanSnapshotByRunResultSchema, deploymentCompiledPlanSnapshotSchema, DeploymentCompiledRollbackGraph (+164 more)

### Community 3 - "Admin Auth Plugin"
Cohesion: 0.12
Nodes (5): AdminMiddlewareDefinition, RequireAdminRoleCheck, AdminPermissionsPlugin, InferRoleNamesFromBuilder, InferStatementFromBuilder

### Community 4 - "HTTP/Traefik Router Builder"
Cohesion: 0.06
Nodes (58): MiddlewareBuilder, CorsOptions, EntrypointConfig, HealthCheck, IPWhiteListConfig, RateLimitConfig, Server, TLSOptions (+50 more)

### Community 5 - "Declarative Routes"
Cohesion: 0.02
Nodes (84): Route, Route, Route, Route, Route, Route, Route, Route (+76 more)

### Community 6 - "Traefik Configuration Repository"
Cohesion: 0.04
Nodes (16): CreateTraefikMiddleware, TraefikMiddleware, CreateConfigFileInput, CreateDomainRouteInput, CreateMiddlewareInput, CreateServiceConfigInput, CreateServiceTargetInput, CreateSSLCertificateInput (+8 more)

### Community 7 - "Permission & Role Checks"
Cohesion: 0.03
Nodes (80): AdminAuthConstraint, AdminPluginFactory, HasPermissionByRoleCheck, HasPermissionCheck, HasRoleCheck, AnyMiddlewareDefinition, AuthWithSessionAPI, BaseAuthConstraint (+72 more)

### Community 8 - "Docker Container Inspection"
Cohesion: 0.04
Nodes (43): cancelEnvelopeSchema, correlationInputSchema, dockerContainerInspectQuerySchema, DockerContainerInspectRequestPayload, DockerContainerInspectResponsePayload, dockerContainerInspectResponsePayloadSchema, DockerContainerListMeta, dockerContainerListMetaSchema (+35 more)

### Community 9 - "Mesh Core & Orchestration"
Cohesion: 0.05
Nodes (64): SystemMeshEventService, Injectable, MESH_TOPIC_SERVICES, MESH_TOPOLOGY_SERVICES, MeshRuntimeModule, Module, MeshConfigTrustKeys, NOTE: if the setup wizard has already run, this is a no-op (row exists). (+56 more)

### Community 10 - "Docker Container Actions"
Cohesion: 0.04
Nodes (93): dockerContainerActionsContract, dockerContainerCreateDirectoryContract, dockerContainerCreateDirectoryOps, dockerContainerDeletePathContract, dockerContainerDeletePathOps, dockerContainerFilesystemContract, dockerContainerFilesContract, dockerContainerFilesOps (+85 more)

### Community 11 - "Docker Module Wiring"
Cohesion: 0.04
Nodes (50): MeshControlEnvelopeAck, meshControlEnvelopeAckSchema, MeshControlEnvelopePublishResult, meshControlEnvelopeSchema, MeshControlEnvelopeType, meshControlEnvelopeTypeSchema, MeshEventPublishPayload, meshEventPublishPayloadSchema (+42 more)

### Community 12 - "Mesh Topology Services"
Cohesion: 0.06
Nodes (9): MeshClusterSyncService, Injectable, MeshQueuePartitionPlanInput, MeshQueuePartitionPlanResult, MeshPeerConnection, MeshPeerHeartbeatInput, MeshPeerHeartbeatResult, MeshPeerLinkMetrics (+1 more)

### Community 13 - "Docker Runtime Activity Projection"
Cohesion: 0.03
Nodes (104): DockerKnownSource, DockerRawEvent, RuntimeEventFieldFilter, RuntimeEventFilterNode, StreamQuery, DockerRuntimeSystemEvent, dockerListMetaSchema, dockerContainerEntitySchema (+96 more)

### Community 14 - "Mesh Cluster Repository"
Cohesion: 0.06
Nodes (29): makeFieldRef(), MeshFieldKey, MeshFieldMap, MeshFieldRef, meshFieldRefSymbol, meshFields(), isMeshResourceOfKind(), MeshResourceQueryBuilder (+21 more)

### Community 15 - "Deployment Pipeline Service"
Cohesion: 0.03
Nodes (77): BASE_PHASE_TRANSITIONS, BASE_RETRY_POLICIES, CANCELLABLE_STATUSES, TODO: Queue retry job via orchestration module (not yet migrated), StreamEventWithMeta, StreamReplayInput, deploymentCompilePlanContract, deploymentCompilePlanPreviewContract (+69 more)

### Community 16 - "hooks.ts"
Cohesion: 0.03
Nodes (101): coerceContainerLogEntry(), CONTAINER_METRICS_STREAM_ACTIONS, debugDockerModalLogs, DockerContainerDetailModalContent(), DockerContainerDetailModalContentProps, DockerContainerDetailModalTriggerProps, DockerTerminalSessionState, extractContainerLogEntries() (+93 more)

### Community 17 - "cn"
Cohesion: 0.04
Nodes (84): DataTableResizer(), Avatar(), AvatarBadge(), AvatarFallback(), AvatarGroup(), AvatarGroupCount(), AvatarImage(), Breadcrumb() (+76 more)

### Community 18 - "SystemMeshTopologyService"
Cohesion: 0.05
Nodes (24): SystemMeshTopicService, Injectable, Inject, MeshTopicRegistryService, Injectable, SystemMeshTopologyService, Injectable, observableToAsyncIterable() (+16 more)

### Community 19 - "types.ts"
Cohesion: 0.10
Nodes (30): BASE_CONFIGURATION, MOCK_PROJECT_CONFIGURATIONS, DependencyGraphMockScenario, PlatformMockOverview, RequiredEnvironmentMap, RequiredEnvironmentName, DependencyGraphMockScenario, DependencyPolicyAdvanced (+22 more)

### Community 20 - "index.ts"
Cohesion: 0.05
Nodes (34): serviceDomainMappings, verificationMethodEnum, verificationStatusEnum, BasePathValidationResult, SubdomainAvailabilityResult, SubdomainConflict, SubdomainValidationResult, InsertOrganizationDomain (+26 more)

### Community 21 - "index.ts"
Cohesion: 0.05
Nodes (44): MeshEntity, MeshOperation, BaseMeshService(), BaseMeshServiceConstructor, buildEntityOperationContracts(), getEntityOperations(), getEntityQueries(), MeshCallManyMetrics (+36 more)

### Community 22 - "scripts"
Cohesion: 0.02
Nodes (101): scripts, add:template, api, build, check, check-env, check-env:api, check-env:doc (+93 more)

### Community 23 - "AuthCoreService"
Cohesion: 0.04
Nodes (23): AuthModuleOptions, AuthWithPlugins, { ConfigurableModuleClass, OPTIONS_TYPE, ASYNC_OPTIONS_TYPE }, MODULE_OPTIONS_TOKEN, PermissionApiExtras, WithPermissionApis, AuthContextErrorMap, AuthErrorType (+15 more)

### Community 24 - "platform-domain.builder.ts"
Cohesion: 0.05
Nodes (44): DependencyAttachmentMode, dependencyAttachmentModeSchema, DependencyEnvironmentDiffStatus, dependencyEnvironmentDiffStatusSchema, DependencyFailureMode, dependencyFailureModeSchema, DependencyHealthGate, dependencyHealthGateSchema (+36 more)

### Community 25 - "index.ts"
Cohesion: 0.02
Nodes (88): ConfigFile, configFiles, configFileSchema, configFilesRelations, CreateConfigFile, createConfigFileSchema, CreateDomainConfig, createDomainConfigSchema (+80 more)

### Community 26 - "mesh-connection-registry.ts"
Cohesion: 0.09
Nodes (34): MeshConnectionRegistry, PromotionDecision, Injectable, ConnectionLifecycleEvent, ConsumerRegistration, MeshConnection, MeshStreamEnvelope, NodeTarget (+26 more)

### Community 27 - "mesh-contracts.ts"
Cohesion: 0.03
Nodes (78): meshConnectPeerContract, meshConsumeJoinGrantContract, meshControlEnvelopePublishContract, meshControlEnvelopePublishOps, meshDisconnectPeerContract, meshEventStreamByIdOps, meshEventStreamListConfig, MeshEventStreamListInput (+70 more)

### Community 28 - "docker.large.mock.ts"
Cohesion: 0.06
Nodes (72): DockerStackDetailModalTrigger(), DockerStackDetailModalTriggerProps, aggregateDeploymentMetrics(), buildBuilderLogs(), BuilderLogProjection, buildReplicaMetrics(), createDeploymentLogsSearch(), createReplicaLogsSearch() (+64 more)

### Community 29 - "query-builder.ts"
Cohesion: 0.08
Nodes (24): EntityOperationOptions, ListOperationOptions, StandardOperations, standard, Input, userSchema, validCreateUser, validUser (+16 more)

### Community 30 - "DockerRuntimeRunnerService"
Cohesion: 0.07
Nodes (35): DelegatedRuntimeRunnerBase, BuildpackRunnerOptions, buildpackRunnerOptionsSchema, DockerComposeRunnerOptions, dockerComposeRunnerOptionsSchema, DockerfileRunnerOptions, dockerfileRunnerOptionsSchema, NixpacksRunnerOptions (+27 more)

### Community 31 - "DeploymentTriggerInput"
Cohesion: 0.07
Nodes (56): QueueJobLike, DeploymentStoragePolicyResolver, ResolveDeploymentStoragePolicyInput, DeploymentStoragePolicy, deploymentStoragePolicyBaseSchema, deploymentStoragePolicyLooseSchema, deploymentStoragePolicySchema, deploymentStoragePolicyWithoutTypeSchema (+48 more)

### Community 32 - "DeploymentService"
Cohesion: 0.06
Nodes (36): deploymentApplyPhaseTransitionContract, deploymentListPhaseTransitionsContract, deploymentPhaseTransitionApplyOps, deploymentPhaseTransitionsCatalogOps, deploymentPhaseTransitionValidationOps, deploymentValidatePhaseTransitionContract, CacheStrategy, DeploymentEnvironment (+28 more)

### Community 33 - "stream.ts"
Cohesion: 0.02
Nodes (127): deploymentEventContracts, deploymentCancelContract, deploymentCancelOps, deploymentCancelOutputSchema, deploymentDeleteContract, deploymentFindByIdContract, deploymentGetLogsContract, deploymentGetLogsOps (+119 more)

### Community 34 - "devDependencies"
Cohesion: 0.02
Nodes (89): devDependencies, @asteasolutions/zod-to-openapi, babel-plugin-react-compiler, @better-fetch/fetch, dotenv, dotenv-cli, dotenv-expand, eslint (+81 more)

### Community 35 - "reporting.ts"
Cohesion: 0.10
Nodes (20): analyticsCreateReportConfigOps, analyticsDeleteReportConfigOps, analyticsDeleteReportOps, analyticsDownloadReportOps, analyticsDownloadReportOutputSchema, analyticsGenerateReportOps, analyticsGenerateReportOutputSchema, analyticsGetReportOps (+12 more)

### Community 36 - "docker.repository.ts"
Cohesion: 0.04
Nodes (64): dockerRuntimeActivities, dockerRuntimeActivityCategoryEnum, dockerRuntimeActivitySeverityEnum, dockerRuntimeActivityStatusEnum, dockerImageLifecycleStateEnum, dockerImageSecurityLifecycle, dockerImageSecurityScanHistory, dockerImageSecurityScans (+56 more)

### Community 37 - "docker-entity-domain.service.ts"
Cohesion: 0.07
Nodes (25): DockerEntityCacheService, DockerEntityFlatKindPayload, DockerEntityKindPayload, Injectable, DockerEntityDomainService, EntityFilterInput, InspectKindResult, KINDS_WITH_FLAT_LIST (+17 more)

### Community 38 - "index.ts"
Cohesion: 0.07
Nodes (29): CreateAdminMiddlewareOptions, AdminPlugin, AdminPluginInstance, AdminPluginWrapperOptions, ApiMethodsWithAdminPlugin, AuthWithAdminPlugin, NOTE: Uses only TPermissionBuilder as generic parameter - TStatement and TRoles, NOTE: Uses only TPermissionBuilder and TAuth as generic parameters. (+21 more)

### Community 39 - "BaseEventService"
Cohesion: 0.06
Nodes (7): BaseEventService, EventLogPersistenceAdapter, TestEventService, dxContracts, DxEventService, typedContracts, TypedDeploymentEventService

### Community 40 - "ProjectService"
Cohesion: 0.05
Nodes (11): ProjectController, Controller, Implement, ProjectRepository, transformCollaborator(), transformEnvironment(), transformProject(), transformTemplate() (+3 more)

### Community 41 - "middlewares.ts"
Cohesion: 0.09
Nodes (23): AuthUtils, AuthUtilsEmpty, buildMeshContext(), parseParcour(), readEnv(), resolveSharedSecret(), createOutputFn(), NOTE: accessControl has been removed in favor of plugin-based middlewares (+15 more)

### Community 42 - "index.ts"
Cohesion: 0.03
Nodes (132): ContainerHashInput, DockerContainerHealth, dockerContainerHealthSchema, DockerContainerStatus, dockerContainerStatusSchema, DockerListMeta, DockerContainerManagedBy, dockerContainerManagedBySchema (+124 more)

### Community 43 - "build-tools.ts"
Cohesion: 0.07
Nodes (68): absoluteFilePath(), Config, ConfigSchema, getConfig(), hasConfig(), writeConfig(), main(), init (+60 more)

### Community 44 - "index.ts"
Cohesion: 0.06
Nodes (28): DockerImageInspectRequestPayload, DockerImageInspectResponsePayload, DockerImagesApplicationService, Injectable, DockerImagesOrchestratorService, Injectable, ActiveImageScanStream, DockerImageSecurityScanService (+20 more)

### Community 45 - "index.ts"
Cohesion: 0.07
Nodes (52): DockerRegistry, DockerRegistryAuthMode, dockerRegistryAuthModeSchema, dockerRegistrySchema, DockerRegistryStatus, dockerRegistryStatusSchema, dockerRegistryRepositoryDetailSchema, DockerRegistrySummary (+44 more)

### Community 46 - "CoreEventSyncService"
Cohesion: 0.07
Nodes (27): AliasForService, AnyServiceSelection, ContractsOf, CoreEventAdapterEvent, CoreEventAdapterInput, CoreEventNamespaceAdapter, CoreEventNamespaceBuilder, CoreEventQueryJoinInput (+19 more)

### Community 47 - "LocalDatabaseService"
Cohesion: 0.08
Nodes (14): test(), bootstrap(), CLIModule, logger, logger, NOTE: We use string tokens instead of Symbols because the @Inject() decorator, AuthModule, createAuthMiddleware() (+6 more)

### Community 48 - "permission-engine.ts"
Cohesion: 0.12
Nodes (21): PermissionService, ctx, mockEngine, orgRoleRule, rule, Injectable, ColumnResolver, buildDynamicVars() (+13 more)

### Community 49 - "VariableRegistry"
Cohesion: 0.18
Nodes (11): buildMiddlewareContext(), codeToStatus(), createOrpcError(), ExtractFirstParamType, mapToOrpcErrorCode(), MiddlewareDefinitionLike, MiddlewareMethodNames, OrpcError (+3 more)

### Community 50 - "deployment-queue-lifecycle.service.ts"
Cohesion: 0.04
Nodes (72): assertClaimedByWorker(), parseClaimResult(), parseDeadLetterJob(), parseDeadLetterListResult(), parseDeadLetterReplayResult(), parseEnqueueResult(), parseHeartbeatResult(), parseQueueJob() (+64 more)

### Community 51 - "context-filter-debug.ts"
Cohesion: 0.07
Nodes (51): ContextFilterDebugLogger, ContextFilterDebugPredicate, createContextFilterDebugLogger(), WEB_CONTEXT_FILTER_VALUE, webLogger, toAbsoluteUrl(), debugMiddleware, debugMiddlewareError (+43 more)

### Community 52 - "mesh-query-builder-types.ts"
Cohesion: 0.04
Nodes (53): AnyRecord, MeshListenResult, applyOrdering(), MeshOrderClause, MeshOrderDirection, applyPagination(), defaultPaginationState, MeshPaginationResult (+45 more)

### Community 53 - "docker-runtime-activity-domain.service.ts"
Cohesion: 0.05
Nodes (34): DockerRuntimeActivityPersistenceService, Injectable, DockerRuntimeActivityDomainService, Injectable, DockerRuntimeOrchestratorService, Injectable, DockerRuntimeActivityRepository, Injectable (+26 more)

### Community 54 - "index.ts"
Cohesion: 0.05
Nodes (85): TemplateCompatibilityMatrix, TemplateCompatibilityMatrixItem, templateCompatibilityMatrixItemSchema, TemplateCompatibilityValidationInput, TemplateCompatibilityValidationResult, BuildTemplateConfig, buildTemplateConfigSchema, dependencyTemplateConfigSchema (+77 more)

### Community 55 - "index.ts"
Cohesion: 0.06
Nodes (72): createZodListOptions(), InferIdSchema, QueryConfigFromOptions, ZodConfigSchema, ZodListOperationOptions, ALL_FILTER_OPERATORS, ARRAY_OPERATORS, arrayField() (+64 more)

### Community 56 - "core-event-sync.service.ts"
Cohesion: 0.04
Nodes (44): AnyEventEmission, BASE_EVENT_SERVICE_SYMBOL, BaseEventServiceRegistryEntry, BaseEventServiceSymbolized, BufferedEventRecord, EventSubscription, EventSubscriptionData, MergedDomainEventEnvelope (+36 more)

### Community 57 - "DockerRuntimeEventsSourceService"
Cohesion: 0.07
Nodes (19): DockerContainerRuntimeEventsService, Injectable, DockerDomainRuntimeEventsService, Injectable, DockerRuntimeEventsSourceService, Injectable, DockerImageRuntimeEventsService, Injectable (+11 more)

### Community 58 - "index.ts"
Cohesion: 0.06
Nodes (29): testContracts, contractBuilder(), EventContract, EventContractBuilder, BASE_DOMAIN_STREAM_SERVICE_SYMBOL, BaseDomainStreamServiceSymbolized, DomainStreamMergeRegisteredOptions, DomainStreamSelection (+21 more)

### Community 59 - "docker-image-detail-modal.tsx"
Cohesion: 0.06
Nodes (39): DockerFlowTabGroup(), DockerFlowTabGroupProps, DockerFlowTabItem, buildScanEventKey(), buildVulnerabilityRowKey(), DockerImageDetailContent(), DockerImageDetailContentProps, DockerImageDetailModalProps (+31 more)

### Community 60 - "requireAuth"
Cohesion: 0.05
Nodes (8): authMiddleware(), requireAuth(), requireMesh(), requirePlatformRole(), DeploymentController, DockerController, Controller, Implement

### Community 61 - "docker-container-runtime-mesh.service.ts"
Cohesion: 0.02
Nodes (98): cancelEnvelopeSchema, containerCreateDirectoryBodySchema, containerDeletePathBodySchema, containerFilesQuerySchema, containerFilesSnapshotSchema, containerLogsQuerySchema, containerLogsSnapshotSchema, containerMutationAckSchema (+90 more)

### Community 62 - "dependencies"
Cohesion: 0.03
Nodes (75): dependencies, better-auth, circular-json, concurrently, cross-env, date-fns, esbuild-wasm, exceljs (+67 more)

### Community 63 - "page.tsx"
Cohesion: 0.04
Nodes (78): DockerDetailLoadingStateProps, DockerInlineLoadingState(), DockerInlineLoadingStateProps, DockerTableLoadingRows(), DockerTableLoadingRowsProps, DockerBatchOperationsBar(), DockerBatchOperationsBarProps, DockerOperationProgressPanel() (+70 more)

### Community 64 - "DockerRepository"
Cohesion: 0.05
Nodes (8): DOCKER_NETWORK_SCOPES, DockerRepository, ScannerRunProgressOptions, Injectable, DockerImageSecurityRepository, Injectable, DockerContainerLinkPath, DockerContainerWithLinks

### Community 65 - "ServiceDomainMappingRepository"
Cohesion: 0.06
Nodes (17): CoreDomainModule, Global, Module, InsertServiceDomainMapping, ServiceDomainMapping, ServiceDomainMappingWithUrls, ServiceDomainMappingRepository, Injectable (+9 more)

### Community 66 - "cn"
Cohesion: 0.08
Nodes (38): Command(), CommandDialog(), CommandEmpty(), CommandGroup(), CommandInput(), CommandItem(), CommandList(), CommandSeparator() (+30 more)

### Community 67 - "page.tsx"
Cohesion: 0.06
Nodes (54): CreateOrganizationPage(), localOrganizationSchema, ProvisioningMode, remoteConnectSchema, AdminSystemPage(), MeshLookupHistoryEntry, MeshRoutePlanHistoryEntry, fleetEndpoints (+46 more)

### Community 68 - "InitializationService"
Cohesion: 0.08
Nodes (16): MeshContract, MeshInitializationModule, Module, MeshBootstrapConfig, MeshInitializationService, MeshSetupSession, MeshVersionService, MeshVersionSummary (+8 more)

### Community 69 - "BaseTriggerService"
Cohesion: 0.07
Nodes (24): createResultBridge(), BridgeResult, SUB_APP_RESULT(), store, SubAppResultStore, logger, SubAppRunner, BaseTriggerService (+16 more)

### Community 70 - "isObjectLike"
Cohesion: 0.09
Nodes (54): s, CONFIG_SYMBOL, ObjectSchema, SchemaWithConfig, withConfig(), comparisonField(), createFieldFilterSchema(), createFilteringConfigSchema() (+46 more)

### Community 71 - "index.ts"
Cohesion: 0.07
Nodes (22): redirect(), mocks, SignInPageLoose, loginSchema, signupSchema, mocks, SetupPageLoose, SetupWizard() (+14 more)

### Community 72 - "index.ts"
Cohesion: 0.04
Nodes (68): MESH_ERROR_CODES, MESH_ERROR_HTTP_STATUS, MESH_ERROR_ORPC_CODE, meshDomainErrorContracts(), MeshDomainErrorPayload, meshDomainErrorPayloadSchema, MeshErrorCode, MeshErrorResponse (+60 more)

### Community 73 - "common.ts"
Cohesion: 0.08
Nodes (28): ORPCGlobalContext, HOOKS, BootstrapModule, DatabaseModule, GlobalDatabaseModule, logger, requireDatabaseUrl(), useFactoryDrizzle() (+20 more)

### Community 75 - "dependencies"
Cohesion: 0.03
Nodes (65): dependencies, bcrypt, bull, drizzle-orm, drizzle-zod, eslint, express-list-routes, mime-types (+57 more)

### Community 76 - "PlatformRole"
Cohesion: 0.28
Nodes (5): AccessControl, PlatformResource, PlatformRole, ResourcePermission, UsePermissionsResult

### Community 77 - "GlobalDatabaseService"
Cohesion: 0.05
Nodes (16): deploymentCache, githubDeploymentRules, imageProjectMembership, projectScanConfig, GlobalDatabase, GlobalDatabaseService, BaseDatabaseService, DeploymentCache (+8 more)

### Community 78 - "TraefikFileSystemService"
Cohesion: 0.05
Nodes (18): traefikServiceConfigs, DownloadResult, FileContentResult, FileOperationResult, FileSystemNamingConventions, ParsedServiceConfigFileName, TraefikFileSystemPaths, VirtualDirectoryTree (+10 more)

### Community 80 - "devDependencies"
Cohesion: 0.03
Nodes (63): @anatine/esbuild-decorators, devDependencies, @anatine/esbuild-decorators, bun-types, @nestjs/cli, @nestjs/schematics, @nestjs/testing, @repo/config-eslint (+55 more)

### Community 81 - "github.module.ts"
Cohesion: 0.05
Nodes (30): DeploymentCoreModule, Module, PreviewCleanupDecision, PreviewCleanupPolicyService, PreviewCleanupTrigger, Injectable, PreviewNamingInput, PreviewNamingResult (+22 more)

### Community 82 - "DeploymentMeshService"
Cohesion: 0.07
Nodes (21): ConfigurationCoreModule, Module, DeploymentModule, Module, DeploymentMeshHandlerRegistrar, Injectable, DeploymentMeshBase, DeploymentMeshService (+13 more)

### Community 83 - "deployment.service.spec.ts"
Cohesion: 0.08
Nodes (27): buildpackRunnerAdvancedOptionsSchema, dockerComposeRunnerAdvancedOptionsSchema, dockerfileRunnerAdvancedOptionsSchema, nixpacksRunnerAdvancedOptionsSchema, railpackRunnerAdvancedOptionsSchema, runtimeRunnerOptionsSchema, customSourceCheckoutContextSchema, DeploymentSourceCheckoutContext (+19 more)

### Community 84 - "runtime-configuration.schema.ts"
Cohesion: 0.03
Nodes (60): configurationEnvironmentDomainSchema, configurationLifecycleStateSchema, configurationProviderTypeSchema, configurationRunnerTypeSchema, configurationSourceProviderSchema, configurationStrategySchema, configurationTriggerSchema, DomainEnvironmentVariables (+52 more)

### Community 85 - "index.ts"
Cohesion: 0.06
Nodes (33): DomainError, DomainAlreadyExistsError, DomainDeletionError, DomainNotVerifiedError, GranularityViolationError, InvalidDomainFormatError, OrganizationDomainNotFoundError, ProjectDomainAlreadyExistsError (+25 more)

### Community 86 - "mdx-components.tsx"
Cohesion: 0.05
Nodes (40): browserCollections, create, docs, create, create, { GET }, Layout(), generateMetadata() (+32 more)

### Community 87 - "MeshControlEnvelope"
Cohesion: 0.07
Nodes (7): MeshEnvelopeSideEffectsService, Injectable, MeshTrustService, Injectable, MeshControlEnvelope, MeshMembershipReconcileInput, MeshMembershipReconcileResult

### Community 88 - "index.ts"
Cohesion: 0.06
Nodes (27): ValidationIssue, ConfigBuildError, ConfigNotFoundError, ConfigSyncError, ConfigValidationError, TraefikError, DirectoryError, FileDeleteError (+19 more)

### Community 89 - "standard-schema-helpers.ts"
Cohesion: 0.05
Nodes (27): createPathParamBuilder(), ExtractPathParams, ParamsToSchemaShape, PathParam, PathParamBuilder, PathParamBuilderWithExisting, SchemaShape, BodyBuilder (+19 more)

### Community 90 - "map.tsx"
Cohesion: 0.05
Nodes (52): DropdownMenu(), DropdownMenuRadioGroup(), DropdownMenuTrigger(), LeafletCircle, LeafletCircleMarker, LeafletFeatureGroup, LeafletLayerGroup, LeafletMapContainer (+44 more)

### Community 91 - "devDependencies"
Cohesion: 0.06
Nodes (31): devDependencies, delete-cli, eslint, @eslint/eslintrc, @eslint/js, jsdom, next, @parcel/watcher (+23 more)

### Community 92 - "route-builder.ts"
Cohesion: 0.04
Nodes (32): ErrorDefinitionBuilder, ExtractErrorFromBuilder, ExtractErrorsFromBuilders, CompactDetailedInputShape, CurrentDetailedInputParts, CurrentInputBody, CurrentInputHeaders, CurrentInputParams (+24 more)

### Community 93 - "index.ts"
Cohesion: 0.05
Nodes (64): ClientSessionInjectedProps, ComponentWithDisplayName, CoreRouteElements, DeleteRouteBuilder, emptySchema, FetchOptions, GetRouteBuilder, LinkProps (+56 more)

### Community 94 - "mesh-entity.ts"
Cohesion: 0.04
Nodes (49): nodeInfoEntity, AnyMeshEntity, BoundEntityMutations, BoundEntityQueries, BoundMeshMutation, BoundMeshQuery, getEventTypes(), MeshEntity (+41 more)

### Community 95 - "deployment.ts"
Cohesion: 0.03
Nodes (82): apiKeysRelations, deploymentEnvironmentEnum, deploymentLogs, deploymentLogsRelations, deploymentPhaseEnum, deploymentRollbacks, deploymentRollbacksRelations, deploymentRules (+74 more)

### Community 96 - "AnalyticsService"
Cohesion: 0.07
Nodes (7): AnalyticsModule, Module, AnalyticsController, Controller, Implement, AnalyticsService, Injectable

### Community 97 - "dependency-graph.mock.ts"
Cohesion: 0.09
Nodes (33): ServiceConfigSubNav(), sectionLabel, sectionRoute, ServiceSection, ServiceSectionNav(), ServiceSectionNavProps, DashboardServiceConfigurationEnvironmentPage(), DashboardServiceConfigurationNetworkPage() (+25 more)

### Community 98 - "standard-operations.ts"
Cohesion: 0.05
Nodes (7): projectEndpoints, enhancedProject, ProjectEndpoints, projectInvalidations, projectInvalidationsConfig, CallableInvalidationConfig, InvalidationConfig

### Community 99 - "MeshTrustStrictModeService"
Cohesion: 0.05
Nodes (22): NodeStartupCheckCommand, Command, Injectable, nodeConfig, NodeMeshConfigRepository, Injectable, SystemMeshConfigService, Injectable (+14 more)

### Community 101 - "contracts.ts"
Cohesion: 0.07
Nodes (34): builderMetadataOps, builderSchemaOps, builderValidationOps, getAllBuildersContract, getAllProvidersContract, getBuilderSchemaContract, getCompatibleBuildersContract, getCompatibleProvidersContract (+26 more)

### Community 102 - "data-table.tsx"
Cohesion: 0.10
Nodes (27): CalendarDatePicker, DataTableExport(), DataTableExportProps, ColumnDefWithMeta, ColumnOrderUpdater, DataFetchParams, DataFetchResult, DataTableProps (+19 more)

### Community 103 - "index.ts"
Cohesion: 0.14
Nodes (34): getEmptyPrimitive(), getEnumValues(), getLiteralValue(), getZodObjectDefaults(), getZodObjectShallowDefaults(), inferEmptyValue(), mergeWithDefaults(), asParserBuilder() (+26 more)

### Community 105 - "AppLifecycleService"
Cohesion: 0.07
Nodes (15): BootstrapOrchestratorService, AppLifecycleModule, AppLifecycleEvent, AppLifecyclePhase, AppLifecycleService, BootstrapStep, HealthController, mockAuthUser (+7 more)

### Community 106 - "index.ts"
Cohesion: 0.09
Nodes (52): any(), array(), boolean(), coerceBoolean(), coerceNumber(), date(), enumeration(), extend() (+44 more)

### Community 107 - ".error"
Cohesion: 0.07
Nodes (6): DockerService, Injectable, ScannerContainerManagerService, Injectable, Inject, Optional

### Community 108 - "SetupStepTracker"
Cohesion: 0.16
Nodes (9): SetupStepTracker, EmitEvent, LocalStepId, RemoteStepId, runStep(), SetupStepId, SetupStreamEvent, SetupStreamSnapshotEvent (+1 more)

### Community 109 - "vitest"
Cohesion: 0.04
Nodes (18): buildAllowedOrigins(), isLocalhostOrigin(), normalizeUrl(), testEndpoints, mocks, MockProcess, mockServer, AlertCircle() (+10 more)

### Community 110 - "makeRoute.tsx"
Cohesion: 0.06
Nodes (26): Route, BasePageProps, CoreRouteElements, createRouteInfo(), DeleteRouteBuilder, FetchOptions, GetInfo, GetRouteBuilder (+18 more)

### Community 111 - "index.ts"
Cohesion: 0.06
Nodes (34): DockerContainerFilesSnapshot, ComposeConfig, DockerContainerComposeTab(), DockerContainerComposeTabProps, DockerContainerConfigTab(), DockerContainerConfigTabProps, RuntimeConfig, DockerContainerEnvTab() (+26 more)

### Community 112 - "user.repository.ts"
Cohesion: 0.07
Nodes (18): createImplementMock(), mockAuthUser, Controller, Implement, UserController, CreateUserInput, GetUserOutput, GetUsersInput (+10 more)

### Community 113 - "index.ts"
Cohesion: 0.14
Nodes (9): BuildArtifactResult, RuntimeConvergenceConfig, RuntimeRunnerExecutionOptions, DeploymentExecutionWorkflowService, DeployPhaseRetryPolicy, RuntimeConvergenceConfigResolution, RuntimeConvergenceSource, Injectable (+1 more)

### Community 114 - "shared.ts"
Cohesion: 0.12
Nodes (17): assertAll(), assertAny(), AssertionDefinition, AssertionMetadata, assertNot(), CompositeOperator, CompositePayload, createAssertion() (+9 more)

### Community 115 - "PushRepository"
Cohesion: 0.08
Nodes (15): pushSubscription, userVapidKeys, PushController, Controller, Implement, PushModule, Module, PushRepository (+7 more)

### Community 116 - "deployment-provider-builder-runner-state-machine.service.ts"
Cohesion: 0.04
Nodes (41): DeploymentBuilderKind, deploymentBuilderKindSchema, DeploymentEnvironment, deploymentEnvironmentSchema, DeploymentExecutionStateMachineInput, deploymentExecutionStateMachineInputSchema, DeploymentExecutionStateMachineResolution, DeploymentExecutionStateMachineViolation (+33 more)

### Community 117 - "ServiceService"
Cohesion: 0.06
Nodes (25): runtimeConfigurationAccessor, ProjectCoreModule, Module, ProjectAccessService, Injectable, createDeploymentCompleteStrategyContext(), DeploymentCompleteStrategyContext, seedTestOwner() (+17 more)

### Community 118 - "traefik-middleware-library.service.ts"
Cohesion: 0.12
Nodes (25): AddPrefixOptions, AdminChainOptions, ApiChainOptions, BasicAuthOptions, BufferingOptions, CircuitBreakerOptions, CompressionOptions, CorsMiddlewareOptions (+17 more)

### Community 119 - "index.ts"
Cohesion: 0.07
Nodes (44): runnerNetworkModeSchema, serviceRunnerStrategySchema, DependencyFilterInputSource, dependencyFilterInputSourceSchema, DependencyFilterOperator, dependencyFilterOperatorSchema, DependencyInstanceProvisioning, dependencyInstanceProvisioningSchema (+36 more)

### Community 120 - "isRecord"
Cohesion: 0.12
Nodes (3): publicAccess(), InitializationService, SetupController

### Community 121 - "InternalErrorInsightService"
Cohesion: 0.06
Nodes (26): getInternalErrorRequestContext(), INTERNAL_ERROR_CONTEXT_KEY, InternalErrorRequestContext, InternalErrorContextMiddleware, Injectable, RequestWithInternalErrorContext, setInternalErrorRequestContext(), InternalErrorExceptionFilter (+18 more)

### Community 122 - "file-upload-link.ts"
Cohesion: 0.15
Nodes (12): ActiveUpload, containsFile(), createUploadWorker(), FileUploadContext, fileUploadDebug, FileUploadOpenAPILink, FileUploadProgressEvent, getUploadWorker() (+4 more)

### Community 123 - "index.ts"
Cohesion: 0.05
Nodes (48): templateCompatibilityMatrixOps, templateCompatibilityValidationOps, templateGetCompatibilityMatrixContract, templateValidateCompatibilityContract, templateCreateContract, templateOps, templateDeleteContract, templateOps (+40 more)

### Community 124 - "docker-create-container-modal.tsx"
Cohesion: 0.06
Nodes (37): COMMON_CAPABILITIES, COMMON_GPU_CAPABILITIES, COMMON_ULIMITS, CONFIG_PRESETS, ConfigSet, DockerCreateContainerModal(), DockerCreateContainerModalProps, DockerCreateContainerPayload (+29 more)

### Community 125 - "index.ts"
Cohesion: 0.04
Nodes (46): DismissHintInput, dismissHintInputSchema, ListHintsResult, listHintsResultSchema, SetupBootstrapStrategy, setupBootstrapStrategySchema, SetupHintId, setupHintIdSchema (+38 more)

### Community 126 - "RouteRegistryService"
Cohesion: 0.10
Nodes (15): createGatewayCatchAll(), handleProxy(), proxyRequest(), RouteRegistryService, HttpMethod, RouteEntry, RoutePattern, SubAppRegistration (+7 more)

### Community 128 - "manager.ts"
Cohesion: 0.11
Nodes (18): assertRuntimeDatabaseBinding(), buildAdminDatabaseUrl(), buildBootstrapAsyncFailure(), buildDatabaseUrlWithDatabase(), ExternalSharedPostgresContainerHandle, isSharedRuntimeLoggingEnabled(), isTruthyEnv(), logStep() (+10 more)

### Community 129 - "TraefikSyncService"
Cohesion: 0.09
Nodes (6): CompleteServiceConfig, MiddlewareData, SyncResult, SyncSummary, TraefikSyncService, Injectable

### Community 131 - "TraefikVariableResolverService"
Cohesion: 0.06
Nodes (22): ClassConstructor, resolveSharedRuntimeMaxConcurrency(), RuntimeModuleBuilder, SharedApiRuntimeOptions, SharedOrpcResponseMeta, SharedOrpcResponseTracker, SharedPostgresContainerHandle, SharedRuntimeContainerContext (+14 more)

### Community 133 - "DockerContainerLogEntry"
Cohesion: 0.11
Nodes (19): userCheckEmailContract, userOps, userCountContract, userOps, userCreateContract, userOps, userDeleteContract, userOps (+11 more)

### Community 134 - "setup-wizard.tsx"
Cohesion: 0.15
Nodes (12): defaultLabels, initialState, localLabels, remoteLabels, WizardState, WizardStep, getDescription(), getHeading() (+4 more)

### Community 135 - "helpers.ts"
Cohesion: 0.08
Nodes (30): meshInvalidations, BetterAuthError, buildInvalidationProxy(), buildKeysProxy(), buildTruthyInputProxy(), custom(), CustomContractConfigWithoutSchema, CustomContractConfigWithSchema (+22 more)

### Community 136 - "index.ts"
Cohesion: 0.09
Nodes (24): allConfig(), baseConfig(), BaseConfigOptions, ignoresConfig, testConfig(), tsconfigRootDir, all(), base() (+16 more)

### Community 137 - "DockerRuntimeEvent"
Cohesion: 0.05
Nodes (19): DockerRuntimeActivityProjectorService, Injectable, DockerKnownSource, DockerRuntimeStreamOrchestratorService, InspectTrigger, InspectTriggerOrigin, RuntimeEventFieldFilter, RuntimeEventFilterNode (+11 more)

### Community 138 - "scan.schema.ts"
Cohesion: 0.07
Nodes (39): ScannerRunResult, DockerContainerSecurityTabProps, DockerImageScanFindingEntry, DockerImageLayerEfficiency, dockerImageLayerEfficiencySchema, DockerImageScannerResult, dockerImageScannerResultSchema, DockerImageScannerStatus (+31 more)

### Community 139 - "schemas.ts"
Cohesion: 0.05
Nodes (42): NodeConfigStatus, nodeConfigStatusSchema, SetupBootstrapStrategy, setupBootstrapStrategySchema, SetupInitializeInput, setupInitializeInputSchema, SetupInitializeLocalInput, setupInitializeLocalInputSchema (+34 more)

### Community 140 - "generate-hooks.ts"
Cohesion: 0.10
Nodes (23): createLiveQueryHook(), createMutationHook(), createQueryHook(), createStreamedQueryHook(), ExtractInput, ExtractMutationInput, ExtractMutationOutput, ExtractOutput (+15 more)

### Community 142 - "platform-domain.schema.ts"
Cohesion: 0.06
Nodes (22): cacheStrategyEnum, deploymentCacheRelations, deploymentStrategyEnum, githubApps, githubAppsRelations, githubDeploymentRulesRelations, githubPreviewDeployments, githubPreviewDeploymentsRelations (+14 more)

### Community 143 - "Startup Architecture v2 — Sub-App Trigger Chain"
Cohesion: 0.04
Nodes (45): Clean Teardown Protocol, Concept, Core Concept: Sub-App Trigger Chain, Degraded Mode, Dependency Graph, Design Goals for v2, Error Handling & Recovery, File Manifest (+37 more)

### Community 144 - "shared-api-runtime.ts"
Cohesion: 0.10
Nodes (23): Auth, RuntimeCase, collectStreamEvents(), runRemoteSetup(), runSetup(), SetupEvent, SetupResult, buildSharedApiRuntimeContext() (+15 more)

### Community 145 - "startup-architecture.mdx"
Cohesion: 0.05
Nodes (42): BootstrapOrchestrator Emits DatabaseReadyTrigger, Complete Timeline, Core Principle, Dependency Graph (Complete), Dependency Graph (DI-Driven), Error Handling, Extending the Pattern: Any Dependency, Any Reason, Files to Create or Modify (+34 more)

### Community 146 - "page.tsx"
Cohesion: 0.11
Nodes (33): ContainerStreamEntity, debugContainerLogsStream, resolveFallbackPayloadMessage(), useContainerStreamLogs(), debugDockerLogsFilter, useFilteredLogs(), UseFilteredLogsParams, usePersistedLogGroups() (+25 more)

### Community 147 - "SystemMeshClusterRepository"
Cohesion: 0.09
Nodes (8): SystemMeshClusterRepository, MeshTopicResourceIndexService, Injectable, MeshResourceRegistryService, Injectable, MeshResourceIndexUpsertInput, MeshResourceIndexUpsertResult, MeshResourceLookupInput

### Community 148 - "auth.ts"
Cohesion: 0.07
Nodes (29): account, accountRelations, invitation, invitationRelations, member, memberRelations, organization, organizationRelations (+21 more)

### Community 149 - "organization.repository.ts"
Cohesion: 0.09
Nodes (21): OrganizationController, Controller, Implement, OrganizationRepository, Injectable, OrganizationService, Injectable, organizationAdminContract (+13 more)

### Community 150 - "page.tsx"
Cohesion: 0.08
Nodes (34): cloneValue(), DashboardProjectEnvironmentDetailsPage(), DEPENDENCY_HEALTH_OPTIONS, DEPENDENCY_REQUIREMENT_OPTIONS, DEPENDENCY_STARTUP_OPTIONS, DependencyHealthGate, DependencyRequirement, DependencyStartup (+26 more)

### Community 151 - "traefik-config-builder.ts"
Cohesion: 0.09
Nodes (4): HttpRouterBuilder, RuleBuilder, RuleMatcher, RuleOperator

### Community 152 - "DockerContainerRuntimeService"
Cohesion: 0.10
Nodes (14): TODO: trigger actual health check, ProjectSettings, TODO: integrate with variable-resolver module, projectContract, ProjectListInput, projectQueryStreamContract, ProjectStreamEvent, projectStreamEventContractEntitySchema (+6 more)

### Community 153 - "index.ts"
Cohesion: 0.18
Nodes (16): serviceCreateContract, ServiceCreateInput, serviceCreateInputSchema, serviceDeleteContract, serviceFindByIdContract, serviceCrudContract, serviceObjectSchema, serviceOps (+8 more)

### Community 154 - "page.tsx"
Cohesion: 0.15
Nodes (14): DockerImageDetailModalTrigger(), ContainerColumnsDeps, ContainerInstanceProjection, ContainerProjection, ContainerTableFetchParams, ContainerTableRow, createContainerColumns(), createContainerTableFetchData() (+6 more)

### Community 155 - "index.ts"
Cohesion: 0.13
Nodes (15): createBetterAuth(), Auth, Session, betterAuthFactory(), CachedUser, createMasterSession(), extractDevAuthKey(), getMasterUser() (+7 more)

### Community 156 - "RoleConfigCollection"
Cohesion: 0.05
Nodes (17): AllRoleActions, AllRoleResources, HasAction, HasActionOnResource, HasAllActionsOnResource, HasAnyActionOnResource, HasResource, ReadOnlyRoles (+9 more)

### Community 157 - "scripts"
Cohesion: 0.05
Nodes (39): scripts, auth:generate, build, clean, cli, compile, db:generate, db:global:generate (+31 more)

### Community 158 - "configuration-core.module.ts"
Cohesion: 0.09
Nodes (12): ConfigurationLifecycleState, ConfigurationProviderType, ConfigurationRunnerType, RuntimeConfigurationStateMachine, runtimeConfigurationStateMachineSchema, RuntimeConfigurationStateNodeInput, ConfigurationStateMachineBuilderService, RuntimeConfigurationPolicyDsl (+4 more)

### Community 159 - "DockerContainerRuntimeMeshService"
Cohesion: 0.11
Nodes (6): MeshCallManyOptions, MeshCallManyResult, DockerContainerRuntimeMeshService, Injectable, DockerMeshHandlerRegistrar, Injectable

### Community 160 - "page.tsx"
Cohesion: 0.09
Nodes (35): createImageColumns(), createImageSubRowColumns(), createImageTableFetchData(), ImageActionMenuOption, ImageColumnsDeps, ImageSubRowColumnsDeps, ImageTableFetchParams, applyImageFilters() (+27 more)

### Community 161 - "package.json"
Cohesion: 0.05
Nodes (38): author, import, require, description, devDependencies, prettier, prettier-plugin-tailwindcss, @repo/config-vitest (+30 more)

### Community 162 - "package.json"
Cohesion: 0.05
Nodes (38): dependencies, @repo/auth, @repo/contracts-common, @repo/orpc-utils, @repo/provider-schema, @repo/type-guards, zod, devDependencies (+30 more)

### Community 163 - "utils.ts"
Cohesion: 0.04
Nodes (49): FunctionContext, toAdvancedUseQuery(), toAdvancedUseSuspenseQuery(), toUseMutation(), toUseQuery(), toUseSuspenseQuery(), transformUseReturn(), keysToSnake() (+41 more)

### Community 164 - "dependencies"
Cohesion: 0.06
Nodes (33): dependencies, class-variance-authority, clsx, cmdk, concurrently, date-fns, date-fns-tz, exceljs (+25 more)

### Community 165 - "command.tsx"
Cohesion: 0.09
Nodes (28): DataTableViewOptionsProps, Badge(), badgeVariants, Command(), CommandDialog(), CommandEmpty(), CommandGroup(), CommandInput() (+20 more)

### Community 166 - "TestController"
Cohesion: 0.30
Nodes (11): ApiBearerAuth, ApiOperation, ApiParam, ApiResponse, AllowAnonymous, AuthenticatedUser, Get, OptionalAuth (+3 more)

### Community 167 - "docker-containers-orchestrator.service.ts"
Cohesion: 0.04
Nodes (33): CreateContainerOptions, DockerContainerCreateDirectoryRequestPayload, DockerContainerDeletePathRequestPayload, DockerContainerReadFileSnapshotRequestPayload, DockerContainerRenamePathRequestPayload, DockerContainerTerminalCloseRequestPayload, DockerContainerTerminalInputRequestPayload, DockerContainerTerminalOpenRequestPayload (+25 more)

### Community 168 - "docker-container-runtime.service.ts"
Cohesion: 0.16
Nodes (4): createPluginMiddlewares(), AdminChecksBuilder, ChecksBuilder, OrganizationChecksBuilder

### Community 169 - "hooks.ts"
Cohesion: 0.10
Nodes (27): AdminInvitesPage(), AdminMembersPage(), AdminOrganizationDetailPage(), AdminSettingsPage(), AdminOrganizationsPage(), useCheckMyFleetAdmission(), useCreateMyFleetAdmissionRequest(), useMyFleetAdmissionRequests() (+19 more)

### Community 170 - ".key"
Cohesion: 0.08
Nodes (6): MeshResourceBuilder, compareHlc(), Hlc, LwwEntry, LwwMap, OrSet

### Community 171 - "DeploymentRepository"
Cohesion: 0.08
Nodes (15): DeploymentStateMachineScopeConfigInput, DeploymentEventService, Injectable, DeploymentRepository, getMetaString(), toDto(), toLogDto(), toRollbackDto() (+7 more)

### Community 172 - "page.tsx"
Cohesion: 0.10
Nodes (12): ADVANCED_FILTER_FIELDS, ADVANCED_FILTER_OPERATORS, AdvancedFilterField, AdvancedFilterLogic, AdvancedFilterOperator, AdvancedServiceFilterRule, DependencyRoutePreview, describeAdvancedRule() (+4 more)

### Community 173 - "package.json"
Cohesion: 0.05
Nodes (35): author, bin, runthenkill, dependencies, commander, @repo/type-guards, description, devDependencies (+27 more)

### Community 174 - "index.ts"
Cohesion: 0.06
Nodes (34): coreContract, createFleetAdmissionRequestInputSchema, deleteFleetAllocationInputSchema, fleetAdmissionCandidateSchema, fleetAdmissionCheckInputSchema, fleetAdmissionCheckOps, fleetAdmissionCheckResultSchema, fleetAdmissionRequestOps (+26 more)

### Community 175 - "standard"
Cohesion: 0.12
Nodes (21): DockerImageDetailModal(), DockerKeyValueGrid(), DockerKeyValueGridProps, DockerKeyValueItem, DockerDetailLoadingState(), DockerModalQuickActionItem, DockerModalQuickActions(), DockerModalQuickActionsProps (+13 more)

### Community 176 - "peerDependencies"
Cohesion: 0.10
Nodes (15): AllKeysOptional, AnyObjectSchema, AnyObjectSchemaWithShape, ExtendSchemaType, ExtractShape, getSchemaOperations(), InferredToSchemaShape, IsEmptyRecord (+7 more)

### Community 177 - "DetailedOutputBuilder"
Cohesion: 0.30
Nodes (5): DetailedOutput, DetailedOutputBuilder, isDetailedMode(), OutputSchemaProxy, getSchemaShape()

### Community 178 - "tasks"
Cohesion: 0.17
Nodes (12): cache, dependsOn, inputs, dependsOn, dependsOn, tasks, clean, doc#type-check (+4 more)

### Community 179 - "terminal-viewer.tsx"
Cohesion: 0.22
Nodes (14): ANSI_FG_BRIGHT_PALETTE, ANSI_FG_PALETTE, applySgrCodes(), buildTerminalLinesFromOutput(), computeSegmentStyle(), defaultSgrState(), parseAnsiColorSequence(), parseAnsiSegments() (+6 more)

### Community 180 - "include"
Cohesion: 0.06
Nodes (35): compilerOptions, customConditions, incremental, paths, plugins, target, useDefineForClassFields, exclude (+27 more)

### Community 181 - "types.ts"
Cohesion: 0.09
Nodes (29): testAuthenticatedContract, TestAuthenticatedInput, testAuthenticatedOps, TestAuthenticatedOutput, testFileDownloadContract, TestFileDownloadInput, testFileDownloadOps, TestFileDownloadOutput (+21 more)

### Community 182 - "crud.ts"
Cohesion: 0.06
Nodes (34): listConfigSchemas, paginationConfigSchema, sortingConfigSchema, sortingFieldsArray, testEntityCheckNameContract, TestEntityCheckNameInput, TestEntityCheckNameOutput, testEntityCountContract (+26 more)

### Community 185 - "route-method-meta.ts"
Cohesion: 0.11
Nodes (25): createRouteMethodMeta(), getRouteMethod(), HasRouteMethodMeta, IsRouteBuilderGetMethod, IsRouteBuilderMutation, IsRouteBuilderNonGetMethod, IsRouteBuilderQuery, RouteMethodMeta (+17 more)

### Community 186 - "init.ts"
Cohesion: 0.09
Nodes (34): applyTransformers(), buildEnvSections(), Colors, createPromptConfig(), DateConstraints, debugLog(), dotenv, DotenvConfig (+26 more)

### Community 187 - "TraefikEventService"
Cohesion: 0.09
Nodes (15): cleanupContract, CleanupInput, CleanupOutput, configChangeContract, ConfigChangeInput, ConfigChangeOutput, configSyncContract, ConfigSyncInput (+7 more)

### Community 188 - "page.tsx"
Cohesion: 0.07
Nodes (19): CompiledContextFilterPattern, ContextFilterLogger, ContextFilterLoggerOptions, ContextFilterLoggerSource, ContextFilterPatternKind, ExtractedSourceIdentity, AppLogger, AppLoggerContextFilterOptions (+11 more)

### Community 189 - "usePermissions.ts"
Cohesion: 0.22
Nodes (17): OrganizationResource, OrganizationRole, createRequirePermissionComponents(), PermissionTarget, RequireOrganizationPermissionProps, RequirePermissionProps, RequirePlatformPermissionProps, createPermissionHooks() (+9 more)

### Community 190 - "compilerOptions"
Cohesion: 0.06
Nodes (34): compilerOptions, allowSyntheticDefaultImports, declaration, emitDecoratorMetadata, experimentalDecorators, forceConsistentCasingInFileNames, incremental, lib (+26 more)

### Community 191 - "index.ts"
Cohesion: 0.13
Nodes (23): AuthInstance, createAuthClientFactory(), CreateAuthClientFactoryOptions, AdminClientPlugin, InviteClientPlugin, OrganizationClientPlugin, useAdminClient(), useInviteClient() (+15 more)

### Community 192 - "TraefikConfigBuilder"
Cohesion: 0.10
Nodes (9): TLSBuilder, TraefikConfigBuilder, CompilationOptions, ConfigStats, TraefikConfig, MiddlewaresConfig, RoutersConfig, ServicesConfig (+1 more)

### Community 193 - "compilerOptions"
Cohesion: 0.06
Nodes (32): compilerOptions, emitDecoratorMetadata, experimentalDecorators, exclude, extends, dist, node_modules, **/*spec.ts (+24 more)

### Community 194 - "DashboardSidebar.tsx"
Cohesion: 0.10
Nodes (15): BreadcrumbConfig, DashboardBreadcrumbs(), formatSegmentLabel(), pathLabels, DashboardLoadingBranded(), DashboardLoadingSkeleton(), DashboardLoadingSpinner(), IMPORTANT: This is rendered in a Suspense fallback, so it MUST NOT: (+7 more)

### Community 195 - "index.ts"
Cohesion: 0.08
Nodes (23): clusterAdmissionRequestStatusEnum, clusterAllocationModeEnum, clusterJoinGrants, clusterJoinGrantsRelations, clusterJoinGrantStatusEnum, clusterNodeMetrics, clusterNodeMetricsRelations, clusterNodes (+15 more)

### Community 196 - "PermissionBuilder"
Cohesion: 0.07
Nodes (5): InlineRoleEntry, PermissionBuilder, ResourceBuilder, RoleBuilder, RoleBuilderWithMeta

### Community 197 - "traefik.module.ts"
Cohesion: 0.13
Nodes (9): ProviderTraefikTemplate, providerTraefikTemplates, ServiceTraefikTemplate, serviceTraefikTemplates, TemplateVariables, TraefikTemplateRepository, Injectable, TraefikTemplateService (+1 more)

### Community 198 - "compilerOptions"
Cohesion: 0.06
Nodes (32): compilerOptions, allowJs, esModuleInterop, forceConsistentCasingInFileNames, incremental, isolatedModules, jsx, lib (+24 more)

### Community 199 - "endpoints.ts"
Cohesion: 0.10
Nodes (25): AdminUsersPage(), adminEndpoints, customContracts, orpcContracts, enhancedAdmin, useAdminActions(), useAdminBanUser(), useAdminCreateUser() (+17 more)

### Community 200 - "Startup v2 Implementation Blueprint — Sub-App Trigger Chain"
Cohesion: 0.06
Nodes (32): Complete File Manifest, Deleted Files, Deletion Checklist, Integration Tests, New Files, Phase 0: Trigger System, Phase 1: Sub-App Runner Infrastructure, Phase 2: Sub-App Orchestrator (+24 more)

### Community 201 - "devDependencies"
Cohesion: 0.06
Nodes (33): knip, @n0safe/rununtil, @n0safe/trycli, nyc, devDependencies, concurrently, cross-env, dotenv-cli (+25 more)

### Community 202 - "index.ts"
Cohesion: 0.02
Nodes (93): projectGetCollaboratorsContract, projectInviteCollaboratorContract, projectRemoveCollaboratorContract, projectCollaboratorInviteOps, projectCollaboratorOps, projectCollaboratorParamsSchema, projectIdParamSchema, projectUpdateCollaboratorContract (+85 more)

### Community 203 - "docker.mock.ts"
Cohesion: 0.08
Nodes (22): MOCK_DOCKER_CONTAINERS, MOCK_DOCKER_DEPLOYMENTS, MOCK_DOCKER_FLEET_SERVERS, MOCK_DOCKER_IMAGES, MOCK_DOCKER_IMAGES_ENTITIES, MOCK_DOCKER_LOG_LINES, MOCK_DOCKER_MESH_EVENT_STREAMS, MOCK_DOCKER_MESH_STATE (+14 more)

### Community 204 - "StatementConfigCollection"
Cohesion: 0.06
Nodes (10): AllActions, FilteredStatement, HasAllOf, HasAnyOf, OmittedStatement, StatementConfigCollection, WithAction, WithAllActions (+2 more)

### Community 205 - "hooks.ts"
Cohesion: 0.12
Nodes (28): buildNuqsParserMap(), buildSchemaDefaults(), createFallbackParser(), createParserForSchema(), emptySchema, extractObjectShape(), extractSchemaDefaultValue(), getInnerSchema() (+20 more)

### Community 206 - "package.json"
Cohesion: 0.06
Nodes (32): dependencies, pino, pino-pretty, @repo/type-guards, description, devDependencies, eslint, @repo/config-eslint (+24 more)

### Community 207 - "ApiKeyRepository"
Cohesion: 0.09
Nodes (14): apiKeys, ProjectModule, Module, ApiKeyCreateInput, ApiKeyRepository, ApiKeyRow, Injectable, ApiKeyService (+6 more)

### Community 208 - "project.module.ts"
Cohesion: 0.11
Nodes (12): webhooks, Injectable, WebhookCreateInput, WebhookRepository, WebhookRow, WebhookUpdateInput, CreateWebhookInput, RotateWebhookSecretResult (+4 more)

### Community 209 - "ProjectDomainRepository"
Cohesion: 0.08
Nodes (25): AddDomainResponse, AddOrganizationDomain, AddProjectDomain, AddServiceDomain, AddServiceDomainResponse, AvailableDomain, CheckSubdomainAvailability, OrganizationDomain (+17 more)

### Community 210 - "scripts"
Cohesion: 0.06
Nodes (32): scripts, analyze, build, build:no:check, build:static-only, clean, compile, dev (+24 more)

### Community 211 - "schemas.ts"
Cohesion: 0.11
Nodes (20): addProjectDomainContract, addProjectDomainOutput, getAvailableDomainsForServiceContract, getAvailableDomainsForServiceInput, getAvailableDomainsInput, listProjectDomainsContract, listProjectDomainsInput, removeProjectDomainContract (+12 more)

### Community 212 - "platform-domain.builder.ts"
Cohesion: 0.09
Nodes (27): PlatformDomainSchemaDeps, DependencyAttachmentMode, dependencyAttachmentModeSchema, DependencyEnvironmentDiffStatus, dependencyEnvironmentDiffStatusSchema, DependencyFailureMode, dependencyFailureModeSchema, DependencyHealthGate (+19 more)

### Community 213 - "types.ts"
Cohesion: 0.11
Nodes (26): asWrapper(), compileDFilter(), compileOperator(), resolveVar(), ProjectSchema, sqlAnd(), sqlOr(), matchFilter() (+18 more)

### Community 214 - ".nowMs"
Cohesion: 0.12
Nodes (3): HybridLogicalClock, PhiAccrualDetector, TokenBucket

### Community 215 - "index.ts"
Cohesion: 0.10
Nodes (20): main(), MeshOpenInternalBridgeInput, collectStreamEvents(), runLocalSetup(), SetupResult, SetupStreamEvent, SetupWorkflowContext, collectStreamEvents() (+12 more)

### Community 216 - "DeploymentQueueProcessor"
Cohesion: 0.09
Nodes (13): RuntimeRunnerOptions, deploymentSourceCheckoutContextSchema, DeploymentBullJobData, DeploymentBullJobPayload, DeploymentExecutionPlan, DeploymentQueueProcessor, executionPlanSchema, PhaseRetryPolicy (+5 more)

### Community 217 - "client.tsx"
Cohesion: 0.08
Nodes (30): CapabilityStatus, platformCapabilities, PlatformCapability, formatDate(), getInitials(), ProfileForm(), ProfileFormProps, addTimingEntry() (+22 more)

### Community 219 - "schemas.ts"
Cohesion: 0.12
Nodes (23): STALE_TIME, CreateOrganizationInput, createOrganizationSchema, DeleteOrganizationInput, deleteOrganizationSchema, GetOrganizationInput, getOrganizationSchema, InviteMemberInput (+15 more)

### Community 220 - "makeRoute.tsx"
Cohesion: 0.08
Nodes (44): usePush(), createPathBuilder(), createRouteBuilder(), makeDeleteRoute(), makeGetRoute(), makeLayoutRoute(), makePostRoute(), makePutRoute() (+36 more)

### Community 221 - "peerDependencies"
Cohesion: 0.14
Nodes (14): peerDependencies, class-variance-authority, clsx, cmdk, framer-motion, @hookform/resolvers, lucide-react, next-themes (+6 more)

### Community 222 - "devDependencies"
Cohesion: 0.06
Nodes (30): devDependencies, @repo/config-prettier, @repo/config-typescript, @repo/config-vitest, tailwind-variants, tailwindcss, tailwindcss-animate, @types/node (+22 more)

### Community 223 - "index.ts"
Cohesion: 0.08
Nodes (24): getPublicKeyContract, getPublicKeyOutputSchema, pushPublicKeyOps, deviceSchema, getStatsContract, getStatsOutputSchema, pushStatsOps, getSubscriptionsContract (+16 more)

### Community 224 - "service.schema.ts"
Cohesion: 0.24
Nodes (17): artifactBundleProviderConfigSchema, bitbucketProviderConfigSchema, containerRegistryProviderConfigSchema, githubProviderConfigSchema, gitlabProviderConfigSchema, manualProviderConfigSchema, providerConfigSchemaById, serviceProviderConfigUnionSchema (+9 more)

### Community 225 - "utils.ts"
Cohesion: 0.06
Nodes (29): days, FromPascalToSnake, IntersectArray, IsUnion, KeysFromCamelToPascal, KeysFromCamelToSnake, KeysFromPascalToCamel, KeysFromPascalToSnake (+21 more)

### Community 228 - "server.ts"
Cohesion: 0.11
Nodes (24): SessionHydrationProvider(), SessionHydrationProviderProps, createPrefetchSession(), createSessionAwareAuthClient(), createSessionQueryOptions(), createUseSession(), IMPORTANT: null is a valid value meaning "user not logged in", SessionQueryResult (+16 more)

### Community 229 - "index.ts"
Cohesion: 0.11
Nodes (14): DefaultErrorFallback(), DefaultErrorFallbackProps, ErrorBoundary, ErrorBoundaryProps, ErrorBoundaryState, FeatureErrorBoundary(), FeatureErrorBoundaryProps, FeatureErrorFallback() (+6 more)

### Community 230 - "router-hooks.types.ts"
Cohesion: 0.11
Nodes (30): ExtractMutationContext, CustomInvalidationContext, CustomInvalidationPart, ExtractCustomHookInput, ExtractCustomHookOutput, ExtractCustomHooksKeys, ExtractHookReturnType, ExtractMutationData (+22 more)

### Community 231 - "cluster.ts"
Cohesion: 0.14
Nodes (15): addServiceDomainResponseSchema, addServiceDomainSchema, checkSubdomainAvailabilitySchema, serviceDomainWithFullUrlSchema, subdomainAvailabilityResponseSchema, updateServiceDomainSchema, listServiceDomainsInput, removeServiceDomainContract (+7 more)

### Community 232 - "mesh-type-utils.ts"
Cohesion: 0.08
Nodes (20): AggregateResult, Awaited, BuilderItem, BuilderResult, DeepReadonly, EntityItem, EntityItemKey, EntityMutationNames (+12 more)

### Community 233 - "mesh-filter.ts"
Cohesion: 0.09
Nodes (40): always, exists(), gt(), gte(), lt(), lte(), matches(), missing() (+32 more)

### Community 234 - "error"
Cohesion: 0.08
Nodes (24): environmentAccessLogs, environmentPromotions, environments, environmentServices, environmentStatusEnum, environmentTemplates, environmentTypeEnum, environmentVariables (+16 more)

### Community 235 - "index.ts"
Cohesion: 0.04
Nodes (22): VariableContext, VariableRegistrationError, VariableRegistry, VariableResolutionError, VariableResolver, NumberVariable, StringVariable, Variable (+14 more)

### Community 236 - "ProviderSchemaService"
Cohesion: 0.12
Nodes (7): ProviderSchemaController, Controller, Implement, ProviderSchemaModule, Module, ProviderSchemaService, Injectable

### Community 237 - "compilerOptions"
Cohesion: 0.07
Nodes (28): compilerOptions, emitDecoratorMetadata, experimentalDecorators, outDir, paths, rootDir, types, exclude (+20 more)

### Community 238 - "domain.module.ts"
Cohesion: 0.11
Nodes (21): ProxyBuilderBase, DetailedOutputBrand, ExtractOutputBody, ExtractOutputHeaders, ExtractOutputStatus, ObservableContractSchema, OutputSchemaProxySchema, BatchOptions (+13 more)

### Community 239 - "utils.test.ts"
Cohesion: 0.07
Nodes (29): ArrayContains, CamelToPascal, FromCamelToSnake, ObjectFromCamelToSnake, ObjectFromPascalToCamel, ObjectFromPascalToSnake, ObjectFromSnakeToPascal, OmitNever (+21 more)

### Community 240 - "platform-domain.schema.ts"
Cohesion: 0.07
Nodes (29): CacheStrategy, cacheStrategySchema, DeploymentEnvironment, DeploymentPhase, deploymentPhaseSchema, DeploymentStatus, deploymentStatusSchema, EnvironmentStatus (+21 more)

### Community 241 - "Phase 1: Dead Code Removal 🧹"
Cohesion: 0.07
Nodes (28): 1.1.1 `core/modules/context/` — 🟢 SAFE DELETE, 1.1.2 `core/modules/ephemeral-http/` — 🟢 SAFE DELETE, 1.1.3 `core/modules/sub-app-orchestrator/` — 🟢 SAFE DELETE, 1.1.4 `core/modules/loader/` — 🟢 SAFE DELETE, 1.1 Dead Core Modules (Round 1 — Safest), 1.2.1 `system/modules/fleet/` — 🟢 SAFE DELETE, 1.2.2 `system/modules/mesh/` — ⚠️ PARTIAL — Check First, 1.2.3 `system/system.module.ts` — 🟢 SAFE DELETE (after loader) (+20 more)

### Community 242 - "🎯 AI Coding Agent Instructions"
Cohesion: 0.07
Nodes (27): 🎯 AI Coding Agent Instructions, Anti-Patterns, Anti-Patterns, Architectural Layers at a Glance, 🔴 Bun Runtime Rule (Mandatory), 🔴 Change Order & Validation, 🔴 Code Smell Catalog, Common Smells and Fixes (+19 more)

### Community 243 - "docker-registry-detail-modal.tsx"
Cohesion: 0.09
Nodes (11): PlatformActionsForResource, ActionsForResource, InvalidActionError, InvalidResourceError, InvalidRoleError, PermissionRequirement, ResourceActionsAccessor, ResourceName (+3 more)

### Community 244 - "dependencies"
Cohesion: 0.07
Nodes (29): @antfu/ni, boxen, diff, execa, fs-extra, handlebars, kleur, log-symbols (+21 more)

### Community 247 - "drizzle-filter.utils.ts"
Cohesion: 0.10
Nodes (10): CommonOperators, createCommonOperators(), DynamicCapableQuery, FilterResolvers, FilterWithLogical, FromParam, PaginationInput, PaginationMeta (+2 more)

### Community 248 - "compilerOptions"
Cohesion: 0.07
Nodes (28): compilerOptions, allowJs, allowSyntheticDefaultImports, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, lib, module (+20 more)

### Community 249 - "compilerOptions"
Cohesion: 0.07
Nodes (28): compilerOptions, allowSyntheticDefaultImports, declaration, declarationMap, emitDecoratorMetadata, experimentalDecorators, forceConsistentCasingInFileNames, isolatedModules (+20 more)

### Community 250 - "compilerOptions"
Cohesion: 0.07
Nodes (28): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, lib (+20 more)

### Community 251 - "compilerOptions"
Cohesion: 0.07
Nodes (28): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, module, moduleResolution (+20 more)

### Community 252 - "Current Architecture & Next Steps for Sub-App Orchestration"
Cohesion: 0.08
Nodes (25): 1. Project Overview, 2.1 `main.ts` (needs simplification), 2.2 Files Already Built (to keep / adapt), 2.3 Existing Sub-App Modules, 2. Current State (What Exists), 3.1 Database Setup Flow, 3.2 Core Bootstrap Flow, 3.3 The "Setup" Problem (+17 more)

### Community 253 - "platform-capabilities-graphs.mdx"
Cohesion: 0.08
Nodes (24): 1) Capability map (what the application should be capable of), 2) How it should work (end-to-end runtime flow), 3) Domain and module hierarchy, 4) Business hierarchy (who owns what), 5) Environment variables model, 6) Dependency graph (technical stack), 7.1 Role hierarchy, 7.2 First-time setup flow (+16 more)

### Community 254 - "app-error.ts"
Cohesion: 0.13
Nodes (10): AppError, BadRequestError, ConflictError, ForbiddenError, NotFoundError, ServiceUnavailableError, TimeoutError, UnauthorizedError (+2 more)

### Community 255 - "Phase 2: Mock→Real Data Migration 🔗"
Cohesion: 0.08
Nodes (24): 2.1 Projects List Page, 2.2 Project Detail Page, 2.3 Project Configuration Pages, 2.5 Deployments List Page, 2.6 Services List Page, 2.7 Cleanup After Migration, 2.8 Testing Strategy, Architecture (+16 more)

### Community 256 - "routing-resources.schema.ts"
Cohesion: 0.04
Nodes (50): cancelEnvelopeSchema, correlationInputSchema, MeshQueueTransitionService, queueTransitionMeshContracts, requestEnvelopeSchema, requestPayloadSchema, responseEnvelopeSchema, Injectable (+42 more)

### Community 257 - "index.ts"
Cohesion: 0.07
Nodes (36): dockerEntityContract, dockerEntityInspectContract, dockerEntityInspectOps, DockerEntityInspectResponse, dockerEntityInspectResponseSchema, dockerEntityListContract, dockerEntityListOps, DockerEntityListResponse (+28 more)

### Community 258 - "hooks.ts"
Cohesion: 0.08
Nodes (24): invitationEndpoints, STALE_TIME, enhancedInvitation, invitationInvalidations, AcceptInvitationInput, acceptInvitationSchema, CancelInvitationInput, cancelInvitationSchema (+16 more)

### Community 259 - "utils"
Cohesion: 0.07
Nodes (28): utils, @antfu/ni, @asteasolutions/zod-to-openapi, circular-json, commander, diff, execa, fs-extra (+20 more)

### Community 260 - "compilerOptions"
Cohesion: 0.07
Nodes (27): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, module, moduleResolution (+19 more)

### Community 261 - "compilerOptions"
Cohesion: 0.07
Nodes (27): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, module, moduleResolution (+19 more)

### Community 262 - "compilerOptions"
Cohesion: 0.07
Nodes (27): compilerOptions, allowJs, composite, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, incremental (+19 more)

### Community 263 - "package.json"
Cohesion: 0.07
Nodes (27): dependencies, @repo/type-guards, zod, devDependencies, @repo/config-typescript, @typescript/native-preview, vitest, @vitest/coverage-v8 (+19 more)

### Community 264 - "entrypoint.prod.ts"
Cohesion: 0.53
Nodes (5): buildWeb(), main(), ProcessHandle, startApi(), startWeb()

### Community 265 - "ServiceContextBuilder"
Cohesion: 0.08
Nodes (10): ContextUtils, ProjectContext, ProjectContextBuilder, ServiceContext, ServiceDomainMapping, TRAEFIK_VARIABLES, TraefikVariableResolverService, Injectable (+2 more)

### Community 266 - "SystemMeshConfigService"
Cohesion: 0.08
Nodes (22): MeshInternalRequestService, Injectable, MeshStreamRuntimeService, { createORPCClientMock, openAPILinkConstructorMock }, streamFields, Injectable, path(), meshDeploymentResourceSchema (+14 more)

### Community 267 - "devDependencies"
Cohesion: 0.07
Nodes (27): devDependencies, eslint, @eslint/eslintrc, @eslint/js, eslint-plugin-prettier, globals, @nestjs/cli, @nestjs/schematics (+19 more)

### Community 268 - "compilerOptions"
Cohesion: 0.07
Nodes (26): compilerOptions, composite, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, inlineSources, isolatedModules (+18 more)

### Community 269 - "migration.schema.ts"
Cohesion: 0.16
Nodes (12): DockerRegistriesModule, Module, DockerRegistriesOrchestratorService, Injectable, DockerRegistriesDomainService, Injectable, dockerRegistriesContract, dockerListRegistriesContract (+4 more)

### Community 270 - "calendar-date-picker.tsx"
Cohesion: 0.09
Nodes (26): CalendarDatePickerProps, months, multiSelectVariants, ExpandIcon(), ExpandIconProps, DataTablePagination(), DataTablePaginationProps, getButtonSizeClass() (+18 more)

### Community 271 - "Startup v2 — Complete NestJS DI Orchestration Graph"
Cohesion: 0.08
Nodes (23): 10. Diagram 9 — The Full Orchestration Graph (Unified), 11. Diagram 10 — Clean Teardown Sequence, 12. Reference: Module Catalog, 1. How NestJS's DI Graph Becomes the Orchestrator, 2. Diagram 1 — Full Module Import Tree (All 30+ Modules), 3. Diagram 2 — NestJS Bootstrap Sequence (3 Phases), 4. Diagram 3 — Provider Barrier Mechanism (DI as DAG), 5. Diagram 4 — Sub-App Trigger Chain (Per-Module Isolation) (+15 more)

### Community 272 - "vitest.shared-postgres.e2e.ts"
Cohesion: 0.16
Nodes (21): dockerode, stopAllSharedApiRuntimes(), globalSetup(), deleteSharedPostgresUriFile(), describeActiveHandles(), describeActiveRequests(), isTruthyEnv(), logSharedPostgres() (+13 more)

### Community 273 - "startup-architecture-analysis.mdx"
Cohesion: 0.08
Nodes (23): Critical Gaps, Dev Entrypoint — Correct Design, Entrypoints, Fix Priority, Full 10-Phase Startup Protocol (Inside StartupOrchestrator), How Lazy Loading Works Practically, Module Lazy Loading Strategy, Module Loading (Current State) (+15 more)

### Community 274 - "mesh-subscription-manager.ts"
Cohesion: 0.13
Nodes (12): MeshSubscriptionManager, SubscriptionRecord, Injectable, GlobalResourceRegistryEntry, MeshResourceChangeNotifier, MeshSubscriptionEvent, MeshSubscriptionEventType, MeshSubscriptionFilter (+4 more)

### Community 275 - "LoadBalancerController"
Cohesion: 0.15
Nodes (8): LoadBalancerController, All, Controller, Get, Param, Req, Res, Query

### Community 276 - "3.1 Fix `as unknown as` Violations (80+)"
Cohesion: 0.08
Nodes (23): 3.1.1 Web: `domains/shared/helpers.ts` (10+ violations), 3.1.2 Web: `domains/docker/use-docker-live.ts` (6 violations), 3.1.3 API: `docker/repositories/facade/docker.repository.ts` (5 violations), 3.1.4 API: `docker/entity/docker-entity-domain.service.ts` (4 violations), 3.1.5 API: `core/modules/events/` (6 files), 3.1.6 Remaining Files (~50 violations), 3.1 Fix `as unknown as` Violations (80+), 3.2.1 UI: `data-table.tsx` (6 violations) (+15 more)

### Community 277 - "index.ts"
Cohesion: 0.10
Nodes (3): HealthCheckBuilder, LoadBalancerBuilder, ServiceBuilder

### Community 278 - "toolbar.tsx"
Cohesion: 0.07
Nodes (21): createConditionalStateHook(), SetStateWithPromise, Comparable, ComparisonCache, isDeepEqual(), memoize(), TypedArray, canUseDOM() (+13 more)

### Community 281 - "domain.types.ts"
Cohesion: 0.08
Nodes (25): platformDomainSchemas, DependencyGraphMockScenario, DependencyPolicyAdvanced, EnvPolicy, EnvPolicyMap, FixtureDependency, FixtureGroupMemberDependency, FixtureGroupMemberService (+17 more)

### Community 282 - "package.json"
Cohesion: 0.08
Nodes (25): description, devDependencies, eslint, @repo/config-eslint, @repo/config-prettier, @repo/config-typescript, @types/node, @typescript/native-preview (+17 more)

### Community 284 - ".handleEvent"
Cohesion: 0.17
Nodes (7): MeshStreamSessionService, Injectable, MeshDuplexStreamInput, MeshDuplexStreamOutput, MeshMembershipSnapshot, MeshPeerDisconnectInput, MeshPeerDisconnectResult

### Community 285 - "dependencies"
Cohesion: 0.08
Nodes (25): dependencies, @nestjs/common, @nestjs/config, @nestjs/core, @nestjs/platform-express, @orpc/client, reflect-metadata, @repo/api-contracts (+17 more)

### Community 286 - "app.module.ts"
Cohesion: 0.13
Nodes (14): AppController, Controller, Get, AppModule, NeedSetupModule, NeedSetupService, SetupModule, SetupService (+6 more)

### Community 287 - "page.tsx"
Cohesion: 0.15
Nodes (16): ContainerProjection, DashboardDockerPage(), DOCKER_LIST_INPUT, fallbackImageRefFromId(), formatDate(), SERVICE_LIST_INPUT, shortId(), toBadgeVariant() (+8 more)

### Community 288 - "api:nest"
Cohesion: 0.08
Nodes (25): express, express-list-routes, nest-commander, @nestjs/cli, @nestjs/common, @nestjs/config, @nestjs/core, nestjs-flub (+17 more)

### Community 289 - "package.json"
Cohesion: 0.08
Nodes (24): dependencies, @repo/type-guards, zod, devDependencies, @repo/config-typescript, @typescript/native-preview, vitest, @vitest/coverage-v8 (+16 more)

### Community 291 - "ConfigurationResolverService"
Cohesion: 0.22
Nodes (6): ResolvedRuntimeConfiguration, RuntimeConfigurationContext, RuntimeConfigurationDispatchRule, RuntimeConfigurationResolverInputByScope, ConfigurationResolverService, Injectable

### Community 292 - "mesh-doc-compliance-matrix.e2e-spec.ts"
Cohesion: 0.08
Nodes (17): defineResource(), MeshPartitionPolicy, PartitionPolicyInput, PartitionPolicyResult, NodeDescriptor, OwnershipChange, OwnershipResolverService, Injectable (+9 more)

### Community 293 - "list.ts"
Cohesion: 0.11
Nodes (13): AdminPlugin, ConfiguredRoleNames, InvitePlugin, OrganizationPlugin, useInvite(), ERROR_CODES, Invite, invitePlugin() (+5 more)

### Community 294 - "hooks.ts"
Cohesion: 0.18
Nodes (16): ProfilePage(), getDeviceName(), PushNotificationSettings(), urlBase64ToUint8Array(), pushEndpoints, enhancedPush, usePushActions(), usePushNotifications() (+8 more)

### Community 295 - "progressive-module-initialization.mdx"
Cohesion: 0.09
Nodes (22): Between Tier 0 and Tier 1 — The Pool Factory in Detail, Dev Mode Differences, `getStateMachine` Endpoint, How DB URL Can Change Mid-Run, `initialize` Endpoint, Module Readiness Truth Table, Overview, Related Documentation (+14 more)

### Community 296 - "package.json"
Cohesion: 0.08
Nodes (23): devDependencies, @repo/config-prettier, @repo/config-vitest, vitest, exports, ./config/*, @repo/config-prettier, @repo/config-vitest (+15 more)

### Community 297 - "shared.ts"
Cohesion: 0.14
Nodes (15): projectGetEnvironmentStatusContract, projectGetAllEnvironmentStatusesContract, projectRefreshEnvironmentStatusContract, availableVariableEntrySchema, environmentStatusOutputSchema, projectAvailableVariableOps, projectEnvironmentParamsSchema, projectEnvironmentStatusOps (+7 more)

### Community 298 - "utils.ts"
Cohesion: 0.13
Nodes (8): createPermission(), hasAllPermissions(), hasAnyPermission(), hasPermission(), hasResourcePermission(), PermissionChecker, Resources, Roles

### Community 301 - "devDependencies"
Cohesion: 0.09
Nodes (23): devDependencies, cross-env, @nestjs/schematics, @nestjs/testing, @repo/config-eslint, @repo/config-prettier, @repo/config-vitest, rimraf (+15 more)

### Community 302 - "load-balancer.service.ts"
Cohesion: 0.12
Nodes (13): IngestLoadReportResult, LoadReportConvergenceNode, LoadReportConvergenceSnapshot, MeshTrustKeyringSecretsSnapshot, NodeSnapshot, LoadBalancerReconnectSignal, LoadBalancerReconnectSignalReason, MeshLoadReport (+5 more)

### Community 303 - "LoadBalancerService"
Cohesion: 0.13
Nodes (3): LoadBalancerService, Injectable, RouteHintPayload

### Community 304 - "compilerOptions"
Cohesion: 0.09
Nodes (22): compilerOptions, allowSyntheticDefaultImports, baseUrl, declaration, emitDecoratorMetadata, esModuleInterop, experimentalDecorators, forceConsistentCasingInFileNames (+14 more)

### Community 305 - "components.json"
Cohesion: 0.09
Nodes (22): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+14 more)

### Community 306 - "page.tsx"
Cohesion: 0.07
Nodes (42): ACTIVITY_STREAM_INPUT, ActivityCategory, ActivitySeverity, ActivityStatus, DashboardDockerActivityPage(), formatDate(), resolveContainerId(), resolveResourceName() (+34 more)

### Community 307 - "index.ts"
Cohesion: 0.11
Nodes (20): cloneValue(), DashboardServiceConfigurationPage(), parseCommaSeparatedValues(), RUNNER_NETWORK_MODES, SERVICE_HEALTH_PROTOCOLS, SERVICE_HEALTH_VALUES, SERVICE_LIFECYCLE_VALUES, SERVICE_RUNNER_STRATEGIES (+12 more)

### Community 308 - "cache-operations.ts"
Cohesion: 0.05
Nodes (27): GET(), addCacheOperations(), AddCacheToRouter, CacheOperations, CacheOperationsWithInput, CacheOperationsWithoutInput, createCacheOperations(), EndpointWithCache (+19 more)

### Community 309 - "📋 Exhaustive Feature Catalog"
Cohesion: 0.09
Nodes (23): 10. 🔔 Push Notifications, 11. 🩺 Health & Monitoring, 12. 🚢 Fleet Management, 13. 🔗 Mesh (Distributed Coordination), 14. 🧪 Test Utilities, 15. 🎛️ Setup & Initialization, 16. 🧩 Provider Schema, 17. 🔄 GitHub Integration (+15 more)

### Community 310 - "devDependencies"
Cohesion: 0.09
Nodes (23): babel-plugin-macros, @eslint/compat, eslint-config-nestjs, eslint-config-turbo, eslint-plugin-next, devDependencies, babel-plugin-macros, eslint (+15 more)

### Community 311 - "auth-core.service.ts"
Cohesion: 0.05
Nodes (38): createOrpcMiddlewareProxy(), AdminPluginWrapper, AppMiddlewares, createAdminWrapper(), createOrganizationWrapper(), createPluginRegistry(), InferPluginsFromRegistry, OrganizationBuilder (+30 more)

### Community 312 - "GitHubService"
Cohesion: 0.05
Nodes (15): GitHubModule, Module, GitHubAppConfig, GitHubCommitAuthor, GitHubCommitInfo, GitHubInstallation, GitHubInstallationAccount, GitHubRepository (+7 more)

### Community 313 - "5.1 Address 14 TODO/FIXME Items"
Cohesion: 0.09
Nodes (22): 5.1.1 Install `@nestjs/schedule` for Domain Auto-Verification, 5.1.2 Create Variable Resolver Integration, 5.1.3 Address GitHub Webhook TODO, 5.1.4 Fix ORPC Middleware DI, 5.1.5 Implement FK-Chain Subquery, 5.1 Address 14 TODO/FIXME Items, 5.2 Add Frontend Tests, 5.3 Add Knip to CI Pipeline (+14 more)

### Community 314 - "fleet-latency-map.tsx"
Cohesion: 0.18
Nodes (15): buildCurvedPositions(), FleetLatencyMap(), FleetLatencyMapProps, FleetMapLink, FleetMapNode, latencyColor(), LINK_PALETTE, linkColor() (+7 more)

### Community 315 - "package.json"
Cohesion: 0.09
Nodes (21): dependencies, @repo/type-guards, zod, devDependencies, @repo/config-typescript, @types/node, @typescript/native-preview, exports (+13 more)

### Community 316 - "state.schema.ts"
Cohesion: 0.10
Nodes (20): SetupConfigureDatabaseInput, setupConfigureDatabaseInputSchema, SetupConfigureDatabaseResult, setupConfigureDatabaseResultSchema, SetupBootstrapStrategy, setupBootstrapStrategySchema, SetupState, SetupStateMachine (+12 more)

### Community 317 - "configuration-resolver.service.ts"
Cohesion: 0.11
Nodes (13): resolvedRuntimeConfigurationSchema, RuntimeConfigurationResolverInput, runtimeConfigurationResolverInputSchema, RuntimeConfigurationResolverScopedInput, RuntimeEnvironmentMutationInput, ConfigurationDefinitionService, Injectable, ALL_PROVIDER_TYPES (+5 more)

### Community 319 - "retry.schema.ts"
Cohesion: 0.13
Nodes (14): Props, StepIndicator(), ModeCard(), Props, Button(), buttonVariants, InputGroupAddon(), inputGroupAddonVariants (+6 more)

### Community 321 - "dependencies"
Cohesion: 0.10
Nodes (21): dependencies, fumadocs-core, fumadocs-mdx, fumadocs-ui, lucide-react, mermaid, next, react (+13 more)

### Community 322 - "devDependencies"
Cohesion: 0.10
Nodes (21): devDependencies, eslint-config-next, @eslint/eslintrc, postcss, @repo/config-eslint, @repo/config-prettier, tailwindcss, @types/mdx (+13 more)

### Community 323 - "next.config.ts"
Cohesion: 0.16
Nodes (9): apiUrl, commandLine, getWorkspaceTranspilePackages(), nextConfig, PackageJsonShape, NOTE: do NOT use `import.meta.url` here — Bun 1.3.14 has a transpiler bug, workspaceTranspilePackages, revalidateAllAction() (+1 more)

### Community 324 - "devDependencies"
Cohesion: 0.10
Nodes (21): devDependencies, @repo/config-prettier, @repo/config-vitest, type-fest, @types/diff, @types/fs-extra, @types/prompts, @typescript/native-preview (+13 more)

### Community 325 - "compilerOptions"
Cohesion: 0.10
Nodes (20): compilerOptions, composite, lib, module, moduleResolution, outDir, rootDir, target (+12 more)

### Community 326 - "dropdown-menu.tsx"
Cohesion: 0.14
Nodes (13): DataTableColumnHeader(), DataTableColumnHeaderProps, DropdownMenu(), DropdownMenuCheckboxItem(), DropdownMenuContent(), DropdownMenuItem(), DropdownMenuLabel(), DropdownMenuRadioItem() (+5 more)

### Community 327 - "devDependencies"
Cohesion: 0.10
Nodes (21): devDependencies, eslint, @repo/config-eslint, @repo/config-prettier, @repo/config-typescript, server-only, @types/node, @types/react (+13 more)

### Community 330 - "devDependencies"
Cohesion: 0.10
Nodes (21): devDependencies, eslint, @repo/config-eslint, @repo/config-prettier, @repo/config-typescript, @repo/use-safe-query-param-states-from-zod, server-only, @types/node (+13 more)

### Community 331 - "merge-hooks.ts"
Cohesion: 0.18
Nodes (17): ConflictWarningScope, getDuplicates(), warnOnConflicts(), mergeHooks(), NOTE: The cast through unknown is necessary because:, extractCustomQueryKeys(), extractRouterQueryKeys(), mergeQueryKeys() (+9 more)

### Community 332 - "compilerOptions"
Cohesion: 0.10
Nodes (20): compilerOptions, declaration, declarationMap, esModuleInterop, jsx, lib, moduleResolution, outDir (+12 more)

### Community 334 - "update-agent-context.sh"
Cohesion: 0.23
Nodes (14): create_new_agent_file(), log_error(), log_info(), log_success(), log_warning(), main(), parse_plan_data(), print_summary() (+6 more)

### Community 335 - "nextjs.json"
Cohesion: 0.10
Nodes (19): compilerOptions, jsx, lib, plugins, display, exclude, extends, include (+11 more)

### Community 336 - "diagnose-build.ts"
Cohesion: 0.20
Nodes (18): checkBunLock(), checkDependencyResolution(), checkDockerEnvironment(), checkEnv(), checkNodeModules(), checkPath(), checkSourceFiles(), checkTurboConfig() (+10 more)

### Community 337 - "config.ts"
Cohesion: 0.11
Nodes (18): OrganizationActionsForResource, ProjectActionsForResource, ProjectResource, ORGANIZATION_RESOURCES, ORGANIZATION_ROLES, organizationBuilder, organizationPermissionConfig, organizationRoleMetaShape (+10 more)

### Community 338 - "rule-validator.ts"
Cohesion: 0.12
Nodes (16): actionsSchema, dFilterOperatorSchema, dFilterSchema, parseResourceRules(), projectResourceSchema, resourceOrWildcardSchema, resourceRuleSchema, ResourceRulesSafeParseResult (+8 more)

### Community 339 - "server.tsx"
Cohesion: 0.12
Nodes (12): nest-commander, DEPLOYER_VERSION, __dirname, __filename, PKG_PATH, StartupCheckOutput, SetupDbCommand, SetupDbOptions (+4 more)

### Community 340 - "nest-cli.json"
Cohesion: 0.11
Nodes (18): compilerOptions, entryFile, root, sourceRoot, type, entryFile, sourceRoot, type (+10 more)

### Community 341 - "AbstractDockerContainerService"
Cohesion: 0.25
Nodes (5): ApiTags, assertAuthenticated(), TestController, Controller, Implement

### Community 343 - "catalogs"
Cohesion: 0.11
Nodes (19): better-auth, @thallesp/nestjs-better-auth, auth, react-compiler, styling, web, babel-plugin-react-compiler, clsx (+11 more)

### Community 344 - "package.json"
Cohesion: 0.11
Nodes (18): author, email, name, bin, declarative-routing, description, exports, files (+10 more)

### Community 345 - "devDependencies"
Cohesion: 0.11
Nodes (19): devDependencies, drizzle-orm, @eslint/js, @repo/config-eslint, @repo/config-prettier, @repo/config-vitest, @types/node, @typescript/native-preview (+11 more)

### Community 346 - "compilerOptions"
Cohesion: 0.11
Nodes (18): compilerOptions, esModuleInterop, jsx, lib, moduleResolution, noEmit, skipLibCheck, target (+10 more)

### Community 347 - "dependencies"
Cohesion: 0.11
Nodes (19): dependencies, @orpc/client, @orpc/contract, @orpc/shared, @orpc/tanstack-query, @repo/type-guards, rxjs, @standard-schema/spec (+11 more)

### Community 348 - "devDependencies"
Cohesion: 0.11
Nodes (19): devDependencies, eslint, react, @repo/config-eslint, @repo/config-prettier, @repo/config-typescript, @types/react, @typescript/native-preview (+11 more)

### Community 349 - "devDependencies"
Cohesion: 0.11
Nodes (19): devDependencies, eslint, @repo/config-eslint, @repo/config-prettier, @repo/config-typescript, @types/node, @types/react, @typescript/native-preview (+11 more)

### Community 350 - "template.schema.exhaustive.test.ts"
Cohesion: 0.18
Nodes (14): collectStreamEvents(), runLocalSetup(), SetupResult, SetupStreamEvent, assertNodeStatusNotPersisted(), assertStateMachineWorkflowShape(), parseNodeConfigStatus(), parseSetupSnapshot() (+6 more)

### Community 351 - "DockerImageAutoScanListenerService"
Cohesion: 0.10
Nodes (7): DockerRuntimeEventsStreamService, Injectable, DockerRuntimeMeshRelayService, Injectable, AUTO_SCAN_EVENT_ACTIONS, DockerImageAutoScanListenerService, Injectable

### Community 352 - "mesh-test.module.ts"
Cohesion: 0.16
Nodes (11): bootstrap(), AppModule, ServiceA1, ServiceA1Module, ServiceB1, ServiceB1Module, ServiceB2, ServiceB2Module (+3 more)

### Community 353 - "hooks.ts"
Cohesion: 0.12
Nodes (17): ConnectionState, ConnectionStatus(), Props, LocalDatabaseStep(), Props, AuthOutcome, Props, RemoteAuthStep() (+9 more)

### Community 354 - "package.json"
Cohesion: 0.11
Nodes (17): detect-libc, dependencies, detect-libc, systeminformation, engines, bun, packages/**, @typescript/native-preview (+9 more)

### Community 355 - "exports"
Cohesion: 0.11
Nodes (18): exports, ./library, ./nestjs, ./nextjs, ./react, ./test, import, import (+10 more)

### Community 356 - "decorators.ts"
Cohesion: 0.10
Nodes (19): analyticsApplicationMetricsOps, analyticsApplicationMetricsOutputSchema, analyticsDatabaseMetricsOps, analyticsDatabaseMetricsOutputSchema, analyticsDeploymentMetricsOps, analyticsDeploymentMetricsOutputSchema, analyticsGetApplicationMetricsContract, analyticsGetDatabaseMetricsContract (+11 more)

### Community 357 - "MiddlewareCheck"
Cohesion: 0.10
Nodes (19): analyticsActivitySummaryOps, analyticsActivitySummaryOutputSchema, analyticsActivitySummaryQuerySchema, analyticsApiUsageOps, analyticsApiUsageOutputSchema, analyticsApiUsageQuerySchema, analyticsDeploymentUsageOps, analyticsDeploymentUsageOutputSchema (+11 more)

### Community 358 - "add.ts"
Cohesion: 0.18
Nodes (11): addOrganizationDomainContract, deleteOrganizationDomainInput, deleteOrganizationDomainOutput, getOrganizationDomainContract, getOrganizationDomainInput, organizationDomainOps, verifyOrganizationDomainContract, verifyOrganizationDomainInput (+3 more)

### Community 360 - "entity.ts"
Cohesion: 0.13
Nodes (14): serviceAddDependencyContract, serviceDependencyAddBodySchema, serviceDependencyAddOps, serviceDependencyAddOutputSchema, serviceDependencyAddParamsSchema, serviceDependenciesContract, serviceDependenciesListOps, serviceDependenciesListOutputSchema (+6 more)

### Community 361 - "Migration Plan — Complete Application Perfection"
Cohesion: 0.09
Nodes (22): Architecture Reference, 🔐 Auth — 3 Subagents, 12 Action Items, 📦 Deployment — 3 Subagents, 25 Action Items, Detailed Phase Breakdown, 🐳 Docker — 3 Subagents, 14 Action Items, Migration Plan — Complete Application Perfection, Per-Feature Summary (from Subagent Reports), Phase 1: 🧹 Dead Code Removal (135+ files, ~8h) (+14 more)

### Community 362 - "permission.repository.ts"
Cohesion: 0.22
Nodes (4): orgRoleRules, PermissionRepository, Injectable, ResourceRule

### Community 363 - "LocalInitializationService"
Cohesion: 0.07
Nodes (13): pg, log, PostgresContainerService, PostgresContainerStartOptions, LocalInitializationService, SetupWizardApiModule, SetupWizardController, SetupWizardInitModule (+5 more)

### Community 364 - "InternalBaseMeshService"
Cohesion: 0.20
Nodes (5): CoreEventSyncService, selectedServiceObservable(), ServiceQuerySourceInternal, Injectable, CoreSyncedEventEnvelope

### Community 365 - "Migration Plan — Global → Scoped Docker"
Cohesion: 0.10
Nodes (19): 1.1 Create Migration, 1.2 Seed Membership for Existing Containers, 1.3 Create Default Scan Configs, 2.1 Add Scoped Endpoints, 2.2 Add Project Resolution to Entity Stream, 3.1 Implement Scan Config, 3.2 Modify Scan Result Persistence, 4.1 New Project Docker Views (+11 more)

### Community 366 - "❌ Not Working / Stub / Dead Features"
Cohesion: 0.11
Nodes (19): 10.1 apps/test/ (💀 DEAD), 10.2 apps/observable-poc/ (💀 DEAD), 10.3 reference/ directory (💀 DEAD), 10. 🗺️ Orphaned Apps, 2.1 System Fleet (💀 DEAD — 4 files), 2.2 System Mesh (⚠️ PARTIAL — partially dead), 2.3 System Root Module (💀 DEAD), 2. 💀 Dead System Modules (+11 more)

### Community 367 - "🛡️ Complete Anti-Pattern Audit Report"
Cohesion: 0.11
Nodes (18): 1. Type Safety Collapse (361 violations in production), 2. Error Handling Fragmentation, 3. Architecture Boundary Violations, 4. Package Health, 5. Dependency & Version Issues, 6. Web App Quality, 7. Documentation Drift, Anti-Pattern Catalog Expansions Applied (+10 more)

### Community 368 - "hooks.ts"
Cohesion: 0.26
Nodes (9): getObservableSchemaDetails(), isAsyncIteratorObject(), isObservableLike(), Observable, OBSERVABLE_DETAILS_SYMBOL, ObservableObserver, ObservableSchemaDetails, ObservableSubscription (+1 more)

### Community 369 - ".params"
Cohesion: 0.13
Nodes (12): Home, convertURLSearchParamsToObject(), emptySchema, LinkProps, NavLinkProps, RouteBaseDefinition, RouteBuilder, RouteInfo (+4 more)

### Community 370 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, declaration, declarationMap, esModuleInterop, jsx, outDir, rootDir, skipLibCheck (+8 more)

### Community 371 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, declaration, declarationMap, esModuleInterop, moduleResolution, outDir, skipLibCheck, types (+8 more)

### Community 372 - "ContainerDockerodeNormalizer"
Cohesion: 0.16
Nodes (3): deploymentEndpoints, enhancedDeployment, deploymentInvalidations

### Community 373 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, declaration, declarationMap, esModuleInterop, moduleResolution, outDir, skipLibCheck, types (+8 more)

### Community 374 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, declaration, declarationMap, esModuleInterop, moduleResolution, outDir, skipLibCheck, types (+8 more)

### Community 375 - "🧪 Features Using Mock Data"
Cohesion: 0.11
Nodes (18): 1.1 Projects List Page, 1.2 Project Detail Page, 1.3 Project Configuration Pages, 1.4 Service Detail & Sub-Pages, 1. Projects Module — Web Frontend Only, 2.1 Deployments List Page, 2. Deployments Module — Web Frontend, 3.1 Services List Page (+10 more)

### Community 376 - "index.ts"
Cohesion: 0.10
Nodes (20): coreEventStreams, CoreEventStreamRepository, CoreEventStreamRow, toDto(), Injectable, coreEventStreamFindByIdContract, coreEventStreamListConfig, coreEventStreamListContract (+12 more)

### Community 377 - "test-copilot-setup.ts"
Cohesion: 0.24
Nodes (14): colors, LogLevel, main(), printSummary(), recordTest(), runCommand(), testAvailableScripts(), testDependencies() (+6 more)

### Community 378 - "package.json"
Cohesion: 0.12
Nodes (15): author, description, chalk, keywords, license, chalk, main, name (+7 more)

### Community 379 - "CliAuthService"
Cohesion: 0.12
Nodes (10): CreateDefaultAdminCommand, CreateDefaultAdminEnv, createDefaultAdminEnvSchema, Command, Injectable, CliAuthContext, CliAuthMethod, CliAuthService (+2 more)

### Community 380 - "runtime-configuration-accessor.ts"
Cohesion: 0.16
Nodes (15): domainEnvironmentVariablesSchema, ProjectRuntimeConfig, runtimeStoragePolicySchema, ServiceRuntimeConfig, parseCpuMillicores(), parseMemoryMb(), PROVIDER_TYPE_SET, PROVIDER_VALUES (+7 more)

### Community 381 - "RuntimeRuleDsl"
Cohesion: 0.11
Nodes (8): RequestedEnvironment, RuntimeConfigurationDispatchConditionInput, RuntimeConfigurationDispatchPatchInput, RuntimeConfigurationDispatchRuleInput, RuntimeConfigurationStateTransitionInput, RuntimeConfigurationStateMachineBuilder, RuntimeRuleDsl, RuntimeTransitionDsl

### Community 382 - "DockerFileEntry"
Cohesion: 0.13
Nodes (14): 1. Entity Definition — Canonical Style, 2. Base Service — `MeshResourceService<TEntity>`, 3.1 Join Builder API, 3.2 Type-Level Join Resolution, 3.3 Runtime Join Execution, 3. Join System Design, 4. Fluent Query Builder — Final API, 5. Unified CRUD Contract Generator (+6 more)

### Community 383 - "server.tsx"
Cohesion: 0.14
Nodes (16): getSessionFromCookie(), getServerSession(), IMPORTANT: This file has server-side side effects., reactQueryConfig, configureLayoutAuth(), createSessionLayout(), NextLayoutProps, IMPORTANT: The fallback must not use hooks that require context (+8 more)

### Community 384 - "index.ts"
Cohesion: 0.12
Nodes (15): activitySummarySchema, analyticsReportSchema, analyticsResourceUsageSchema, analyticsServiceHealthSchema, applicationMetricsSchema, databaseMetricsSchema, dateRangeSchema, deploymentAnalyticsSchema (+7 more)

### Community 385 - "test.json"
Cohesion: 0.12
Nodes (15): compilerOptions, noImplicitAny, noUnusedLocals, noUnusedParameters, types, display, exclude, extends (+7 more)

### Community 386 - "devDependencies"
Cohesion: 0.12
Nodes (16): devDependencies, chokidar, jsdom, @testing-library/jest-dom, @testing-library/react, @testing-library/user-event, @types/node, @vitejs/plugin-react (+8 more)

### Community 387 - "Implementation Order (Topological)"
Cohesion: 0.12
Nodes (16): Deleted Files (29 files), File Manifest, Implementation Order (Topological), Modified Files (3 files), New Files (est. 25 files), Overview, Phase 0: Foundation Layer, Phase 1: SubAppOrchestrator (+8 more)

### Community 388 - "tsconfig.json"
Cohesion: 0.13
Nodes (15): compilerOptions, moduleResolution, paths, strict, exclude, extends, include, ./* (+7 more)

### Community 389 - "components.json"
Cohesion: 0.12
Nodes (15): aliases, components, hooks, lib, ui, utils, rsc, $schema (+7 more)

### Community 390 - "package.json"
Cohesion: 0.14
Nodes (14): exports, ./styles/globals.css, import, license, main, module, name, overrides (+6 more)

### Community 391 - "context-menu.tsx"
Cohesion: 0.13
Nodes (14): 1. Entity Definition, 2.1 Full Implementation, 2.2 Key Design Decisions, 2. MeshResourceService — Complete Base Service, 3.1 New `MeshQueryBuilder.join()` Signature, 3.2 Executor Join Execution with Entity Cache, 3.3 User-Facing Examples, 3. Join System — Design Spec (v3) (+6 more)

### Community 392 - "tsconfig.json"
Cohesion: 0.12
Nodes (15): compilerOptions, outDir, paths, plugins, exclude, extends, include, dist (+7 more)

### Community 393 - "config.js"
Cohesion: 0.12
Nodes (15): ORGANIZATION_RESOURCES, ORGANIZATION_ROLES, organizationBuilder, organizationPermissionConfig, organizationRoleMetaShape, PLATFORM_RESOURCES, PLATFORM_ROLES, platformBuilder (+7 more)

### Community 394 - "compilerOptions"
Cohesion: 0.12
Nodes (15): compilerOptions, esModuleInterop, moduleResolution, outDir, rootDir, skipLibCheck, target, exclude (+7 more)

### Community 395 - "init"
Cohesion: 0.50
Nodes (4): cache, dependsOn, outputs, init

### Community 396 - "utils.ts"
Cohesion: 0.38
Nodes (5): ButtonGroup(), ButtonGroupSeparator(), ButtonGroupText(), buttonGroupVariants, Separator()

### Community 397 - "MIGRATION-PLAN-OVERVIEW.md"
Cohesion: 0.12
Nodes (13): API-specific Issues, Controller: 38 ORPC handlers, Module Structure, Project — Subagent 1/3: API/Backend Layer, Repositories (3), Services (4), Todo List (API Layer ONLY), Current Status (+5 more)

### Community 398 - "2. API Backend — Core Infrastructure"
Cohesion: 0.12
Nodes (16): 2.1 Authentication (✅ WORKING), 2.2 Database (✅ WORKING), 2.3 Mesh (✅ WORKING), 2.4 Events (✅ WORKING), 2.5 Lifecycle (✅ WORKING), 2. API Backend — Core Infrastructure, 3.1 Docker Dashboard (All Docker pages), 3.2 Admin Pages (+8 more)

### Community 399 - "🔍 Code Perfection Standards"
Cohesion: 0.12
Nodes (17): Auth, Bundle size, 🔍 Code Perfection Standards, Dates & Timezones, Heavy computations, HTTP, i18n / l10n, Input Validation (+9 more)

### Community 400 - "scripts"
Cohesion: 0.13
Nodes (14): name, private, scripts, build, compile, dev, format, format:check (+6 more)

### Community 401 - "development-session.ts"
Cohesion: 0.17
Nodes (13): createInitialDevelopmentServicePolicies(), createTimelineEvent(), DevelopmentDependencyHandling, DevelopmentExecutionLocation, DevelopmentPolicyService, DevelopmentServiceExecutionPolicy, DevelopmentSessionActionGuards, DevelopmentSessionLifecyclePhase (+5 more)

### Community 402 - "vocabulary.schema.ts"
Cohesion: 0.09
Nodes (20): deploymentGetTemplateProvenanceByRunContract, deploymentGetTemplateProvenanceContract, deploymentTemplateProvenanceByRunOps, deploymentTemplateProvenanceOps, deploymentTemplateProvenanceUpsertOps, deploymentUpsertTemplateProvenanceContract, DeploymentTemplateProvenance, DeploymentTemplateProvenanceByRunResult (+12 more)

### Community 403 - "utils.ts"
Cohesion: 0.30
Nodes (11): getMembersArray(), isOrganizationRoleAtLeast(), RequireOrganizationRole(), RequireOrganizationRoleProps, useOrganizationRole(), isPlatformRoleAtLeast(), RequirePlatformRole(), RequirePlatformRoleProps (+3 more)

### Community 404 - "QueryBuilder"
Cohesion: 0.22
Nodes (7): userEndpoints, enhancedUser, useCreateUser(), useDeleteUser(), useUpdateUser(), useUserActions(), userInvalidations

### Community 405 - "dependencies"
Cohesion: 0.13
Nodes (15): cookie, dependencies, better-auth, cookie, react, @repo/type-guards, @simplewebauthn/server, sonner (+7 more)

### Community 407 - "compilerOptions"
Cohesion: 0.13
Nodes (14): compilerOptions, declaration, declarationMap, esModuleInterop, moduleResolution, outDir, skipLibCheck, exclude (+6 more)

### Community 408 - "compilerOptions"
Cohesion: 0.13
Nodes (14): compilerOptions, declaration, declarationMap, esModuleInterop, moduleResolution, outDir, skipLibCheck, exclude (+6 more)

### Community 409 - "utils.ts"
Cohesion: 0.24
Nodes (14): convertToRequiredType(), getExpectedType(), getInnerSchema(), getSchemaShape(), getSchemaTypeName(), parseShape(), ProcessedValue, processSchema() (+6 more)

### Community 410 - "invalidation.ts"
Cohesion: 0.25
Nodes (9): autoInvalidate(), commonPatterns, getProcedureQueryKey(), invalidateProcedure(), refetchProcedure(), RouterProcedure, InvalidationStrategy, ProcedureInvalidationConfig (+1 more)

### Community 411 - "service.service.ts"
Cohesion: 0.14
Nodes (13): analyticsContract, analyticsCreateReportConfigContract, analyticsDeleteReportConfigContract, analyticsDeleteReportContract, analyticsDownloadReportContract, analyticsGenerateReportContract, analyticsGetReportContract, analyticsListReportConfigsContract (+5 more)

### Community 412 - "migration-journal.service.ts"
Cohesion: 0.14
Nodes (5): AppModule, OrchestratorService, SubAppCascadeService, SetupSubAppModule, runSubApp()

### Community 413 - "fleet-rollout-planner.service.ts"
Cohesion: 0.24
Nodes (3): buildFuzzyHaystack(), CoreEventSyncQueryBuilder, Optional

### Community 414 - "tsconfig.json"
Cohesion: 0.14
Nodes (13): compilerOptions, outDir, paths, exclude, extends, include, dist, node_modules (+5 more)

### Community 415 - "AppLoadingScreen.tsx"
Cohesion: 0.21
Nodes (6): AppLoadingCard(), AppLoadingElegant(), AppLoadingMinimal(), AppLoadingScreen(), AppLoadingSpinner(), IMPORTANT: This is rendered in Suspense fallbacks, so it MUST NOT:

### Community 416 - "scripts"
Cohesion: 0.14
Nodes (14): scripts, build, clean, compile, dev, format, format:check, lint (+6 more)

### Community 417 - "build.ts"
Cohesion: 0.18
Nodes (12): args, copyStylesToDist(), distCjsDir, distDir, distEsmDir, getFiles(), getSharedConfig(), isTestFile() (+4 more)

### Community 418 - "form.tsx"
Cohesion: 0.15
Nodes (10): FormControl, FormDescription, FormFieldContext, FormFieldContextValue, FormItem, FormItemContext, FormItemContextValue, FormLabel (+2 more)

### Community 419 - "environment.schema.ts"
Cohesion: 0.14
Nodes (13): ENV_NAMES, EnvName, envNameSchema, ProjectEnvironmentDeploymentStrategy, ProjectEnvironmentHealthGate, ProjectEnvironmentStartupMode, projectEnvironmentStartupModeSchema, ServiceHealthProtocol (+5 more)

### Community 420 - "provider-runner.schema.ts"
Cohesion: 0.14
Nodes (13): OrganizationPlan, organizationPlanSchema, RunnerNetworkMode, SERVICE_PROVIDER_TYPES, SERVICE_RUNNER_TYPES, ServiceDeploymentProfile, serviceDeploymentProfileSchema, ServiceProviderType (+5 more)

### Community 421 - "test"
Cohesion: 0.67
Nodes (3): test, dependsOn, inputs

### Community 422 - "ServiceDependencyGraphPanel.tsx"
Cohesion: 0.09
Nodes (19): computeServiceDepths(), DependencyRecord, ENV_EDGE_COLORS, EnvironmentGroupNode(), EnvironmentGroupNodeData, getColorForEnvironment(), isDependencyEnabledInEnvironment(), isServiceInEnvironment() (+11 more)

### Community 423 - "domain.ts"
Cohesion: 0.08
Nodes (25): organizationDomains, organizationDomainsRelations, projectDomains, projectDomainsRelations, serviceDomainMappingsRelations, sslProviderEnum, AddDomainResponse, OrganizationDomain (+17 more)

### Community 424 - "Phase 4: Architecture Cleanup 🏗️"
Cohesion: 0.12
Nodes (15): 4.1 Create GitHub ORPC Contract, 4.2.1 Mesh Connect Flow (2 calls), 4.2.2 Setup Wizard Sign-In (2 calls), 4.2 Replace Direct `fetch()` Calls with ORPC (4 files), 4.3.1 Dashboard Sidebar, 4.3.2 RequirePlatformRole JSDoc, 4.3 Fix Hardcoded `href` → Declarative Routing (2 files), 4.4 Consolidate Auth Layers (+7 more)

### Community 425 - "index.ts"
Cohesion: 0.26
Nodes (9): GuardResult, StateChangeEvent, TypedEvent, TypedState, MachineContext, StateConfig, TransitionConfig, StateMachineModule (+1 more)

### Community 426 - "SystemMetricsService"
Cohesion: 0.33
Nodes (6): MetricsCache, SystemMetricsService, Injectable, SystemMetricsModule, Module, SystemMetricsSnapshot

### Community 427 - "load-balancer.controller.ts"
Cohesion: 0.23
Nodes (4): AuthModule, Module, ApiSessionAuthService, Injectable

### Community 428 - "dependencies"
Cohesion: 0.15
Nodes (13): dependencies, @nestjs/common, @nestjs/core, @nestjs/platform-express, reflect-metadata, @repo/type-guards, rxjs, @nestjs/common (+5 more)

### Community 429 - "jest"
Cohesion: 0.15
Nodes (13): jest, collectCoverageFrom, coverageDirectory, moduleFileExtensions, rootDir, testEnvironment, testRegex, transform (+5 more)

### Community 430 - "scripts"
Cohesion: 0.15
Nodes (13): scripts, build, format, lint, start, start:debug, start:dev, start:prod (+5 more)

### Community 431 - "utils.ts"
Cohesion: 0.28
Nodes (12): convertToRequiredType(), getAllParamsAsArrays(), getInnerType(), getRequireds(), parseArray(), parseBoolean(), ParsedData, parseNumber() (+4 more)

### Community 432 - "scripts"
Cohesion: 0.15
Nodes (13): scripts, build, dev, format, format:check, migrate:unified-info-files, prepublish, prettify (+5 more)

### Community 433 - "react.ts"
Cohesion: 0.24
Nodes (9): Props, createBaseConfig(), defaultConfig, createNextJsConfig(), defaultConfig, createNodeConfig(), defaultConfig, createReactConfig() (+1 more)

### Community 435 - "case-utils.ts"
Cohesion: 0.23
Nodes (11): CaseFormat, CaseFormatConfig, convertCase(), ConvertedKeys, convertObjectKeys(), DEFAULT_CASE_CONFIG, KeyMappingFunction, toCamelCase() (+3 more)

### Community 436 - "chart.tsx"
Cohesion: 0.21
Nodes (11): ChartConfig, ChartContainer(), ChartContext, ChartContextProps, ChartLegendContent(), ChartTooltipContent(), getPayloadConfigFromPayload(), INITIAL_DIMENSION (+3 more)

### Community 437 - "builder.ts"
Cohesion: 0.15
Nodes (10): ActionsForResource, AllResourceNames, ExtractPermissions, ExtractRolePermissions, ExtractStatementUnion, IsEmptyRecord, MergeDefaultStatements, MergeRecords (+2 more)

### Community 438 - "index.ts"
Cohesion: 0.27
Nodes (12): AccessControlInstance, ActionsForResource, AllRoleNames, AuthenticatedUserType, CommonPermissionPattern, Permission, PermissionTypes, Resource (+4 more)

### Community 439 - "configuration-definition.service.ts"
Cohesion: 0.24
Nodes (11): ConfigurationScope, configurationScopeSchema, organizationRuntimeConfigSchema, projectRuntimeConfigSchema, serviceRuntimeConfigSchema, userRuntimeConfigSchema, RuntimeConfigurationDefinition, RuntimeConfigurationDefinitionMap (+3 more)

### Community 440 - "Vulnerability Scanning — Per-Project Architecture"
Cohesion: 0.13
Nodes (14): Architecture Overview, Data Flow, Design Goals, Merge Rules, Per-Node Configuration, Scan Config Merge Strategy (Layered Override), Service Filters, ServiceFilter (Single Canonical Definition) (+6 more)

### Community 441 - "Dashboard View"
Cohesion: 0.13
Nodes (11): API (Backend) — 16 Product Modules + 2 System Modules, API Core (Infrastructure) — 23 Core Modules, Dashboard View, Feature Status Assessment, Orphaned Apps, Quick Navigation, Status Legend, Sub-Apps (+3 more)

### Community 442 - "Individual Report Summaries"
Cohesion: 0.13
Nodes (14): 1. Docker — API Layer (`06-DOCKER-API-REPORT.md`), 2. Docker — Web Layer (`07-DOCKER-WEB-REPORT.md`), 3. Docker — Contracts (`08-DOCKER-CONTRACTS-REPORT.md`), 4. Deployment — API Layer (`01-DEPLOYMENT-API-REPORT.md`), 5. Deployment — Web Layer (`02-DEPLOYMENT-WEB-REPORT.md`), 6. Deployment — Contracts (`03-DEPLOYMENT-CONTRACTS-REPORT.md`), 7. Project — 3 Separate Subagents, 8. Service — 3 Separate Subagents (+6 more)

### Community 443 - "mesh-observable-types.ts"
Cohesion: 0.23
Nodes (7): MeshChangeEvent, MeshChangeType, MeshOperation, MeshRequestResult, MeshStreamEvent, MeshStreamEventType, MeshStreamResult

### Community 444 - "docker-common.module.ts"
Cohesion: 0.05
Nodes (53): CoreDockerModule, EventsModule, Global, Module, MeshCoreModule, Module, DockerCommonModule, Module (+45 more)

### Community 445 - "fleet.module.ts"
Cohesion: 0.20
Nodes (9): FLEET_SERVICES, FleetModule, Module, CrossProjectDependency, CrossProjectGateConfig, CrossProjectGateMode, CrossProjectGateService, CrossProjectGateServiceResult (+1 more)

### Community 446 - "DependencyTemplateConfig"
Cohesion: 0.23
Nodes (12): BASE_STATUS, createDefaultServiceConfig(), createExecutionOverrides(), createProviderConfig(), createRunnerConfig(), createStatusMap(), PROFILE_OVERRIDES, resolveProviderType() (+4 more)

### Community 447 - "fleet-rollback.service.ts"
Cohesion: 0.23
Nodes (6): FleetRollbackDecision, FleetRollbackRequest, FleetRollbackScope, FleetRollbackService, ServiceDeploymentRef, Injectable

### Community 448 - "nest-cli.json"
Cohesion: 0.17
Nodes (11): collection, compilerOptions, deleteOutDir, entryFile, sourceRoot, type, monorepo, projects (+3 more)

### Community 449 - "app.module.ts"
Cohesion: 0.21
Nodes (8): AppModule, Module, HealthController, Controller, Get, bootstrap(), LoadBalancerModule, Module

### Community 450 - "🔴 Anti-Pattern Catalog (Always Reject)"
Cohesion: 0.13
Nodes (15): 🔴 Anti-Pattern Catalog (Always Reject), Architecture & Boundary Anti-Patterns, Configuration & Environment Anti-Patterns, Defensive Code That Hides Bugs, Documentation Anti-Patterns, Error Handling Anti-Patterns (Expanded), Infrastructure Anti-Patterns, Logging / Observability Anti-Patterns (+7 more)

### Community 451 - "makeRoute.tsx"
Cohesion: 0.24
Nodes (9): convertURLSearchParamsToObject(), useSearchParams(), CoreRouteElements, createPathBuilder(), createRouteBuilder(), emptySchema, makeRoute(), RouteBuilder (+1 more)

### Community 452 - "package.json"
Cohesion: 0.17
Nodes (11): files, src, name, private, scripts, format, format:check, prettify (+3 more)

### Community 453 - "build.ts"
Cohesion: 0.21
Nodes (10): args, distCjsDir, distDir, distEsmDir, isTestFile(), packageRoot, runBuild(), setupWatcher() (+2 more)

### Community 454 - "package.json"
Cohesion: 0.17
Nodes (11): author, description, config, keywords, license, main, name, private (+3 more)

### Community 455 - "exports"
Cohesion: 0.17
Nodes (12): import, types, exports, ./base, ./node, ./react, import, import (+4 more)

### Community 456 - "check.ts"
Cohesion: 0.20
Nodes (9): healthCheckContract, healthCheckInput, healthCheckOps, healthCheckOutput, healthDetailedContract, healthDetailedInput, healthDetailedOps, healthDetailedOutput (+1 more)

### Community 457 - "tsconfig.test.json"
Cohesion: 0.17
Nodes (11): compilerOptions, baseUrl, jsx, paths, extends, include, ./, @repo/config-typescript/config/test.json (+3 more)

### Community 458 - "exports"
Cohesion: 0.17
Nodes (12): exports, ./client, ./mesh, ./permissions, ./permissions/plugins, ./react, ./react/session, ./react/session/client (+4 more)

### Community 459 - "configuration.schema.ts"
Cohesion: 0.17
Nodes (11): ProjectEnvironmentConfig, projectEnvironmentConfigSchema, ServiceEnvironmentExecutionOverride, serviceEnvironmentExecutionOverrideSchema, ServiceProviderConfig, serviceProviderConfigSchema, ServiceRunnerConfig, serviceRunnerConfigSchema (+3 more)

### Community 460 - "peerDependencies"
Cohesion: 0.17
Nodes (12): @repo/auth, next, nprogress, react, @repo/auth, @tanstack/react-query, peerDependencies, next (+4 more)

### Community 461 - "Sub-App Architecture"
Cohesion: 0.14
Nodes (13): Architecture Decisions, Bridge Pattern, `GlobalDatabaseModule` — Pool Creation, Key Principles, Lifecycle Sequence, Overview, `SetupSubAppModule` — Central Sub-App Runner, Startup Order in AppModule (+5 more)

### Community 462 - "filtering.ts"
Cohesion: 0.23
Nodes (8): FleetReadinessGateService, GateCheckFn, GateEvaluation, GateStatus, ReadinessGateOptions, ServiceGateResult, Injectable, DependencyTemplateConfig

### Community 463 - "drift-reconciliation.service.ts"
Cohesion: 0.24
Nodes (8): DeploymentStatus, DriftReconciliationReport, DriftReconciliationService, DriftSeverity, ServiceActualState, ServiceDesiredState, ServiceDriftRecord, Injectable

### Community 464 - "scripts"
Cohesion: 0.18
Nodes (11): scripts, build, clean, compile, dev, lint, start, test (+3 more)

### Community 465 - "orpc"
Cohesion: 0.18
Nodes (11): orpc, @orpc/client, @orpc/contract, @orpc/json-schema, @orpc/nest, @orpc/openapi, @orpc/openapi-client, @orpc/server (+3 more)

### Community 466 - "2026-01-01-unified-info-files.migration.ts"
Cohesion: 0.36
Nodes (10): fileExists(), findSourceFile(), migrate(), migrateDirectory(), MigrationOptions, MigrationResult, parseArgs(), printHelp() (+2 more)

### Community 467 - "build.ts"
Cohesion: 0.20
Nodes (9): args, distCjsDir, distDir, distEsmDir, isTestFile(), packageRoot, runBuild(), srcDir (+1 more)

### Community 468 - "react-library.json"
Cohesion: 0.18
Nodes (10): compilerOptions, jsx, lib, display, extends, ./base.json, dom, dom.iterable (+2 more)

### Community 469 - "exclude"
Cohesion: 0.17
Nodes (11): 1. Entity Definition — Single Canonical Style, 2. Base Service — `MeshResourceService<TEntity>`, 3.1 Flat-Object Join API (Fixes TAlias inference), 3.2 Hash-Join Execution (Fixes performance), 3.3 User-Facing API (DX Examples), 3. Join System — Fixed Design, 4. Cross-Mesh Execution — MeshContractCaller, 5. DI & Module Wiring (+3 more)

### Community 470 - "MeshResourceSubQueryBuilder"
Cohesion: 0.27
Nodes (9): serviceStreamsContract, serviceQueryStreamContract, serviceStreamEventContractEntitySchema, serviceStreamEventOps, serviceStreamEventSchema, serviceStreamEventTypeSchema, serviceStreamQueryFiltersSchema, ServiceStreamQueryInput (+1 more)

### Community 471 - "lint"
Cohesion: 0.67
Nodes (3): dependsOn, inputs, lint

### Community 472 - "turbo.json"
Cohesion: 0.25
Nodes (7): globalDependencies, globalEnv, globalPassThroughEnv, remoteCache, enabled, $schema, ui

### Community 473 - "progress-tasks.tsx"
Cohesion: 0.18
Nodes (8): buildTasksFromEvents(), LogLine(), mapStatus(), ProgressTask, StatusBadge(), TaskCard(), TaskStatus, TaskTemplate

### Community 474 - "🚩 Category 1: Dead Code — Unused Files"
Cohesion: 0.15
Nodes (13): 1.10 `reference/` Directory, 1.11 Config Packages — Prettier, 1.12 Root Level, 1.1 API Core — Legacy/Deprecated Modules (17+ files), 1.2 API Modules — Orphaned/Empty Files (6+ files), 1.3 API System — Fleet & Mesh Modules (7+ files), 1.4 API Sub-Apps — Setup Wizard (6+ files), 1.5 Web Frontend — Mock Hooks (12+ files) (+5 more)

### Community 475 - "Auth — Subagent 1/3: API/Backend Layer"
Cohesion: 0.15
Nodes (12): Auth Configuration, Auth — Subagent 1/3: API/Backend Layer, AuthCoreService, AuthGuard, AuthModule, AuthService, Issues, Key Components (+4 more)

### Community 476 - "Auth — Subagent 2/3: Web/Frontend Layer"
Cohesion: 0.15
Nodes (12): ACTIVE:, ACTIVE (has external importers):, Auth Components, Auth Lib, Auth Pages, Auth — Subagent 2/3: Web/Frontend Layer, DEAD:, DEAD (no external importers): (+4 more)

### Community 477 - "common.sh"
Cohesion: 0.20
Nodes (3): get_feature_paths(), has_git(), common.sh script

### Community 478 - "build.ts"
Cohesion: 0.29
Nodes (9): args, cleanup(), copyMigrations(), distDir, main(), runBuild(), runDbGenerate(), srcDir (+1 more)

### Community 479 - "entrypoint.dev.ts"
Cohesion: 0.33
Nodes (9): DEPLOYER_VERSION, EntrypointConfig, interpretStartupResult(), main(), phaseDiagnostics(), phaseStartupCheck(), phaseValidateEnvironment(), phaseVersionExtraction() (+1 more)

### Community 480 - "1. API Backend — Product Modules"
Cohesion: 0.15
Nodes (13): 1.10 Provider Schema (✅ WORKING), 1.11 Test Endpoints (✅ WORKING), 1.12 Domain Management (✅ WORKING), 1.1 Docker Management (✅ WORKING — Most Complex Feature), 1.2 Deployment Management (✅ WORKING), 1.3 User Management (✅ WORKING), 1.4 Organization Management (✅ WORKING), 1.5 Project Management (✅ WORKING with TODOs) (+5 more)

### Community 482 - "fleet-failure-containment.service.ts"
Cohesion: 0.29
Nodes (5): ContainmentInput, ContainmentResult, FailureDomain, FleetFailureContainmentService, Injectable

### Community 483 - "input-group.tsx"
Cohesion: 0.24
Nodes (9): InputGroup(), InputGroupAddon(), inputGroupAddonVariants, InputGroupButton(), inputGroupButtonVariants, InputGroupInput(), InputGroupText(), InputGroupTextarea() (+1 more)

### Community 484 - "openapi.ts"
Cohesion: 0.20
Nodes (7): GET, Route, GET, Route, docs, generator, registry

### Community 485 - "database"
Cohesion: 0.20
Nodes (10): database, bcrypt, better-sqlite3, drizzle-kit, drizzle-orm, drizzle-zod, nanoid, postgres (+2 more)

### Community 486 - "testing"
Cohesion: 0.20
Nodes (10): testing, jsdom, @testing-library/jest-dom, @testing-library/react, @testing-library/user-event, @vitejs/plugin-react, vitest, @vitest/coverage-istanbul (+2 more)

### Community 487 - "ui"
Cohesion: 0.20
Nodes (10): ui, boxen, class-variance-authority, cmdk, framer-motion, lucide-react, ora, radix-ui (+2 more)

### Community 488 - "tsconfig.test.json"
Cohesion: 0.20
Nodes (9): compilerOptions, baseUrl, paths, extends, include, @repo/config-typescript/config/test.json, ./src, __tests__/**/* (+1 more)

### Community 489 - "DeploymentLoadBalancerSyncAdapter"
Cohesion: 0.21
Nodes (5): DeploymentLoadBalancerSyncAdapter, LoadBalancerReportedResource, SyncLoadBalancerInput, SyncLoadBalancerResult, Injectable

### Community 490 - "package.json"
Cohesion: 0.05
Nodes (39): dependencies, @orpc/contract, @orpc/shared, @repo/auth, @repo/contracts-common, @repo/contracts-entities, @repo/orpc-utils, @repo/provider-schema (+31 more)

### Community 491 - "dependencies"
Cohesion: 0.22
Nodes (10): dependencies, concurrently, @repo/type-guards, rimraf, concurrently, @repo/type-guards, rimraf, peerDependencies (+2 more)

### Community 492 - "exports"
Cohesion: 0.20
Nodes (10): exports, ./hooks, ./layout-wrappers, ./layout-wrappers/server, ./page-wrappers, ./page-wrappers/client, ./page-wrappers/server, ./utils (+2 more)

### Community 493 - "tsconfig.test.json"
Cohesion: 0.20
Nodes (9): compilerOptions, baseUrl, paths, extends, include, @repo/config-typescript/config/test.json, ./src, @ (+1 more)

### Community 495 - "VariableNumber"
Cohesion: 0.25
Nodes (8): tanstack, @tanstack/eslint-plugin-query, @tanstack/react-devtools, @tanstack/react-form, @tanstack/react-query, @tanstack/react-query-devtools, @tanstack/react-table, @tanstack/react-virtual

### Community 496 - "exclude"
Cohesion: 0.22
Nodes (8): exclude, extends, dist, node_modules, **/*spec.ts, test, ./tsconfig.json, vitest.config.mts

### Community 497 - "page.tsx"
Cohesion: 0.08
Nodes (33): computeDurationLabel(), DashboardDeploymentsPage(), DeploymentSortKey, formatDate(), projectLabel(), severityVariant(), SortDirection, statusBadgeVariant() (+25 more)

### Community 498 - "build"
Cohesion: 0.22
Nodes (9): chokidar, @parcel/watcher, rimraf, @types/bun, @types/node, typescript, @typescript/native-preview, vite (+1 more)

### Community 499 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, build, clean, compile, format, format:check, prettify, test (+1 more)

### Community 500 - "page.tsx"
Cohesion: 0.31
Nodes (3): AbstractDockerContainerService, DockerContainerStartOptions, Injectable

### Community 501 - "exclude"
Cohesion: 0.29
Nodes (7): react, react, react-dom, react-hook-form, react-use, @types/react, @types/react-dom

### Community 503 - "Data Model — Scoped Docker Entities"
Cohesion: 0.17
Nodes (11): Container Project Resolution Strategy, Core Principle: Project is the Root Scope, Data Model — Scoped Docker Entities, Entity Relationship Diagram, `ImageProjectMembership` (NEW — tracking table), New/Modified Schemas, Orphan Container Handling, `ProjectScanConfig` (NEW — persisted to Postgres) (+3 more)

### Community 504 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, format, format:check, lint, prettify, test, test:coverage, test:watch (+1 more)

### Community 505 - "tsconfig.test.json"
Cohesion: 0.22
Nodes (8): compilerOptions, paths, extends, include, @repo/config-typescript/config/test.json, ./src, __tests__/**/*, @

### Community 506 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, format, format:check, lint, prettify, test, test:coverage, test:watch (+1 more)

### Community 507 - "schemas.ts"
Cohesion: 0.25
Nodes (8): RolesAsRoleObjects, InferAllActions, InferPermission, InferResourceNames, InferRoleNames, InferSchemas, assertDefined(), createSchemas()

### Community 508 - "dependencies"
Cohesion: 0.22
Nodes (9): dependencies, nuqs, query-string, @repo/type-guards, zod, nuqs, query-string, @repo/type-guards (+1 more)

### Community 509 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, format, format:check, lint, prettify, test, test:coverage, test:watch (+1 more)

### Community 510 - "tsconfig.json"
Cohesion: 0.22
Nodes (8): exclude, extends, include, dist, node_modules, @repo/config-typescript/config/base.json, src/**/*, $schema

### Community 511 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, format, format:check, lint, prettify, test, test:coverage, test:watch (+1 more)

### Community 512 - "pagination.ts"
Cohesion: 0.22
Nodes (8): 1. ORPC Contract Generation — Type-Safe (v4 fix), 2.1 External API Protection, 2.2 Internal Mesh Peer Authentication, 2.3 Security Architecture Summary, 2. Security & Authentication (v4 addition), 3. Final Compliance Matrix — v4, Changes from v3 — Fixing Final Blockers, Mesh Resource Access Architecture — Proposal v4 (Final)

### Community 513 - "detect-changed-packages.ts"
Cohesion: 0.33
Nodes (8): ChangeType, detectChangedPackages(), extractPackageName(), getChangedFiles(), GitDiffOptions, main(), PackageChange, parseArgs()

### Community 514 - "test.controller.ts"
Cohesion: 0.09
Nodes (23): AfterHook(), AllowAnonymous(), AuthenticatedUser, AuthHookContext, BeforeHook(), Hook(), OptionalAuth(), Session (+15 more)

### Community 517 - "exclude"
Cohesion: 0.25
Nodes (7): exclude, extends, dist, node_modules, **/*spec.ts, test, ./tsconfig.json

### Community 518 - "package.json"
Cohesion: 0.25
Nodes (7): name, optionalDependencies, overrides, @types/react, @types/react-dom, private, version

### Community 519 - "entrypoint.dev.ts"
Cohesion: 0.39
Nodes (7): DECLARATIVE_ROUTING_DIST, DECLARATIVE_ROUTING_PKG_DIR, ensureLocalDeclarativeRoutingBuilt(), ensureRoutesGenerated(), main(), startProcesses(), validateEnvironment()

### Community 520 - "dev"
Cohesion: 0.25
Nodes (8): dev, concurrently, cross-env, dotenv, dotenv-cli, dotenv-expand, wait-on, wait-port

### Community 521 - "next"
Cohesion: 0.25
Nodes (8): next, declarative-routing, eslint-config-next, next, @next/bundle-analyzer, next-sitemap, next-themes, nextjs-toploader

### Community 522 - "docker-compose.mdx"
Cohesion: 0.18
Nodes (10): Compose hygiene checklist, Environment variables, Partial development, Production with Compose, Project isolation (multiple instances), Quick start (development), Related docs, Troubleshooting (+2 more)

### Community 523 - "build.ts"
Cohesion: 0.13
Nodes (17): args, cleanup(), distDir, main(), runBuild(), srcDir, watch, args (+9 more)

### Community 524 - "tsconfig.scripts.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, extends, include, ./scripts/**/*.ts, ../tsconfig/config/base.json, $schema

### Community 525 - "tsconfig.scripts.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, extends, include, ./scripts/**/*.ts, $schema, ../tsconfig/config/scripts.json

### Community 526 - "tsconfig.scripts.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, extends, include, ./scripts/**/*.ts, ../tsconfig/config/base.json, $schema

### Community 527 - "Startup v2 — Complete Flow Per Case"
Cohesion: 0.18
Nodes (10): Case 1: First-Time Setup (Fresh Install), Case 2: Normal Restart (Already Configured), Case 3: CLI / Script Mode, Case 4: Degraded Mode (Non-Critical Failure), Case 5: Fatal Error (Critical Failure), Case 6: Setup Wizard Deep Flow (User Interaction), Key Decision Points, Startup v2 — Complete Flow Per Case (+2 more)

### Community 528 - "tsconfig.scripts.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, extends, include, ./scripts/**/*.ts, ../tsconfig/config/base.json, $schema

### Community 530 - "tsconfig.scripts.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, extends, include, ./scripts/**/*.ts, ../tsconfig/config/base.json, $schema

### Community 531 - "package.json"
Cohesion: 0.15
Nodes (12): main, name, optional, peerDependenciesMeta, nprogress, @tanstack/react-query, private, sideEffects (+4 more)

### Community 532 - "package.json"
Cohesion: 0.25
Nodes (7): main, name, private, sideEffects, type, types, version

### Community 533 - "generate"
Cohesion: 0.67
Nodes (3): cache, dependsOn, generate

### Community 534 - "docker-compose.mdx"
Cohesion: 0.18
Nodes (10): Compose hygiene checklist, Environment variables, Partial development, Production with Compose, Project isolation (multiple instances), Quick start (development), Related docs, Troubleshooting (+2 more)

### Community 535 - "add-upstream.ts"
Cohesion: 0.39
Nodes (7): addTemplate(), getRemotes(), GitRemoteConfig, hasTemplate(), main(), REMOTE_CONFIG, verifyGitRepository()

### Community 536 - "prepare-render.ts"
Cohesion: 0.46
Nodes (7): checkFile(), main(), printDeploymentInstructions(), RenderDeploymentConfig, testDockerBuild(), verifyDocker(), verifyRequiredFiles()

### Community 537 - "index.tsx"
Cohesion: 0.31
Nodes (6): DockerContainerLogsTab(), formatInputDateTimeToIso(), formatIsoToInputDateTime(), sanitizeContainerLogMessage(), DockerContainerLogsTabProps, LogsTimeRange

### Community 541 - "service-dag.service.ts"
Cohesion: 0.16
Nodes (8): CanaryConfig, RollingConfig, RolloutMode, RolloutPlan, RolloutWave, ServiceDagEdge, ServiceDagNode, ServiceDependencyInput

### Community 542 - ".reportMeshLoad"
Cohesion: 0.43
Nodes (4): Body, HttpCode, Post, HttpHeader

### Community 543 - "Auth — Subagent 3/3: Packages/Contracts Layer"
Cohesion: 0.18
Nodes (10): Auth Contracts (User module — 7 operations), Auth — Subagent 3/3: Packages/Contracts Layer, Comparison: Internal vs Nest Auth, External importers: 80+ matches across 62 files, packages/nest/auth (DEAD — 39 source files, 8 spec files), packages/utils/auth (ACTIVE — 74+ source files), Permissions Builder (system/builder/), Permissions Engine (engine/) (+2 more)

### Community 544 - "package.json"
Cohesion: 0.29
Nodes (6): author, description, license, name, private, version

### Community 545 - "debug-file-watcher.ts"
Cohesion: 0.33
Nodes (6): DebouncedDir, debounceMap, flushPending(), ROOT, WATCH_DIRS, watchDir()

### Community 546 - "tsconfig.scripts.json"
Cohesion: 0.29
Nodes (6): compilerOptions, outDir, extends, include, scripts/**/*.ts, ../../packages/tsconfig/config/scripts.json

### Community 547 - "deploy.sh"
Cohesion: 0.52
Nodes (6): print_error(), print_header(), print_info(), print_success(), print_warning(), deploy.sh script

### Community 548 - "index.ts"
Cohesion: 0.39
Nodes (7): OrchestrationModule, bootstrap(), createCorsMiddleware(), GATEWAY_PORT, log, registerProcessSignalHandlers(), selfTestHealthEndpoint()

### Community 549 - "tsconfig.test.json"
Cohesion: 0.29
Nodes (6): compilerOptions, baseUrl, extends, include, __tests__/**/*, ../../packages/tsconfig/config/test.json

### Community 550 - "index.ts"
Cohesion: 0.10
Nodes (14): SCANNER_META, SCANNER_TYPES, ScannerContainerState, ScannerExecResult, ScannerType, extractDefaultColumnSizes(), hasAccessorKeyAndSize(), hasIdAndSize() (+6 more)

### Community 551 - "build.ts"
Cohesion: 0.54
Nodes (3): FleetRolloutPlannerService, Injectable, ServiceDag

### Community 553 - "package.json"
Cohesion: 0.29
Nodes (6): author, description, keywords, license, name, version

### Community 554 - "use-session.ts"
Cohesion: 0.38
Nodes (6): createUseSession(), CreateUseSessionOptions, DEFAULT_SESSION_QUERY_KEY, SessionResult, useQueryCacheSubscription(), useQueryClientSafe()

### Community 555 - "8. Detailed File Contents for Context"
Cohesion: 0.20
Nodes (10): 8.1 main.ts (CURRENT — to be simplified), 8.2 sub-app-runner.ts (EXISTING — keep as-is), 8.3 sub-app-cascade.service.ts (EXISTING — roll back changes), 8.4 sub-app-runner.module.ts (EXISTING — SubAppRunner.forRoot), 8.5 BaseTriggerService (EXISTING — bridge pattern), 8.6 SubAppResultStore (EXISTING — static result store), 8.7 SetupSubAppModule (EXISTING — orchestrates setup sub-apps), 8.8 RouterModule (EXISTING — gateway NestJS module) (+2 more)

### Community 556 - "dependencies"
Cohesion: 0.29
Nodes (7): dependencies, nuqs, @repo/type-guards, zod, nuqs, @repo/type-guards, zod

### Community 557 - "peerDependencies"
Cohesion: 0.29
Nodes (7): next, react, next, react, peerDependencies, next, react

### Community 558 - "Container Lifecycle — Scoped"
Cohesion: 0.20
Nodes (9): Container Adoption Table, Container Discovery (Existing → Scope Resolution), Container Label Convention, Container Lifecycle — Scoped, Container List API (Scoped), Creation Flow (Deployment-Driven), Cross-Node Container Querying, Environment as Scope (+1 more)

### Community 559 - "node-startup-check.command.ts"
Cohesion: 0.08
Nodes (18): MigrateCommand, Command, Injectable, ResetCommand, Command, Injectable, resetLocal(), SeedCommand (+10 more)

### Community 561 - "🔍 UI/UX Standards"
Cohesion: 0.20
Nodes (10): Accessibility (WCAG 2.1 AA Minimum), Dark Mode / Light Mode, Empty States (Three Patterns), Error States (Three Patterns), Focus Management, Form UX, Loading States (Three Patterns), Microinteractions / Animations (+2 more)

### Community 562 - "🔴 NestJS Server Patterns"
Cohesion: 0.20
Nodes (10): Anti-Patterns, Database Migrations, DI Tokens (Class Tokens Preferred), Logging in Services, Module Imports (Gotcha), Module Structure, 🔴 NestJS Server Patterns, Service / Repository / Controller Split (+2 more)

### Community 564 - "🔍 Developer Experience Standards"
Cohesion: 0.20
Nodes (10): Common Dev Workflows, Debugging Tips, 🔍 Developer Experience Standards, Editor Setup (VS Code), NestJS / API, ORPC / Network, React / Next.js, Test Running Strategies (+2 more)

### Community 566 - "query-builder.test.ts"
Cohesion: 0.25
Nodes (7): domainContract, deleteOrganizationDomainContract, getAvailableDomainsContract, addServiceDomainContract, checkSubdomainAvailabilityContract, listServiceDomainsContract, setPrimaryServiceDomainContract

### Community 567 - "nest-cli.json"
Cohesion: 0.33
Nodes (5): collection, compilerOptions, deleteOutDir, $schema, sourceRoot

### Community 568 - "ORPC Auth Patterns — Public vs Protected Routes"
Cohesion: 0.22
Nodes (8): Available Middleware Functions, Middleware Chain Order, ORPC Auth Patterns — Public vs Protected Routes, Pattern 1: Public Route (No Auth), Pattern 2: Protected Route (Auth Required), Pattern 3: Protected + Role Check, Pattern 4: Protected + Organization Check, Pattern 5: Sub-App Setup Wizard (No Auth)

### Community 569 - "Dead Code, Deprecated APIs & Enhancement Audit Report"
Cohesion: 0.22
Nodes (8): 2.1 API — `@deprecated` Usage (17 instances), 2.2 Packages — `@deprecated` Usage (11 instances), 🚩 Category 2: Deprecated API Annotations & Migration Leftovers, 🚩 Category 7: Duplicate Exports (Knip report), Confidence & Gaps, Dead Code, Deprecated APIs & Enhancement Audit Report, Evidence Map, Executive Summary

### Community 570 - "Deployment — API Layer (Subagent 1/3)"
Cohesion: 0.22
Nodes (8): Action Items (14), Deployment — API Layer (Subagent 1/3), Issues Found, Module Structure, Queue System, Repositories, Services, State Machine

### Community 571 - "Service — Subagent 2/3: Web/Frontend Layer"
Cohesion: 0.22
Nodes (8): Current Status, Domain Hooks: 9 real hooks available, Nested under projects:, Pages: 11 service page.tsx files, Service — Subagent 2/3: Web/Frontend Layer, Standalone:, Todo List (Web Layer ONLY), Web-specific Issues

### Community 572 - "package.json"
Cohesion: 0.33
Nodes (5): description, main, name, types, version

### Community 573 - "Docker — Contracts Layer (Subagent 3/3)"
Cohesion: 0.22
Nodes (8): Action Items, Contract Files: ~8 files in packages/contracts/api/modules/docker/, Docker — Contracts Layer (Subagent 3/3), Dockerode Schemas: 4 entity types, Entity Schemas: 8 docker entity types, Gaps, Stream/SSE Schema, Total Operations: ~30+

### Community 574 - "scripts"
Cohesion: 0.33
Nodes (6): scripts, format, format:check, lint, prettify, type-check

### Community 575 - "package.json"
Cohesion: 0.33
Nodes (5): description, main, name, types, version

### Community 576 - "standardize-package-names.ts"
Cohesion: 0.53
Nodes (5): createReplacePattern(), getAllFiles(), main(), packageRenames, updateFileContent()

### Community 577 - "Auth — Complete Analysis (Subagents 1-3 combined)"
Cohesion: 0.22
Nodes (8): Action Items, API Internal Auth (ACTIVE), Auth — Complete Analysis (Subagents 1-3 combined), Auth Consolidation Summary, packages/nest/auth (DEAD — 39 files), packages/utils/auth (ACTIVE), Web Auth Components, Web Auth Lib

### Community 578 - "setup.state.service.ts"
Cohesion: 0.40
Nodes (4): SetupContext, SetupEvent, setupMachine, SetupState

### Community 579 - "mock-node-server.js"
Cohesion: 0.40
Nodes (3): http, OWNS, server

### Community 580 - "package.json"
Cohesion: 0.40
Nodes (4): main, name, private, version

### Community 581 - "types.ts"
Cohesion: 0.36
Nodes (5): CompositeHooksConfig, CompositeHooksOptions, getOptionalHook(), MutationResult, QueryResult

### Community 583 - "resource-graph.ts"
Cohesion: 0.33
Nodes (6): getAncestorChain(), isAncestorOf(), RESOURCE_GRAPH, ResourceGraph, ResourceNode, ResourceParent

### Community 586 - "exports"
Cohesion: 0.40
Nodes (5): exports, import, types, require, types

### Community 587 - "peerDependencies"
Cohesion: 0.40
Nodes (5): drizzle-orm, @tanstack/react-query, peerDependencies, drizzle-orm, @tanstack/react-query

### Community 588 - "search.ts"
Cohesion: 0.33
Nodes (5): serviceLifecycleContract, serviceToggleActiveBodySchema, serviceToggleActiveContract, serviceToggleActiveOps, serviceToggleActiveParamsSchema

### Community 589 - "exports"
Cohesion: 0.40
Nodes (5): exports, ./builder, ./hooks, ./query, ./standard

### Community 590 - "3. Current Architecture"
Cohesion: 0.25
Nodes (8): 3.1 The Gateway, 3.2 Sub-Apps, 3.3 Current Bootstrap Sequence (main.ts), 3.4 Current Sub-App Lifecycle (SubAppCascadeService), 3.5 Current Database Resolution (Phase 0), 3.6 Current SubAppRunner.forRoot() Pattern, 3.7 Current Bridge Pattern (BaseTriggerService), 3. Current Architecture

### Community 591 - "5. Key Files & Their Roles"
Cohesion: 0.25
Nodes (8): 5.1 Entry Point & Orchestration, 5.2 Gateway & Routing, 5.3 Sub-App Runner, 5.4 Core Infrastructure Modules, 5.5 Existing Sub-App Modules, 5.6 New Files Created (to be removed/replaced), 5.7 Files Modified (to roll back), 5. Key Files & Their Roles

### Community 592 - "generate_label"
Cohesion: 0.50
Nodes (4): generate_label(), Score how well a label matches each domain., Generate a concise label for a community., score_domain()

### Community 594 - "next.config.ts"
Cohesion: 0.50
Nodes (3): config, NOTE: do NOT use `import.meta.url` here — Bun 1.3.14 has a transpiler bug, withMDX

### Community 598 - "deployer-version.ts"
Cohesion: 0.29
Nodes (6): DEPLOYER_VERSION, __dirname, __filename, PKG_PATH, semverCompare(), semverSatisfies()

### Community 599 - "Startup v2 — Complete Bootstrap Flow"
Cohesion: 0.25
Nodes (7): Execution Timeline (Milliseconds), Key Design Points, Key Files Involved, Startup v2 — Complete Bootstrap Flow, Sub-App Dependency Graph (Topological), The Complete Flow, Trigger Emission → Module Resolution Map

### Community 601 - "openapi.template.ts"
Cohesion: 0.50
Nodes (3): docs, generator, registry

### Community 602 - "New Architecture: Image Project Membership"
Cohesion: 0.25
Nodes (7): Current Problem, Image Catalog — Scoped, Image Detail: Scoped Scan Results, Image → Project Association Triggers, New Architecture: Image Project Membership, Scoped Image List API, Tracking Table

### Community 607 - "exports"
Cohesion: 0.50
Nodes (4): exports, ./defaults, ./parsers, ./types

### Community 608 - "API Surface — Scoped Docker Endpoints"
Cohesion: 0.25
Nodes (7): API Surface — Scoped Docker Endpoints, Contract Reorganization, Deprecation Path, Modified Endpoints, New Endpoints, New Module Structure, UI Impact

### Community 610 - "2.4 Service Pages (8 pages + 1 component)"
Cohesion: 0.25
Nodes (8): 2.4.1 Service Detail, 2.4.2 Service Configuration Pages (5 pages), 2.4.3 Service Deployments, 2.4.4 Service Logs, 2.4.5 Service Monitoring, 2.4.6 Service Previews, 2.4.7 Service Ops Overview Cards, 2.4 Service Pages (8 pages + 1 component)

### Community 614 - "Service — Subagent 1/3: API/Backend Layer"
Cohesion: 0.25
Nodes (7): API-specific Issues, Controller: 10 ORPC endpoints, Module Structure, Repository (1), Service — Subagent 1/3: API/Backend Layer, Services (2), Todo List (API Layer ONLY)

### Community 615 - "Service — Subagent 3/3: Contracts/Entities Layer"
Cohesion: 0.25
Nodes (7): Contract Files: 17 files, Entity Schemas: 4 files, Issues, Service — Subagent 3/3: Contracts/Entities Layer, Sub-routers: 4, Todo List (Contracts Layer ONLY), Total Operations: 10

### Community 617 - "./mdx"
Cohesion: 0.67
Nodes (3): ./mdx, import, types

### Community 618 - "./nextjs"
Cohesion: 0.67
Nodes (3): ./nextjs, import, types

### Community 619 - "files"
Cohesion: 0.67
Nodes (3): files, dist, src

### Community 624 - "Docker — API Layer (Subagent 1/3)"
Cohesion: 0.25
Nodes (7): Action Items, Docker — API Layer (Subagent 1/3), Domains (8), Known Issues, Module Structure, Repository Facade, SSE/Stream Infrastructure

### Community 625 - "🔍 Proposing Structural Changes"
Cohesion: 0.25
Nodes (8): 🔍 Proposing Structural Changes, The Principle, The Proposal Template, The "Show, Don't Tell" Rule, What Counts as "Structural", What Does NOT Count as Structural, When NOT to Stop and Propose, When to Stop and Propose

### Community 626 - "styling"
Cohesion: 0.48
Nodes (4): TraefikDynamicConfig, traefikDynamicConfigSchema, traefikRuleSchema, traefikServiceSchema

### Community 627 - "compatibility.ts"
Cohesion: 0.53
Nodes (3): cleanupOrphanedDeployerContainers(), isTruthyEnv(), logSetup()

### Community 628 - "post-setup-hints.tsx"
Cohesion: 0.14
Nodes (10): nextjs-toploader, geist, metadata, HintDef, HintResult, HINTS, PostSetupHints(), nextjs-toploader (+2 more)

### Community 633 - "Handoff: Sub-App Orchestration Refactor"
Cohesion: 0.29
Nodes (6): 14. Testing the Result, 1. Project Context, Appendix A: How NestFactory.create() Works, Appendix B: Diagram of New Architecture, Handoff: Sub-App Orchestration Refactor, Table of Contents

### Community 635 - "10. Implementation Plan"
Cohesion: 0.29
Nodes (7): 10. Implementation Plan, Step 1: Create OrchestratorService, Step 2: Create OrchestrationModule, Step 3: Simplify main.ts, Step 4: Remove shared-context files, Step 5: Roll back sub-app-cascade.service.ts, Step 6: Wire the result flow

### Community 636 - "nanoid"
Cohesion: 0.33
Nodes (6): linting, eslint, @eslint/eslintrc, @eslint/js, prettier, prettier-plugin-tailwindcss

### Community 638 - "1.4 Dead Web Frontend Files"
Cohesion: 0.29
Nodes (7): 1.4.1 Mock Hooks (9 files) — 🟢 SAFE DELETE, 1.4.2 Orphaned Domain Hooks (4 files) — 🟢 SAFE DELETE, 1.4.3 Dead Lib Files (15 files) — 🟢 SAFE DELETE, 1.4.4 Dead Auth/Permission Components — 🟢 SAFE DELETE, 1.4.5 Dead `components/auth/index.ts` — 🟢 SAFE DELETE, 1.4.6 Routes files — Check First, 1.4 Dead Web Frontend Files

### Community 639 - "Deployment — Contracts Layer (Subagent 3/3)"
Cohesion: 0.29
Nodes (6): Action Items, Contract Files, Deployment — Contracts Layer (Subagent 3/3), Entity Schemas, Gap Analysis, Operation Categories

### Community 641 - "Project — Complete Analysis Report (Subagents 1-3 combined)"
Cohesion: 0.29
Nodes (6): Action Items (8), API Layer, Contracts Layer, Known Issues, Project — Complete Analysis Report (Subagents 1-3 combined), Web Layer

### Community 642 - "Project — Subagent 3/3: Contracts/Entities Layer"
Cohesion: 0.29
Nodes (6): Contract Completeness, Contract Files: 58 files, Entity Schemas: 5 files, Project — Subagent 3/3: Contracts/Entities Layer, Todo List (Contracts Layer ONLY), Total Operations: 38

### Community 643 - "@octokit/rest"
Cohesion: 0.33
Nodes (5): listOrganizationDomainsContract, listOrganizationDomainsInput, organizationListOps, organizationDomainSchema, verificationStatusSchema

### Community 644 - "Service — Complete Analysis Report (Subagents 1-3 combined)"
Cohesion: 0.29
Nodes (6): Action Items (7), API Layer, Contracts Layer, Known Issues, Service — Complete Analysis Report (Subagents 1-3 combined), Web Layer

### Community 645 - "Docker — Web Layer (Subagent 2/3)"
Cohesion: 0.29
Nodes (6): Action Items, Components: 34 in `_components/`, Docker — Web Layer (Subagent 2/3), Domain Hooks: 73 total, Issues, Pages: 10 active + 3 redirects

### Community 648 - "🔴 Error Handling"
Cohesion: 0.29
Nodes (7): 3-Tier Error Pipeline, Client Error Handling, 🔴 Error Handling, Layer 1: AppError Hierarchy, Layer 2: ORPC Error Contract, Layer 3: Global Filter, Request Correlation

### Community 650 - "@repo/contracts-common"
Cohesion: 0.40
Nodes (4): AFTER_HOOK_KEY, AUTH_MODULE_OPTIONS_KEY, BEFORE_HOOK_KEY, HOOK_KEY

### Community 651 - "🔴 Type Safety — Zero Tolerance for Assertions"
Cohesion: 0.29
Nodes (7): Allowed Narrow Casts (and only these), Audit Before Committing, ❌ Banned Patterns, ✅ The Right Patterns, 🔴 Type Safety — Zero Tolerance for Assertions, TypeScript Compiler Settings (Already Enforced), When the Type System Fights You

### Community 652 - "🔴 React / Next.js Patterns"
Cohesion: 0.29
Nodes (7): App Router Conventions, Component Patterns, Declarative Routing, Feature-Local Structure (mandatory for non-trivial routes), Information Presentation, 🔴 React / Next.js Patterns, State Management

### Community 653 - "@scalar/nestjs-api-reference"
Cohesion: 0.53
Nodes (4): extractRoutesFromExpress(), getExpressStack(), getInstance(), NestAppWithHttpAdapter

### Community 654 - "🔴 Performance — SSE / High-Frequency Streams"
Cohesion: 0.29
Nodes (7): Audit Checklist for SSE/Stream Pages, Modal/Dialog Components, 🔴 Performance — SSE / High-Frequency Streams, React Compiler, Required Pattern for SSE-Driven Lists, Stable References for Heavy Children, The SSE Update Problem

### Community 655 - "@thallesp/nestjs-better-auth"
Cohesion: 0.50
Nodes (4): LocalAccountStep(), passwordStrength(), Props, strengthLabels

### Community 662 - "🔴 Pre-Work Investigation Protocol"
Cohesion: 0.29
Nodes (7): Check for Dead Code, Check If the Implementation Already Exists, Check the Related Code, Find All Call Sites, 🔴 Pre-Work Investigation Protocol, Read Relevant AGENTS.md First, When Implementing a New Concept, Check Everywhere

### Community 672 - "🔍 Deep Debugging Protocol"
Cohesion: 0.29
Nodes (7): 🔍 Deep Debugging Protocol, Forbidden Easy-Way Fixes, The 5 Whys, The "I Don't Know" Rule, The Investigation Order (Always Follow), The "Why Does This Work?" Check, When You Hit a Wall

### Community 673 - "2. Tech Stack & Tooling"
Cohesion: 0.33
Nodes (6): 2. Tech Stack & Tooling, Available Type Declarations (read these), Core, Important Library Versions, Key Findings for Implementation, Key NestJS Internals (read before implementing)

### Community 674 - "7. The New Architecture"
Cohesion: 0.33
Nodes (6): 7.1 Core Principle, 7.2 What "Orchestrator IS the Bridge" Means, 7.3 Sub-App Communication, 7.4 The Orchestrator Pipeline, 7.5 Key Simplifications, 7. The New Architecture

### Community 675 - "index.mdx"
Cohesion: 0.33
Nodes (5): Documentation principles, Full navigation, Readiness checklist, Recommended learning flow, Start with what matters

### Community 676 - "page.tsx"
Cohesion: 0.40
Nodes (3): deriveNodeCoordinates(), hashNodeId(), MAP_ANCHOR

### Community 677 - "ClientAuthProviders.tsx"
Cohesion: 0.40
Nodes (3): ClientAuthProviders(), NOTE: We do NOT fetch session here - that would defeat lazy loading!, configureClientAuth()

### Community 678 - "🚩 Category 4: Technical Debt & Anti-Patterns"
Cohesion: 0.33
Nodes (6): 4.1 TODO/FIXME/HACK Residue (14 instances), 4.2 `console.log` Bypassing Logger (40+ occurrences), 4.3 `process.env` Scattered (30+ occurrences), 4.4 Hardcoded `href` Bypassing Declarative Routing (4 occurrences), 4.5 Direct `fetch()` Bypassing ORPC (4 occurrences), 🚩 Category 4: Technical Debt & Anti-Patterns

### Community 690 - "Scoped Docker Architecture — Overview"
Cohesion: 0.33
Nodes (5): Documentation Index, Key Changes, Scoped Docker Architecture — Overview, The Problem, The Solution: Scoped Architecture

### Community 691 - "1. 💀 Dead Core Modules (Superseded)"
Cohesion: 0.33
Nodes (6): 1.1 State Machine (💀 DEAD — 4 files), 1.2 Sub-App Orchestrator (💀 DEAD — 2 files), 1.3 Sub-App Runner (💀 DEAD — 3 files), 1.4 Loader (💀 DEAD — 3 files), 1.5 Gateway (💀 DEAD — 3 files), 1. 💀 Dead Core Modules (Superseded)

### Community 694 - "5. 💀 Dead Web Frontend Files"
Cohesion: 0.33
Nodes (6): 5.1 Orphaned Domain Hooks (💀 DEAD — 6 files), 5.2 Unused Auth Components (💀 DEAD — 2 files), 5.3 Mock Hooks (💀 DEAD — 9 files), 5.4 Unused Lib Files (💀 DEAD — 15+ files), 5.5 Unused Lib Errors (💀 DEAD — 6 files), 5. 💀 Dead Web Frontend Files

### Community 696 - "Deployment — Web Layer (Subagent 2/3)"
Cohesion: 0.33
Nodes (5): Action Items (9), Deployment — Web Layer (Subagent 2/3), Domain Hooks (9 total), Issues Found, Pages

### Community 698 - "🔴 Dependency Inversion (DI Tokens)"
Cohesion: 0.33
Nodes (6): Anti-Patterns, 🔴 Dependency Inversion (DI Tokens), Examples, NestJS Wiring, Principles, When to Use DI Tokens

### Community 699 - "🔴 Engineering Mindset & Operating Posture"
Cohesion: 0.33
Nodes (6): Ask When Uncertain, Development Project, Not Production, 🔴 Engineering Mindset & Operating Posture, Goal: Finish the Work the User *Intended*, Not Just the Work They Asked For, Never Silently Degrade, No Budget

### Community 701 - "🔴 ORPC Contract Patterns"
Cohesion: 0.33
Nodes (6): Client Consumption, Contract Composition, Contract Definition, Endpoints Layout, 🔴 ORPC Contract Patterns, Server Implementation

### Community 702 - "🔴 Testing"
Cohesion: 0.33
Nodes (6): Coverage, Frameworks, Mocking Patterns, Per-App Setup Files, Test Location Conventions, 🔴 Testing

### Community 703 - "🔴 Logging"
Cohesion: 0.33
Nodes (6): Filter Syntax, 🔴 Logging, Per-Call Class Identity, Use `ContextFilterLogger` Everywhere, What NOT to Log, What to Log Where

### Community 704 - "🔴 Refactor Incrementalism & Atomic Refactors"
Cohesion: 0.33
Nodes (6): Incrementalism Done Right, 🔴 Refactor Incrementalism & Atomic Refactors, The Atomic Refactor, The "Migration Commit" Anti-Pattern, The Right Pattern, When You CANNOT Do It Atomically

### Community 705 - "11. Files to Create, Modify, Delete"
Cohesion: 0.40
Nodes (5): 11.1 Create, 11.2 Modify, 11.3 Delete, 11.4 Keep (no changes), 11. Files to Create, Modify, Delete

### Community 706 - "12. Rules & Constraints"
Cohesion: 0.40
Nodes (5): 12.1 Architecture Rules, 12.2 Coding Rules (from AGENTS.md), 12.3 NestJS Rules, 12.4 Implementation Rules, 12. Rules & Constraints

### Community 707 - "13. AppModule Dependencies (Critical)"
Cohesion: 0.40
Nodes (5): 13. AppModule Dependencies (Critical), Core Modules, Feature Modules (all need Database + Auth), Foundation Modules, Key Insight for the Orchestrator

### Community 710 - "9. Sub-App Definitions (What to Register)"
Cohesion: 0.40
Nodes (5): 9.1 db-resolver (Phase 0 replacement), 9.2 setup-wizard (if DB not configured), 9.3 mesh-initializer, 9.4 main-app (AppModule — last), 9. Sub-App Definitions (What to Register)

### Community 712 - "slug.utils.ts"
Cohesion: 0.40
Nodes (3): Slug, SlugifyOptions, slugSchema

### Community 713 - "🚩 Category 6: Orphaned Apps & Packages"
Cohesion: 0.40
Nodes (5): 6.1 `apps/test/` — Completely Unintegrated, 6.2 `apps/observable-poc/` — Standalone POC, 6.3 `packages/poc/core-sync-system/` — Standalone POC Package, 6.4 Load Balancer Status, 🚩 Category 6: Orphaned Apps & Packages

### Community 714 - "💡 Enhancement Recommendations"
Cohesion: 0.40
Nodes (5): Architectural Enhancement: Connect Everything Better, 💡 Enhancement Recommendations, P0: High Impact / Low Effort, P1: Medium Impact / Medium Effort, P2: Lower Impact / Higher Effort

### Community 715 - "6. ❌ Not Working — Infrastructure Gaps"
Cohesion: 0.40
Nodes (5): 6.1 Context Module (❌ NOT WORKING), 6.2 Ephemeral HTTP Module (❌ NOT WORKING), 6.3 Git Module (❌ NOT WORKING), 6.4 Triggers Module (❌ NOT WORKING), 6. ❌ Not Working — Infrastructure Gaps

### Community 718 - "7. ❌ Not Working — Architectural Gaps"
Cohesion: 0.40
Nodes (5): 7.1 GitHub Module — No ORPC Contract, 7.2 Permission Module — No HTTP Exposure, 7.3 Web: Direct `fetch()` Calls Bypassing ORPC (4 files), 7.4 Web: Hardcoded `href` Bypassing Declarative Routing (2 files), 7. ❌ Not Working — Architectural Gaps

### Community 719 - "8. 📋 Planned / Blocked Features"
Cohesion: 0.40
Nodes (5): 8.1 Domain Verification via Cron (TODO), 8.2 Variable Resolver Module (TODO), 8.3 FK-Chain Subquery for Permissions (TODO), 8.4 ORPC Middleware DI (TODO), 8. 📋 Planned / Blocked Features

### Community 721 - "🔴 Type-Driven Development — Make Illegal States Unrepresentable"
Cohesion: 0.40
Nodes (5): Anti-Patterns, Examples, In This Monorepo, Principles, 🔴 Type-Driven Development — Make Illegal States Unrepresentable

### Community 729 - "🔴 Monorepo Package Boundaries"
Cohesion: 0.40
Nodes (5): Anti-Patterns, 🔴 Monorepo Package Boundaries, Package Boundary Rules, The Packages That Already Exist (Know These Before Adding a New One), When to Create a Package

### Community 731 - "🔴 No Bridges, No Compatibility Layers"
Cohesion: 0.40
Nodes (5): Anti-Patterns, 🔴 No Bridges, No Compatibility Layers, The Exception (Rare), The Rule, This Applies To

### Community 732 - "4. What the App Does (Domain)"
Cohesion: 0.50
Nodes (4): 4. What the App Does (Domain), Core Domains (in rough dependency order), Feature Modules (imported by AppModule), The Critical Dependency Chain

### Community 733 - "6. The Problem"
Cohesion: 0.50
Nodes (4): 6.1 What's Wrong, 6.2 The Root Cause, 6.3 What "Over-Engineered" Means Specifically, 6. The Problem

### Community 745 - "entrypoint.prod.ts"
Cohesion: 0.83
Nodes (3): main(), startNext(), validateEnvironment()

### Community 763 - "🚩 Category 3: Type Safety Violations"
Cohesion: 0.50
Nodes (4): 3.1 Banned: `as unknown as` (80+ occurrences), 3.2 Banned: `as any` (35+ occurrences), 3.3 Unresolved Import, 🚩 Category 3: Type Safety Violations

### Community 767 - "🚩 Category 5: Unused Dependencies (Production Mode)"
Cohesion: 0.50
Nodes (4): 5.1 `@repo/type-guards` Listed but Not Used, 5.2 Dead Dependencies in `packages/ui/base`, 5.3 Potentially Dead Major Deps, 🚩 Category 5: Unused Dependencies (Production Mode)

### Community 768 - "index.ts"
Cohesion: 0.40
Nodes (5): resetSharedRuntimeRequestOverride(), setSharedRuntimeAuthHeaders(), setSharedRuntimeRequestOverride(), sharedRequestOverride, SharedRuntimeRequestOverride

### Community 770 - "Appendices"
Cohesion: 0.50
Nodes (4): A: Knip Output Summary, Appendices, B: Key File for Tracking, C: Related Configuration

### Community 771 - "🔴 Ripple Effect Analysis"
Cohesion: 0.50
Nodes (4): After Implementing, Before Implementing, 🔴 Ripple Effect Analysis, The "Check Everywhere Related" Discipline

### Community 772 - "🔴 DRY — Don't Repeat Yourself"
Cohesion: 0.50
Nodes (4): Anti-Patterns, DRY at Every Layer in This Monorepo, 🔴 DRY — Don't Repeat Yourself, The Three Strikes Rule

### Community 773 - "🔴 YAGNI — You Aren't Gonna Need It"
Cohesion: 0.50
Nodes (4): Anti-Patterns, Principles, The "Just In Case" Trap, 🔴 YAGNI — You Aren't Gonna Need It

### Community 774 - "🔴 Separation of Concerns"
Cohesion: 0.50
Nodes (4): Anti-Patterns, Layered Architecture, 🔴 Separation of Concerns, The Rule of "One Reason to Change"

### Community 775 - "🔴 Error Handling Philosophy"
Cohesion: 0.50
Nodes (4): Anti-Patterns, 🔴 Error Handling Philosophy, Principles, When to Throw vs Return

### Community 776 - "🔴 The Boy Scout Rule"
Cohesion: 0.50
Nodes (4): Anti-Patterns, Application, Scope Discipline, 🔴 The Boy Scout Rule

### Community 777 - "🔴 "Scream" Test for Architecture"
Cohesion: 0.50
Nodes (4): Anti-Patterns, Application, 🔴 "Scream" Test for Architecture, Screaming Architecture Principles

### Community 778 - "🔴 Git Workflow"
Cohesion: 0.50
Nodes (4): Branch Naming, Commit Policy (from root `AGENTS.md`), 🔴 Git Workflow, Safety Boundaries

### Community 779 - "🔴 Composition & Open/Closed Principle"
Cohesion: 0.50
Nodes (4): 🔴 Composition & Open/Closed Principle, Composition Over Inheritance, Open/Closed Principle, Patterns to Know (and When to Use Them)

### Community 780 - "🔴 Documentation Awareness"
Cohesion: 0.50
Nodes (4): 🔴 Documentation Awareness, Documentation Discovery Pattern, Scoped AGENTS.md Files (Read Before Touching Code), When to Read Documentation

### Community 781 - "🔴 MCP-First Workflow"
Cohesion: 0.50
Nodes (4): Error Recovery Protocol, Local vs Docker Execution, Mandatory MCP Tools by Area, 🔴 MCP-First Workflow

### Community 782 - "🔴 Zod-as-Source-of-Truth for All External Data"
Cohesion: 0.50
Nodes (4): ORPC Stream Validation — Critical Rule, Trust Boundaries That MUST Validate, 🔴 Zod-as-Source-of-Truth for All External Data, Zod Schema Conventions

### Community 783 - "AccessDeniedError"
Cohesion: 0.11
Nodes (16): AccessControlResult, AccessControlUser, AccessDeniedError, allowAllPermissions(), allowAllRoles(), allowAnyPermission(), allowOwnerOrRoles(), allowPermission() (+8 more)

### Community 784 - "build"
Cohesion: 0.50
Nodes (4): dependsOn, inputs, outputs, build

### Community 785 - "compile"
Cohesion: 0.50
Nodes (4): dependsOn, inputs, outputs, compile

### Community 786 - "start"
Cohesion: 0.50
Nodes (4): cache, dependsOn, persistent, start

### Community 787 - "test:coverage"
Cohesion: 0.50
Nodes (4): test:coverage, dependsOn, inputs, outputs

### Community 788 - "test:ui"
Cohesion: 0.50
Nodes (4): test:ui, cache, dependsOn, persistent

### Community 789 - "test:watch"
Cohesion: 0.50
Nodes (4): test:watch, cache, dependsOn, persistent

### Community 790 - "dev"
Cohesion: 0.67
Nodes (3): cache, persistent, dev

### Community 791 - "docker:build"
Cohesion: 0.67
Nodes (3): cache, dependsOn, docker:build

### Community 792 - "docker:dev"
Cohesion: 0.67
Nodes (3): cache, persistent, docker:dev

### Community 793 - "generate:directus:types"
Cohesion: 0.67
Nodes (3): cache, dependsOn, generate:directus:types

### Community 798 - "@repo/logger"
Cohesion: 0.40
Nodes (5): doc, fumadocs-core, fumadocs-mdx, fumadocs-ui, @types/mdx

### Community 802 - "zod"
Cohesion: 0.40
Nodes (5): Alert(), AlertAction(), AlertDescription(), AlertTitle(), alertVariants

### Community 861 - "index.ts"
Cohesion: 0.40
Nodes (5): Tabs(), TabsContent(), TabsList(), tabsListVariants, TabsTrigger()

### Community 863 - "index.ts"
Cohesion: 0.40
Nodes (4): fix_file(), Fix MDX math in doc content files. - Replace $$...$$ blocks with ```latex ... ``, Returns True if file was modified., Path

## Knowledge Gaps
- **6421 isolated node(s):** `TriggerPayload`, `ColumnDefWithMeta`, `DataFetchParams`, `DataFetchResult`, `PaginationUpdater` (+6416 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **290 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `vitest` connect `vitest` to `test.controller.ts`, `HTTP/Traefik Router Builder`, `Permission & Role Checks`, `Docker Container Inspection`, `Docker Runtime Activity Projection`, `SystemMeshTopologyService`, `AuthCoreService`, `platform-domain.builder.ts`, `mesh-connection-registry.ts`, `service-dag.service.ts`, `DockerRuntimeRunnerService`, `DeploymentTriggerInput`, `query-builder.ts`, `stream.ts`, `docker.repository.ts`, `docker-entity-domain.service.ts`, `BaseEventService`, `middlewares.ts`, `index.ts`, `build-tools.ts`, `node-startup-check.command.ts`, `permission-engine.ts`, `deployment-queue-lifecycle.service.ts`, `mesh-query-builder-types.ts`, `index.ts`, `InitializationService`, `index.ts`, `resource-graph.ts`, `github.module.ts`, `DeploymentMeshService`, `deployment.service.spec.ts`, `index.ts`, `index.ts`, `route-builder.ts`, `deployment.ts`, `AnalyticsService`, `contracts.ts`, `index.ts`, `AppLifecycleService`, `user.repository.ts`, `index.ts`, `PushRepository`, `deployment-provider-builder-runner-state-machine.service.ts`, `ServiceService`, `compatibility.ts`, `index.ts`, `isRecord`, `InternalErrorInsightService`, `index.ts`, `DockerRuntimeEvent`, `generate-hooks.ts`, `shared-api-runtime.ts`, `organization.repository.ts`, `traefik-config-builder.ts`, `DockerContainerRuntimeService`, `index.ts`, `index.ts`, `configuration-core.module.ts`, `utils.ts`, `package.json`, `page.tsx`, `index.ts`, `TraefikConfigBuilder`, `index.ts`, `ApiKeyRepository`, `project.module.ts`, `types.ts`, `DeploymentQueueProcessor`, `mesh-filter.ts`, `index.ts`, `ProviderSchemaService`, `utils.test.ts`, `SystemMeshConfigService`, `calendar-date-picker.tsx`, `mesh-subscription-manager.ts`, `index.ts`, `mesh-doc-compliance-matrix.e2e-spec.ts`, `list.ts`, `load-balancer.service.ts`, `auth-core.service.ts`, `configuration-resolver.service.ts`, `rule-validator.ts`, `template.schema.exhaustive.test.ts`, `devDependencies`, `development-session.ts`, `LocalEventOutboxDispatcherService`, `load-balancer.controller.ts`, `mesh-observable-types.ts`, `fleet-rollback.service.ts`, `package.json`, `drift-reconciliation.service.ts`, `fleet-failure-containment.service.ts`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `isRecord()` connect `DockerRuntimeEvent` to `TraefikSyncService`, `Permission & Role Checks`, `Docker Container Inspection`, `Mesh Core & Orchestration`, `SystemMeshConfigService`, `generate-hooks.ts`, `Docker Runtime Activity Projection`, `Mesh Cluster Repository`, `Deployment Pipeline Service`, `page.tsx`, `toolbar.tsx`, `DockerContainerRuntimeService`, `mesh-connection-registry.ts`, `DeploymentTriggerInput`, `docker.repository.ts`, `docker-entity-domain.service.ts`, `index.ts`, `docker-containers-orchestrator.service.ts`, `ProjectService`, `index.ts`, `DeploymentRepository`, `index.ts`, `VariableRegistry`, `page.tsx`, `index.ts`, `core-event-sync.service.ts`, `route-method-meta.ts`, `page.tsx`, `docker-container-runtime-mesh.service.ts`, `index.ts`, `DockerRepository`, `InitializationService`, `isObjectLike`, `index.ts`, `hooks.ts`, `TraefikFileSystemService`, `github.module.ts`, `rule-validator.ts`, `types.ts`, `MeshControlEnvelope`, `standard-schema-helpers.ts`, `makeRoute.tsx`, `index.ts`, `deployment.ts`, `MeshTrustStrictModeService`, `index.ts`, `index.ts`, `.error`, `hooks.ts`, `index.ts`, `ServiceService`, `drizzle-filter.utils.ts`, `InternalErrorInsightService`, `runtime-configuration-accessor.ts`, `app-error.ts`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `GlobalDatabaseService` connect `GlobalDatabaseService` to `TypeScript Type Utilities`, `manager.ts`, `sorting.ts`, `Mesh Core & Orchestration`, `platform-domain.schema.ts`, `shared-api-runtime.ts`, `index.ts`, `auth.ts`, `LocalEventOutboxDispatcherService`, `organization.repository.ts`, `index.ts`, `docker.repository.ts`, `domain.ts`, `node-startup-check.command.ts`, `LocalDatabaseService`, `ServiceDomainMappingRepository`, `traefik.module.ts`, `common.ts`, `ApiKeyRepository`, `project.module.ts`, `deployment.ts`, `AppLifecycleService`, `permission.repository.ts`, `.error`, `error`, `user.repository.ts`, `PushRepository`, `ServiceService`, `index.ts`, `CliAuthService`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **What connects `TriggerPayload`, `ColumnDefWithMeta`, `DataFetchParams` to the rest of the system?**
  _6462 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `TypeScript Type Utilities` be split into smaller, more focused modules?**
  _Cohesion score 0.054426705370101594 - nodes in this community are weakly interconnected._
- **Should `Mesh Networking Contracts` be split into smaller, more focused modules?**
  _Cohesion score 0.01451559934318555 - nodes in this community are weakly interconnected._
- **Should `Deployment Domain Schemas` be split into smaller, more focused modules?**
  _Cohesion score 0.011560693641618497 - nodes in this community are weakly interconnected._