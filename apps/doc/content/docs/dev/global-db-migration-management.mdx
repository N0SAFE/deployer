...
description: Multi-node safe schema migration protocol with global version registry, node registration guard, uniform version enforcement, and auto-update for the shared PostgreSQL database.
order: 8
---
...
1. **Migration manifest as contract** — Every migration file declares minimum app/schema version requirements. The manifest is the single source of truth.
2. **Registration guard** — A node can only join the mesh if its code contains ALL migrations already applied to the global DB. This prevents outdated code from operating on an incompatible schema.
3. **Minimum version check** — Migrations only proceed when ALL active nodes have the minimum app version required by that migration's manifest entry.
4. **Uniform version enforcement** — All nodes in the mesh must run compatible versions. Mismatches are detected, reported, and surfaced via health checks.
5. **Global version registry** — The `cluster_migration_state` table tracks cluster-wide schema version, required app version, and consensus node IDs.
6. **Auto-update capability** — Nodes can detect newer versions and self-update via Docker image pull + restart or git pull + rebuild.

### The Big Picture

```mermaid
flowchart TB
    subgraph "Central Registry (Global DB)"
        PG[("PostgreSQL<br/>Global DB")]
        SV[("schema_version<br/>per node")]
        CMS[("cluster_migration_state<br/>singleton")]
    end

    subgraph "Code-Based Registry"
        MANIFEST[("migration-manifest.json<br/>declares requirements")]
    end

    subgraph "Node A (v2.1.0)"
        A1[Startup Protocol]
        A2[Registration Guard ✅]
        A3[Migration Coordinator]
        A4[Schema: 0011]
    end

    subgraph "Node B (v2.1.0)"
        B1[Startup Protocol]
        B2[Registration Guard ✅]
        B3[Migration Coordinator]
        B4[Schema: 0011]
    end

    subgraph "Node C (v1.5.0 - outdated)"
        C1[Startup Protocol]
    ...
    A1 <-->|Mesh WebSocket| B1
    A1 <-.->|Mesh WebSocket| C1
    B1 <-.->|Mesh WebSocket| C1
...
## Migration Manifest — The Code-Based Registry

The manifest is a JSON file at `apps/api/src/config/drizzle/global/migrations/migration-manifest.json`. It is the **single source of truth** for migration requirements. Every migration file has a corresponding manifest entry declaring its minimum app version and minimum schema version prerequisites.

```json
{
  "$schema": "./migration-manifest.schema.json",
  "currentSchemaVersion": "0011",
  "migrations": [
    {
      "version": "0000",
      "name": "brave_steve_rogers",
      "minAppVersion": ">=1.0.0",
    ...
    {
      "version": "0001",
      "name": "wealthy_maelstrom",
      "minAppVersion": ">=1.0.0",
      "minSchemaVersion": "0000",
      "description": "Passkey table, column defaults"
    },
    {
      "version": "0002",
      "name": "whole_sumo",
...
The registration guard is the centerpiece of multi-node safety. Here is its complete decision tree:

```mermaid
flowchart TD
    START([Registration Guard Start]) --> A[Read __drizzle_migrations<br/>from global DB]
    A --> B[Read migration-manifest.json<br/>from this node's code]
    B --> C{manifest valid?}
    
    C -->|Missing or invalid| D[❌ EXIT 5<br/>MANIFEST_MISSING_OR_INVALID]
    C -->|Valid| E[Build set of DB-applied versions]
    
    E --> F[Build set of code-available versions]
    F --> G{Every DB version<br/>exists in code?}
    
    G -->|YES| H{manifest.currentSchemaVersion<br/>>= maxDbVersion?}
    G -->|NO - missing in code| I[❌ EXIT 5: NODE_TOO_OLD]
    
    I --> J[Log detailed error with missing versions]
    J --> K[Return: REGISTRATION_DENIED]
    
    H -->|NO - manifest outdated| L[❌ EXIT 5: MANIFEST_OUTDATED]
    H -->|YES| M[✅ Guard PASSED]
    
    M --> N[Proceed to Phase 8: Peer Check]
...
The `migration-manifest.json` defines the exact compatibility matrix:

| App Version | Compatible Schema | Can Apply Migrations Up To | Registration Guard Result |
|-------------|-------------------|----------------------------|---------------------------|
| `1.0.x` | 0000-0004 | 0004 | ✅ If cluster ≤ 0004 |
