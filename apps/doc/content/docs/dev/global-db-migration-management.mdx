...
| **Error cases** | MANIFEST_MISSING, MANIFEST_OUTDATED | Only JOURNAL_MISSING — can't be outdated |
    {
      "version": "0002",
      "name": "whole_sumo",
      "minAppVersion": ">=1.0.0",
    ...
    },
    {
      ...
      "description": "schema_version, cluster_migration_state, migration journal, registration guard"
    }
  ]
}
```
...
The `_journal.json` is auto-generated by Drizzle Kit and has a well-known shape. The `MigrationJournalService` reads and caches it at startup:

```typescript
// apps/api/src/core/utils/migration-journal.service.ts
...
|------|-------------|-----|
| Every migration must have a journal entry | Drizzle auto-generates it | Without the entry, `MigrationJournalService` can't determine which migrations exist |
| Journal must be committed to version control | CI check | The journal is the source of truth for what migrations exist in this code version |
| Migration files must not be modified after creation | Code review | Drizzle uses the `tag` field to track what's been applied; modifying a migration would break reproducibility |
| Journal is read at every startup and registration | Runtime | Ensures the node's migration set matches what the cluster expects |
...
```mermaid
flowchart TD
    START([Node starts]) --> A[Phase 1: Read Drizzle _journal.json]
    ...
    G -->|Journal entry doesn't match DB version| G2[❌ Exit 5: NODE_TOO_OLD]
...
#### Phase 1: Read Drizzle _journal.json

Read the auto-generated  from disk via . This tells the node:
- Which migrations exist in this code version (from )
- What the latest migration index is (from )

**Failure handling:** If the  is missing, malformed, or fails Zod validation:
- Log error: `❌ Migration journal missing or corrupted at {path}.`
...
type RegistrationGuardResult =
  ...
  | { allowed: false; reason: 'JOURNAL_MISSING_OR_INVALID'; error: string }

async function registrationGuard(
  db: GlobalDatabase,
  journalService: MigrationJournalService
  ...
  // 2. Read the node's own migration versions from the journal
  const journal = journalService.load()
  const myTags = new Set(journal.entries.map(e => e.tag))

  // 3. Check: every DB-applied migration must exist in my code
  const missingFromCode = [...dbVersions].filter(v => !myTags.has(v))
...
| `_journal.json` has syntax error | JSON parse or Zod validation fails | ❌ **Block**. Log: "Journal corrupted" |
...
| `NODE_TOO_OLD` | Node code is missing migrations already applied to DB. | Deploy newer app version. |
| `JOURNAL_MISSING_OR_INVALID` | Drizzle _journal.json missing or corrupted at {migrationsPath}/meta/_journal.json | Ensure Drizzle Kit has generated the journal file. |
...
sequenceDiagram
    ...
    Node->>Node: Read Drizzle _journal.json (now includes 0012)
...
### Scenario H: Journal Is Outdated

```
Cluster: Schema 0011
Developer adds migration 0012 (Drizzle auto-generates _journal.json with entry for 0012)
```

1. Phase 4: Drizzle applies 0012. `__drizzle_migrations` now has up to 0012.
2. Phase 7 (Registration Guard):
   - DB has: 0000-0012
   - Code\'s `_journal.json` has: entries for 0000-0012 (auto-generated by Drizzle)
   - They always match because Drizzle generates both!
3. ✅ Guard passes automatically — no manual manifest to keep in sync.

**Result:** The journal is always in sync with migration files because Drizzle Kit generates both simultaneously. No manual update required.

### Scenario I: Registry Guard With Missing Journal

```
Node starts without _journal.json (deleted, never created, or volume mount issue)
```

1. Phase 1: `MigrationJournalService.load()` fails — file not found
2. ❌ **`JOURNAL_MISSING_OR_INVALID`** — Exit code 5
3. Log: "Migration journal missing or corrupted. Cannot verify schema compatibility."
4. Node never reaches the DB.

**Result:** Registration blocked. Node can't join until the journal is restored (re-run `drizzle-kit generate`).
...
| **5** | **Registration DENIED** | Guard blocked the node | `❌ NODE_TOO_OLD` or `❌ NODE_BEHIND_CLUSTER` |
...
### Phase 1: Schema Tables + Migration Journal (Migration 0011)
...
- [ ] Drizzle Kit auto-generates `meta/_journal.json` with `drizzle-kit generate`
...
| **Migration journal** | Drizzle auto-generated `meta/_journal.json` tracking all migration files with their tags |
