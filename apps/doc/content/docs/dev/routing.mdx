...
1. Each route page owns a `route.info.ts` descriptor.
...
2. Define/update matching `route.info.ts` routing descriptor (name/params/search); per-page SEO metadata lives in `page.tsx` `export const metadata`/`generateMetadata`.
...
- Ensure `route.info.ts` exists in the target route segment.
