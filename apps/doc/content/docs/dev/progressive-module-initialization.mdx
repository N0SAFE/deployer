...
```mermaid
flowchart TB
    CREATE["NestFactory.create(AppModule)<br/>all sync providers created"] --> POOL["GLOBAL_DATABASE_POOL (async factory)<br/>checkConfigAndEmit() → reads local SQLite cache"]

    POOL -->|"cached URL exists"| M1["try mesh first for latest"]
    M1 -->|"mesh responds with URL"| OK1["use mesh URL, update cache<br/>→ emitCompleted()<br/>→ waitForSetup() resolves<br/>→ new Pool(connectionString) ✅ POOL CREATED"]
    M1 -->|"mesh can't resolve ¹"| OK2["use cached URL as fallback<br/>→ emitCompleted()<br/>→ waitForSetup() resolves<br/>→ new Pool(connectionString) ✅ POOL CREATED"]

    POOL -->|"no cached URL"| M2["try mesh first"]
    M2 -->|"mesh responds with URL"| OK3["use mesh URL, persist as cache<br/>→ emitCompleted() → Pool created ✅"]
    M2 -->|"mesh can't resolve ¹"| BLOCK1["does NOT emit<br/>→ waitForSetup() BLOCKS<br/>→ NO POOL — needs setup wizard"]

    POOL -->|"DEV_AUTO_SETUP=true"| DEV["dev auto-setup branch"]
    DEV -->|"with DATABASE_URL env"| OK4["generate config, persist<br/>→ emitCompleted() → Pool created ✅"]
    DEV -->|"without DATABASE_URL"| WIZ1["log, no emit<br/>→ waitForSetup() BLOCKS<br/>→ SetupWizard sub-app auto-triggers<br/>→ LocalInitializationService.initialize()<br/>→ provision Postgres container (dockerode)<br/>→ run migrations → seed data<br/>→ emitCompleted() → Pool created"]

    POOL -->|"setup wizard — HTTP POST /api/setup/initialize"| REMOTE["RemoteInitializationService.initialize():<br/>mesh handshake — issue grant + bootstrap<br/>mesh provides DB URL<br/>cache URL in local SQLite<br/>emitCompleted() → waitForSetup() resolves<br/>Pool created"]

    NOTE["¹ Mesh can't resolve when multiple nodes share the same state,<br/>or the mesh contains only one node. The local SQLite cache stores<br/>the last-known-good URL as fallback."]
    style NOTE fill:transparent,stroke:none,color:#888
```
...
```mermaid
flowchart TD
    INIT["onModuleInit() fires on all modules"] --> P0["Phase 0: Bootstrap<br/>read journal, local config, log version<br/>(local SQLite + filesystem only)"]
    P0 --> P1["Phase 1: Update marker<br/>check /tmp/pending-update.json"]
    P1 --> P2["Phase 2: Env validation<br/>validateApiEnvSafe"]
    P0 & P1 & P2 -.->|"do NOT need global DB"| SQLITE[("local SQLite")]
    P2 --> P3["Phase 3: DB connectivity — SELECT 1<br/>⬅ FIRST USE OF POOL"]
    P3 --> P4["Phase 4: Run Drizzle migrations"]
    P4 --> P5["Phase 5: Read state tables<br/>__drizzle_migrations · cluster_migration_state"]
    P5 --> P6["Phase 6: Registration guard 🛡️"]
    P6 --> P7["Phase 7: Peer version check + cluster consensus"]
    P7 --> P8["Phase 8: Register this node in the mesh"]
    P8 --> P9["Phase 9: Report schema version"]
    P9 --> P10["Phase 10: Create default admin user"]
    P10 --> P11["Phase 11: Start periodic tasks"]
    P11 --> P12["Phase 12: Signal readiness<br/>→ isStartupComplete() = true<br/>→ GLOBAL DB IS NOW AVAILABLE<br/>→ Feature modules start serving"]
    style P3 stroke:#f59e0b,stroke-width:2px
    style P12 stroke:#10b981,stroke-width:2px
```
