...
<DocCallout title="Key Rule" tone="warning">
A module that requires data or a service that is not finished should **NOT start at all**. Do NOT use null providers to hide the problem — use proper dependency waiting patterns.
</DocCallout>
...
```mermaid
flowchart LR
    P0["Phase 0: HTTP Server (raw Express)<br/>→ /health, CORS"] --> P1["Phase 1: Setup Wizard<br/>→ LocalDatabaseModule + SetupWizardApiModule"]
    P1 --> P2["Phase 2: Mesh Initializer<br/>→ MeshInitializationModule"]
    P2 --> P3["Phase 3: Global Database<br/>→ GlobalDatabaseModule (real connection)"]
    P3 --> P4["Phase 4: Auth<br/>→ AuthModule"]
    P4 --> P5["Phase 5: Feature Modules<br/>→ All domain modules"]
    P5 --> P6["Phase 6: READY<br/>→ Full functionality"]
```
...
```mermaid
flowchart LR
    SETUP["SetupWizardSubApp"] --> MESH["MeshInitializerSubApp"] --> GDB["GlobalDatabase"] --> AUTH["Auth"] --> FEAT["Features"]

    N1["needs:<br/>LocalDatabase, Env"] -.-> SETUP
    N2["needs:<br/>SetupWizard result,<br/>MeshInitModule"] -.-> MESH
    N3["needs:<br/>MeshInit result"] -.-> GDB
    N4["needs:<br/>DB, Auth"] -.-> AUTH
    N5["needs:<br/>DB, Auth"] -.-> FEAT
```
...
```mermaid
flowchart TB
    MAIN["main.ts"] --> BOOT["BootstrapOrchestratorService<br/>(OnApplicationBootstrap)"]
    MAIN --> FML["FeatureModuleLoader<br/>(OnApplicationBootstrap)"]
    MAIN --> AML["AuthModuleLoader<br/>(OnApplicationBootstrap)"]

    BOOT --> B1["Phase 1: Setup Wizard sub-app"]
    BOOT --> B2["Phase 2: Mesh Initializer sub-app"]
    BOOT --> B3["Phase 3: GlobalDatabaseService.initialize()"]
    BOOT --> B4["Phase 3: DatabaseReadyTrigger.emit(initialized, url)"]

    FML --> F1["injects: DatabaseReadyResult<br/>← DI resolves via bridge.waitFor()"]
    F1 --> F2["Phase 5: LazyModuleLoader →<br/>HealthModule, UserModule, ..."]

    AML --> A1["injects: DatabaseReadyResult<br/>← DI resolves via bridge.waitFor()"]
    A1 --> A2["Phase 4: LazyModuleLoader → AuthModule"]
```
