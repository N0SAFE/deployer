...
description: How the custom load balancer works — request lifecycle, cross-node routing, diagnostic routes, shared secrets, and port conventions across single and multi-instance deployments.
order: 5
---

## Overview

Every node in the cluster runs a **full local stack**: Traefik, the custom LB app, the NestJS API, and the Next.js Web. There is no single shared load balancer.
...
**Key principle**: Traefik handles intra-node routing only (it knows nothing about other nodes). The custom LB app is the cross-node brain — it redirects requests to the correct node when a service lives elsewhere, and load-balances across replicas.
