...
- ✅ Contracts moved to `packages/contracts/api/modules/nodes` and wired into the `nodes` contract group (the mesh is the single tenant — no org concept).
- ✅ API fleet module (originally `apps/api/src/system/modules/fleet`) reworked as the node-scoped fleet surface under `apps/api/src/modules/fleet`.
- ✅ Web domain layer added under `apps/web/src/domains/fleet`.
- ✅ Fleet/allocations surfaced in `apps/web/src/app/dashboard/nodes` (fleet view) and `apps/web/src/app/dashboard/configuration` (per-node capacity cards).
