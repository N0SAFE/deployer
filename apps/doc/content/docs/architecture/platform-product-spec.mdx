...
Platform super admin
 └── Cluster (1+ nodes)
      └── Organization (configurable instance allocation)
           └── Project (logical grouping — independent repos)
                └── Service (web / worker / cron / one-shot)
                     └── User (custom role/permission matrix)
```
...
- **Design**: Redirect-based (307) + signed forward token — NOT a proxy
- **Score formula**: `0.4*(1-cpu) + 0.2*(1-memory) + 0.15*(1-streamPressure) + 0.15*(1-queuePressure) + 0.1*(1-errorRate) + ownerBonus(0.2) + priorityBonus`
- **Gossip**: max 2 hops, dedup by reportId, TTL 30s
- **Tokens**: `v1.<b64payload>.<hmac-sha256>` — route hint, forward token, diagnostic key

See [Load Balancer Architecture](/docs/deployment/load-balancer) for full details.
...
- **Path A**: Join existing cluster — connect via path + super admin login + one-time token
...
Automatic with manual override — LB handles rebalancing.
