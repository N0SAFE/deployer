...
- `mesh` (public mesh surface), `nodes` (fleet servers, allocations, admission)

The mesh is the single tenant — there is no organization layer. Projects and
services are mesh-wide; deployments, docker and node configuration are scoped
per node.
...
- Auth/bootstrap: `DEV_AUTH_KEY` (secrets resolved via mesh at runtime)
- Mesh/cluster: `MESH_CLUSTER_ID`, `MESH_STREAM_SHARED_SECRET`
