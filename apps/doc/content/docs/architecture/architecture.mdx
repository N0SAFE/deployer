...
The v3 runtime baseline is a Docker Swarm cluster with manager/worker roles (implemented — see `docs/swarm-orchestration/README.md`).
...
The Swarm execution backend, master election, node sharing, and cluster API surface are implemented behind the dockerode SDK (zero CLI). See `docs/swarm-orchestration/` for design docs and the `IMPLEMENTATION_TODO.md` status checklist.
</DocCallout>
...
- [Entity & Model Hierarchy](/docs/architecture/entity-model-hierarchy)
...
- [Traefik Ingress & Failover](/docs/deployment/instance-ports) — Traefik is the sole reverse proxy; a direct-proxy supervisor takes over only while Traefik is down
