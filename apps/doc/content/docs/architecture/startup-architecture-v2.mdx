...
Main App shell created (empty DI container)

  │
  ├─► SubApp 1: Config SubApp
  │     starts → loads env → fires ConfigTrigger → destroyed
  │       └─► Main App resolves ConfigModule
  │
  ├─► SubApp 2: Database SubApp
  │     starts → creates pool → runs migrations → fires DbTrigger → destroyed
  │       └─► Main App resolves DatabaseModule
  │
  ├─► SubApp 3: Auth SubApp
  │     starts → inits BetterAuth → creates admin → fires AuthTrigger → destroyed
  │       └─► Main App resolves AuthModule
  │
  ├─► SubApp 4: Mesh SubApp
  │     starts → registers with mesh → fires MeshTrigger → destroyed
  │       └─► Main App resolves MeshModule
  │
  ├─► SubApp 5: Events SubApp
  │     starts → connects event bus → fires EventsTrigger → destroyed
  │       └─► Main App resolves EventsModule
  │
  ├─► SubApp 6: Docker SubApp
  │     starts → validates socket → fires DockerTrigger → destroyed
  │       └─► Main App resolves DockerModule
  │
  └─► All done → feature modules have everything they need

Main App fully initialized (all modules resolved, no sub-apps alive)
```
