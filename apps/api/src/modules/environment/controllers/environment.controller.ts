...
export class EnvironmentController {
    ...
    private transformEnvironment(env: any) {
        ...
        return {
            id: env.id,
            projectId: (env.projectId ?? '00000000-0000-0000-0000-000000000000'), // Ensure required field
    ...
    bulkDelete() {
        return implement(environmentContract.bulkDelete).handler(async ({ input }) => {
            this.logger.log(`Bulk deleting ${String(input.environmentIds.length)} environments`);
    ...
    cleanupPreviewEnvironments() {
        return implement(environmentContract.cleanupPreviewEnvironments).handler(async ({ input }) => {
            this.logger.log(`Cleaning up preview environments for project: ${input.projectId}, dry run: ${String(input.dryRun)}`);
            const resolvedProjectId = await this.resolveProjectId(input.projectId);
    ...
    parseTemplate() {
        return implement(environmentContract.parseTemplate).handler(({ input }) => {
    ...
    resolveTemplate() {
        return implement(environmentContract.resolveTemplate).handler(({ input }) => {
            this.logger.log(`Resolving template for environment: ${String(input.environmentId)}`);
    ...
    batchResolveTemplates() {
        return implement(environmentContract.batchResolveTemplates).handler(({ input }) => {
            this.logger.log(`Batch resolving templates: ${String(Object.keys(input.templates).length)} templates`);
    ...
    resolveEnvironmentVariables() {
        return implement(environmentContract.resolveEnvironmentVariables).handler(async ({ input }) => {
            this.logger.log(`Resolving environment variables for: ${String(input.id)}`);
    ...
    getResolutionHistory() {
        return implement(environmentContract.getResolutionHistory).handler(({ input }) => {
    ...
    validate() {
        return implement(environmentContract.validate).handler(({ input }) => {
    ...
    compare() {
        return implement(environmentContract.compare).handler(({ input }) => {
