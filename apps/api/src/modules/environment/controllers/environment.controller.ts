...
export class EnvironmentController {
  ...
  create() {
    return implement(environmentContract.create).handler(async ({ input }) => {
      ...
      // Log access
      await this.environmentService.logAccess(
        environment.id,
        userId,
        'create'
      );
  ...
  updateVariables() {
    return implement(environmentContract.updateVariables).handler(async ({ input }) => {
      ...
      const variables = await this.environmentService.updateEnvironmentVariables(
        input.environmentId,
  ...
  updateStatus() {
    return implement(environmentContract.updateStatus).handler(async ({ input }) => {
      ...
      const environment = await this.environmentService.updateEnvironmentStatus(
        input.environmentId,
        simpleStatus as 'healthy' | 'pending' | 'updating' | 'error' | 'inactive',
        input.metadata
      );
