...
import { eq, desc, asc, count, ilike, and, or, inArray, SQL } from 'drizzle-orm';
...
export class EnvironmentRepository {
    ...
    async createEnvironment(data: NewEnvironment): Promise<Environment> {
        ...
        const results = await db.insert(environments).values(data).returning();
    ...
    async listEnvironments(params: {
        ...
        if (type) {
            whereConditions.push(eq(environments.type, type as typeof environments.type.enumValues[number]));
        }
        if (status) {
            whereConditions.push(eq(environments.status, status as typeof environments.status.enumValues[number]));
        }
        if (search) {
    ...
    async createEnvironmentVariable(data: NewEnvironmentVariable): Promise<EnvironmentVariable> {
        ...
        const db = this.databaseService.db;
        const results = await db.insert(environmentVariables).values(data).returning();
    ...
    async bulkUpdateVariables(environmentId: string, variables: {
        key: string;
        value: string;
        isSecret?: boolean;
        description?: string;
    ...
    async linkServiceToEnvironment(data: NewEnvironmentService): Promise<EnvironmentService> {
        ...
        const db = this.databaseService.db;
        const results = await db.insert(environmentServices).values(data).returning();
        const link = results[0];
        if (!link) {
            throw new Error('Failed to link service to environment');
        }
        return link;
    }
    ...
    async createEnvironmentTemplate(data: NewEnvironmentTemplate): Promise<EnvironmentTemplate> {
        ...
        const db = this.databaseService.db;
        const results = await db.insert(environmentTemplates).values(data).returning();
        const template = results[0];
        if (!template) {
            throw new Error('Failed to create environment template');
        }
        return template;
    }
    async findEnvironmentTemplates(type?: string): Promise<EnvironmentTemplate[]> {
        ...
        if (type) {
            return await db
                ...
                .where(eq(environmentTemplates.type, type as typeof environmentTemplates.type.enumValues[number]))
    ...
    async createVariableTemplate(data: NewVariableTemplate): Promise<VariableTemplate> {
        ...
        const db = this.databaseService.db;
        const results = await db.insert(variableTemplates).values(data).returning();
        const template = results[0];
        if (!template) {
    ...
    async cleanupExpiredPreviewEnvironments(): Promise<string[]> {
        ...
        if (expiredIds.length > 0) {
            this.logger.log(`Cleaned up ${String(expiredIds.length)} expired preview environments`);
        }
        return expiredIds;
    }
    // Bulk operations
    async bulkDeleteEnvironments(ids: string[]): Promise<number> {
        if (ids.length === 0)
            return 0;
        this.logger.log(`Bulk deleting ${String(ids.length)} environments`);
        const db = this.databaseService.db;
        const result = await db
            .update(environments)
            .set({ isActive: false, updatedAt: new Date() })
