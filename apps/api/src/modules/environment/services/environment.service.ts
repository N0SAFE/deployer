...
export class EnvironmentService {
    ...
    async createEnvironment(data: {
        ...
        if (data.templateId) {
            this.applyEnvironmentTemplate(environment.id, data.templateId, data.createdBy);
        }
        return environment;
    }
    ...
    async updateEnvironmentVariables(environmentId: string, variables: {
        key: string;
        value: string;
        isSecret?: boolean;
        description?: string;
    ...
    private async resolveVariableTemplate(template: string, environmentId: string): Promise<string> {
        ...
        if (matches) {
            for (const match of matches) {
                ...
                const resolvedValue = this.resolveVariablePath(path, environmentId);
                resolved = resolved.replace(match, resolvedValue);
            }
        }
        return resolved;
    }
    private resolveVariablePath(path: string, environmentId: string): string {
    ...
    async createPreviewEnvironment(data: {
        ...
        if (data.name.startsWith('pr-')) {
            ...
            baseSlug = `pr-${String(data.sourcePR)}`;
        }
    ...
    applyEnvironmentTemplate(environmentId: string, templateId: string, _userId: string): void {
