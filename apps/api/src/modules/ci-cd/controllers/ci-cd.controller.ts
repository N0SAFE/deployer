...
export class CiCdController {
    ...
    createPipeline() {
        return implement(ciCdContract.pipeline.createPipeline).handler(({ input }) => {
            return this.ciCdService.createPipeline(input);
        });
    }
    @Implement(ciCdContract.pipeline.getPipeline)
    getPipeline() {
        return implement(ciCdContract.pipeline.getPipeline).handler(({ input }) => {
            const { id } = input;
    ...
    updatePipeline() {
        return implement(ciCdContract.pipeline.updatePipeline).handler(({ input }) => {
            const { id, data } = input;
            return this.ciCdService.updatePipeline(id, data);
        });
    }
    @Implement(ciCdContract.pipeline.deletePipeline)
    deletePipeline() {
        return implement(ciCdContract.pipeline.deletePipeline).handler(({ input }) => {
            const { id } = input;
            return this.ciCdService.deletePipeline(id);
        });
    }
    @Implement(ciCdContract.pipeline.listPipelines)
    listPipelines() {
        return implement(ciCdContract.pipeline.listPipelines).handler(({ input: query }) => {
            return this.ciCdService.listPipelines(query);
        });
    }
    @Implement(ciCdContract.pipeline.triggerPipeline)
    triggerPipeline() {
        return implement(ciCdContract.pipeline.triggerPipeline).handler(({ input }) => {
            const { id, ...options } = input;
            return this.ciCdService.triggerPipeline(id, options);
        });
    }
    @Implement(ciCdContract.pipeline.cancelPipeline)
    cancelPipeline() {
        return implement(ciCdContract.pipeline.cancelPipeline).handler(({ input }) => {
            const { pipelineId, runId } = input;
            return this.ciCdService.cancelPipeline(pipelineId, runId);
        });
    }
    @Implement(ciCdContract.pipeline.getPipelineStatus)
    getPipelineStatus() {
        return implement(ciCdContract.pipeline.getPipelineStatus).handler(({ input }) => {
            const { id } = input;
            return this.ciCdService.getPipelineStatus(id);
        });
    }
    @Implement(ciCdContract.pipeline.createPipelineFromTemplate)
    createPipelineFromTemplate() {
        return implement(ciCdContract.pipeline.createPipelineFromTemplate).handler(({ input }) => {
            ...
            return this.ciCdService.createPipeline(pipelineData);
        });
    }
    @Implement(ciCdContract.pipeline.listPipelineTemplates)
    listPipelineTemplates() {
        return implement(ciCdContract.pipeline.listPipelineTemplates).handler(({ input: _input }) => {
            return {
                templates: [
                    {
                        id: "nodejs-basic",
    ...
    getPipelineStats() {
        return implement(ciCdContract.pipeline.getPipelineStats).handler(() => {
            return {
                totalPipelines: 42,
                activePipelines: 15,
                successRate: 0.87,
    ...
    validatePipelineConfig() {
        return implement(ciCdContract.pipeline.validatePipelineConfig).handler(({ input }) => {
    ...
    createBuild() {
        return implement(ciCdContract.build.createBuild).handler(({ input }) => {
            return this.ciCdService.createBuild(input);
        });
    }
    @Implement(ciCdContract.build.getBuild)
    getBuild() {
        return implement(ciCdContract.build.getBuild).handler(({ input: { id } }) => {
            return this.ciCdService.getBuild(id);
        });
    }
    @Implement(ciCdContract.build.listBuilds)
    listBuilds() {
        return implement(ciCdContract.build.listBuilds).handler(({ input: query }) => {
            return this.ciCdService.listBuilds(query);
        });
    }
    @Implement(ciCdContract.build.cancelBuild)
    cancelBuild() {
        return implement(ciCdContract.build.cancelBuild).handler(() => {
            return { success: true };
        });
    }
    @Implement(ciCdContract.build.retryBuild)
    retryBuild() {
        return implement(ciCdContract.build.retryBuild).handler(({ input: { id } }) => {
            return this.ciCdService.retryBuild(id);
        });
    }
    @Implement(ciCdContract.build.getBuildLogs)
    getBuildLogs() {
        return implement(ciCdContract.build.getBuildLogs).handler(({ input }) => {
            const { id, ...options } = input;
            return this.ciCdService.getBuildLogs(id, options);
        });
    }
    ...
    downloadArtifact() {
        return implement(ciCdContract.build.downloadArtifact).handler(({ input: { buildId, artifactId } }) => {
            return {
    ...
    getBuildConfig() {
        return implement(ciCdContract.build.getBuildConfig).handler(({ input: { pipelineId } }) => {
            return {
    ...
    updateBuildConfig() {
        return implement(ciCdContract.build.updateBuildConfig).handler(() => {
    ...
    getBuildQueue() {
        return implement(ciCdContract.build.getBuildQueue).handler(({ input }) => {
            return {
                queue: [
                    {
                        id: "build-1",
    ...
    updateBuildPriority() {
        return implement(ciCdContract.build.updateBuildPriority).handler(() => {
    ...
    getBuildStats() {
        return implement(ciCdContract.build.getBuildStats).handler(() => {
            return {
                totalBuilds: 156,
    ...
    listBuildEnvironments() {
        return implement(ciCdContract.build.listBuildEnvironments).handler(() => {
            return {
                environments: [
                    {
                        id: "env-1",
    ...
    createWebhook() {
        return implement(ciCdContract.webhook.createWebhook).handler(({ input }) => {
            return this.ciCdService.createWebhook(input);
        });
    }
    @Implement(ciCdContract.webhook.getWebhook)
    getWebhook() {
        return implement(ciCdContract.webhook.getWebhook).handler(({ input }) => {
            return this.ciCdService.getWebhook(input.id);
        });
    }
    @Implement(ciCdContract.webhook.updateWebhook)
    updateWebhook() {
        return implement(ciCdContract.webhook.updateWebhook).handler(({ input }) => {
            return this.ciCdService.updateWebhook(input.id, input.data);
        });
    }
    @Implement(ciCdContract.webhook.deleteWebhook)
    deleteWebhook() {
        return implement(ciCdContract.webhook.deleteWebhook).handler(({ input }) => {
            return this.ciCdService.deleteWebhook(input.id);
        });
    }
    @Implement(ciCdContract.webhook.listWebhooks)
    listWebhooks() {
        return implement(ciCdContract.webhook.listWebhooks).handler(({ input }) => {
            const result = this.ciCdService.listWebhooks(input);
            return {
                ...result,
                limit: result.limit ?? input.limit ?? 10,
            };
        });
    }
    @Implement(ciCdContract.webhook.testWebhook)
    testWebhook() {
        return implement(ciCdContract.webhook.testWebhook).handler(({ input }) => {
            return this.ciCdService.testWebhook(input.id, input.event, input.payload);
        });
    }
    @Implement(ciCdContract.webhook.validateWebhookUrl)
    validateWebhookUrl() {
        return implement(ciCdContract.webhook.validateWebhookUrl).handler(({ input }) => {
            // Mock URL validation
            try {
                new URL(input.url);
                return {
    ...
    listWebhookDeliveries() {
        return implement(ciCdContract.webhook.listWebhookDeliveries).handler(({ input }) => {
            const deliveries = [
                {
                    id: "delivery-1",
    ...
    getWebhookDelivery() {
        return implement(ciCdContract.webhook.getWebhookDelivery).handler(({ input }) => {
            return {
                id: input.id,
    ...
    retryWebhookDelivery() {
        return implement(ciCdContract.webhook.retryWebhookDelivery).handler(() => {
            return {
                success: true,
    ...
    listWebhookEvents() {
        return implement(ciCdContract.webhook.listWebhookEvents).handler(() => {
            return {
                events: [
                    {
    ...
    generateWebhookSecret() {
        return implement(ciCdContract.webhook.generateWebhookSecret).handler(({ input }) => {
            const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
            let secret = "";
            Array(input.length).fill(0).forEach(() => {
    ...
    getWebhookHealth() {
        return implement(ciCdContract.webhook.getWebhookHealth).handler(({ input: { id } }) => {
    ...
    getOverview() {
        return implement(ciCdContract.getOverview).handler(({ input }) => {
            return this.ciCdService.getOverview(input);
        });
    }
}
