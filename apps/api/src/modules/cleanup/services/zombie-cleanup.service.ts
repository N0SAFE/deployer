 ...
 * This service was moved from DockerModule to CleanupModule to fix
 * an inverted dependency issue - Docker (core) should not depend on
 * business modules like Project and Deployment.
 * 
 * Responsibilities:
...
export class ZombieCleanupService {
  ...
  async resumeIncompleteDeployments(): Promise<{
    ...
    this.logger.log(
      `Found ${String(incompleteDeployments.length)} stuck deployments (no update for 5+ minutes)`
    );
    ...
    this.logger.log(
      `📊 Resume summary: ${String(resumed)} resumed, ${String(failed)} failed, ${String(skipped)} skipped`
    );
  ...
  private async checkDeploymentFiles(deployment: Deployment): Promise<boolean> {
    try {
      const sourceConfig = deployment.sourceConfig as Record<string, unknown> | null;
      const phaseMetadata = deployment.phaseMetadata as Record<string, unknown> | null;
      const uploadPath =
        (sourceConfig?.filePath as string | undefined) ??
        (phaseMetadata?.uploadPath as string | undefined);
  ...
  async autoCleanup() {
    ...
    try {
      ...
      this.logger.log(
        `Symlink reconciliation: ${String(symlinkResult.fixed)} fixed, ${String(symlinkResult.verified)} verified, ${String(symlinkResult.errors)} errors`,
      );

      // Step 2: Try to reconcile containers
      const reconcileResult = await this.reconcileAllContainers({ dryRun: false });
      this.logger.log(
        `Reconciliation: ${String(reconcileResult.restarted)} restarted, ${String(reconcileResult.relabeled)} relabeled, ${String(reconcileResult.skipped)} skipped`,
      );

      // Step 3: Clean up truly orphaned containers
      const cleanupResult = await this.cleanupZombieContainers();
      this.logger.log(
        `Auto cleanup completed: ${String(cleanupResult.removed)} removed, ${String(cleanupResult.errors)} errors`,
      );

      // Step 4: Clean up old helper containers
      const helperResult = await this.cleanupZombieHelpers();
      this.logger.log(
        `Helper cleanup: ${String(helperResult.removed)} removed, ${String(helperResult.errors)} errors`,
      );
    } catch (error) {
  ...
  async reconcileSymlinks(): Promise<{
    ...
    this.logger.log(
      `✅ Symlink reconciliation complete: ${String(fixed)} fixed, ${String(verified)} verified, ${String(errors)} errors`
    );
  ...
  async reconcileAllContainers(options?: {
    ...
    this.logger.log(
      `Starting container reconciliation (dryRun=${String(dryRun)}, projectId=${projectId ?? 'all'})`,
    );
    ...
    this.logger.log(`Found ${String(activeProjectMap.size)} active projects in database`);
    ...
    this.logger.log(`Found ${String(allContainers.length)} containers with deployer labels`);
    ...
    this.logger.log(
      `Reconciliation ${dryRun ? '(dry run) ' : ''}completed: ${String(restarted)} restarted, ${String(relabeled)} relabeled, ${String(skipped)} skipped, ${String(errors)} errors`,
    );
  ...
  async cleanupZombieContainers(options?: {
    ...
    this.logger.log(
      `Starting zombie container cleanup (dryRun=${String(dryRun)}, projectId=${projectId ?? 'all'})`,
    );
    ...
    this.logger.log(`Found ${String(activeProjectIds.size)} active projects in database`);
    ...
    this.logger.log(
      `Found ${String(allContainers.length)} containers with deployer.project_server label`,
    );
    ...
    if (dryRun) {
      this.logger.log(
        `Dry run completed: found ${String(zombies.length)} zombie containers (not removed)`,
      );
    } else {
      this.logger.log(
        `Cleanup completed: ${String(removed)} removed, ${String(errors)} errors, ${String(zombies.length)} total zombies`,
      );
    }
  ...
  async cleanupZombieHelpers(options?: {
    ...
    this.logger.log(
      `Cleaning up zombie helper containers older than ${String(olderThanMinutes)} minutes (dryRun=${String(dryRun)})`,
    );
    ...
    for (const containerInfo of allContainers) {
      ...
      if (
        ...
        if (containerInfo.State === 'exited' && age > maxAge) {
          this.logger.warn(
            `Found zombie helper: ${name} (${String(Math.round(age / 60000))} minutes old)`,
          );
    ...
    if (dryRun) {
      this.logger.log(`Dry run: would remove ${String(removed)} helper containers`);
    } else {
      this.logger.log(
        `Helper cleanup: ${String(removed)} removed, ${String(errors)} errors`,
      );
    }
