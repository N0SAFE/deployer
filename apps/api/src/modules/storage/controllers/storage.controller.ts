...
export class StorageController {
    ...
    listBackups() {
        return implement(storageContract.listBackups).handler(({ input }) => {
            const { databaseName, status, limit = 20, offset = 0 } = input;
            return this.storageService.listBackups(databaseName, status, limit, offset);
        });
    }
    @Implement(storageContract.getBackup)
    getBackup() {
        return implement(storageContract.getBackup).handler(({ input }) => {
    ...
    restoreBackup() {
        return implement(storageContract.restoreBackup).handler(({ input }) => {
            const { backupId } = input;
    ...
    deleteBackup() {
        return implement(storageContract.deleteBackup).handler(({ input }) => {
            const { backupId } = input;
    ...
    cancelBackup() {
        return implement(storageContract.cancelBackup).handler(({ input }) => {
            const { backupId } = input;
    ...
    downloadBackup() {
        return implement(storageContract.downloadBackup).handler(({ input }) => {
            const { backupId } = input;
    ...
    getUsage() {
        return implement(storageContract.getUsage).handler(({ input }) => {
            const { path } = input;
            return this.storageService.getStorageUsage(path);
        });
    }
        ...
        return implement(storageContract.getMetrics).handler(async ({ input }) => {
            ...
            return {
                ...
                topFiles: Array.from({ length: Math.min(topFilesLimit, 3) }, (_, i) => ({
                    path: `/data/large-file-${String(i + 1)}.log`,
    ...
    getHistoricalUsage() {
        return implement(storageContract.getHistoricalUsage).handler(({ input }) => {
            ...
            this.logger.log(`Retrieved historical usage data from ${String(startDate)} to ${String(endDate)} with ${granularity} granularity`);
            return data;
        });
    }
    @Implement(storageContract.listQuotas)
    listQuotas() {
        return implement(storageContract.listQuotas).handler(({ input }) => {
    ...
    createQuota() {
        return implement(storageContract.createQuota).handler(({ input }) => {
    ...
    updateQuota() {
        return implement(storageContract.updateQuota).handler(({ input }) => {
    ...
    deleteQuota() {
        return implement(storageContract.deleteQuota).handler(({ input }) => {
            const { path } = input;
    ...
    cleanup() {
        return implement(storageContract.cleanup).handler(({ input }) => {
            const options = input;
            return this.storageService.performCleanup(options);
        });
    }
}
