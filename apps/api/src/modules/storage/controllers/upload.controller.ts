...
import {
  uploadFileContract,
  deployUploadContract,
  getUploadInfoContract,
  deleteUploadContract,
  getStaticFileContract,
  listStaticFilesContract,
  setupStaticServingContract,
  getStaticServingStatsContract,
  cleanupOldUploadsContract,
  getUploadStatsContract,
} from "@repo/api-contracts";
import { StaticProviderService } from "@/core/modules/providers/static/static-provider.service";
import {
  FileUploadService,
  type UploadedFile,
} from "@/core/modules/storage/services/file-upload.service";
...
export class UploadController {
  ...
  uploadFile() {
    return implement(uploadFileContract).handler(async ({ input }) => {
      ...
      const result =
        await this.fileUploadService.processUploadedFile(uploadedFile);
  ...
  deployUpload() {
    return implement(deployUploadContract).handler(async ({ input }) => {
      ...
      this.logger.log(`Deployment job created with ID: ${String(job.id)}`);
      return {
        deploymentId: `deploy-${String(job.id)}`,
  ...
  getUploadInfo() {
    return implement(getUploadInfoContract).handler(async ({ input }) => {
      ...
      const uploadInfo = await this.fileUploadService.getUploadedFileInfo(
        input.uploadId
      );
      if (!uploadInfo) {
  ...
  getStaticFile() {
    return implement(getStaticFileContract).handler(async ({ input }) => {
      this.logger.log(
        `Serving static file: ${input.filePath} for service: ${input.serviceId}`
      );
      const fileInfo = await this.staticFileServingService.getStaticFile(
        input.serviceId,
        input.filePath
      );
  ...
  listStaticFiles() {
    return implement(listStaticFilesContract).handler(({ input }) => {
  ...
  setupStaticServing() {
    return implement(setupStaticServingContract).handler(async ({ input }) => {
      this.logger.log(
        `Setting up static serving for deployment: ${input.deploymentId}`
      );
      // Require serviceId (or derive it from deployment in a later migration)
      if (!input.serviceId) {
        throw new BadRequestException(
          "serviceId is required for project-level static serving"
        );
      }
  ...
  getStaticServingStats() {
    return implement(getStaticServingStatsContract).handler(
      async ({ input }) => {
        this.logger.log(
          `Getting static serving stats for service: ${input.serviceId}`
        );
        const stats = await this.staticFileServingService.getServingStats(
          input.serviceId
        );
        return {
  ...
  cleanupOldUploads() {
    return implement(cleanupOldUploadsContract).handler(async ({ input }) => {
      this.logger.log(
        `Cleaning up uploads older than ${String(input.olderThanDays)} days`
      );
      const deletedCount = await this.fileUploadService.cleanupOldUploads(
        input.olderThanDays
      );
      return {
        deletedCount,
  ...
  getUploadStats() {
    return implement(getUploadStatsContract).handler((_input) => {
