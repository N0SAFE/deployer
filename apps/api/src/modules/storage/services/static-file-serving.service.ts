...
import { Database } from '../../../core/modules/db/drizzle/index';
import { Inject } from '@nestjs/common';
import { DATABASE_CONNECTION } from '../../../core/modules/db/database-connection';
import { services, deployments } from '../../../core/modules/db/drizzle/schema';
...
export class StaticFileServingService {
   ...
   */
  async getStaticFile(serviceId: string, filePath: string): Promise<{
    content: Buffer;
    contentType: string;
    ...
    try {
      ...
      if (!deployment) {
        throw new NotFoundException(`No successful deployment found for service ${serviceId}`);
      }
      ...
      this.logger.log(`Served static file: ${filePath} for service ${serviceId}`);
      return {
        content,
        contentType,
        headers,
      ...
      this.logger.error(`Failed to serve static file ${filePath} for service ${serviceId}:`, error);
      throw error;
    }
  }
  ...
  async setupStaticServing(deploymentId: string, sourcePath: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to setup static serving for deployment ${deploymentId}:`, error);
      throw error;
    }
  }
   ...
   */
  private async copyStaticFiles(sourcePath: string, targetPath: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to copy static files from ${sourcePath} to ${targetPath}:`, error);
      throw error;
    }
  }
   ...
   */
  private async generateManifest(deploymentId: string, staticDir: string): Promise<void> {
    try {
      ...
      this.logger.log(`Generated manifest for deployment ${deploymentId} (${files.length} files)`);

    } catch (error) {
      this.logger.error(`Failed to generate manifest for deployment ${deploymentId}:`, error);
      throw error;
    }
  }
   ...
   */
  private async resolveFilePath(deploymentId: string, requestPath: string): Promise<string> {
    const staticDir = path.join(this.staticFilesDir, deploymentId);
   ...
   * Generate cache headers
   */
  private generateCacheHeaders(fileInfo: StaticFileInfo, _cacheConfig?: any): Record<string, string> {
  ...
  async cleanupStaticFiles(deploymentId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Cleaned up static files for deployment: ${deploymentId}`);
    } catch (error) {
      this.logger.error(`Failed to cleanup static files for deployment ${deploymentId}:`, error);
    }
  }
  ...
  async getDeploymentManifest(deploymentId: string): Promise<any> {
    try {
      const manifestPath = path.join(this.staticFilesDir, deploymentId, '.manifest.json');
      if (await fs.pathExists(manifestPath)) {
        return await fs.readJson(manifestPath);
      }
      return null;
    } catch (error) {
      this.logger.error(`Failed to get manifest for deployment ${deploymentId}:`, error);
      return null;
    }
  }
  ...
  async getServingStats(serviceId: string): Promise<{
    ...
    try {
      ...
      const manifest = await this.getDeploymentManifest(service.latestDeployment.id);
      if (!manifest) {
        return { totalFiles: 0, totalSize: 0, lastAccessed: null };
      }

      const files = Object.values(manifest) as Array<{ size: number }>;
      ...
      this.logger.error(`Failed to get serving stats for service ${serviceId}:`, error);
      return { totalFiles: 0, totalSize: 0, lastAccessed: null };
    }
  }
}