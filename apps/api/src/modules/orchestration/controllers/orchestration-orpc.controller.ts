...
import {
  ...
  HealthCheckService,
  JobTrackingService,
  TraefikOrchestrationService,
} from '@/core/modules/orchestration';
...
export class OrchestrationOrpcController {
    ...
    getStack() {
        return implement(orchestrationContract.getStack).handler(async ({ input }) => {
            ...
            try {
                ...
                return this.orchestrationAdapter.adaptGetStackToContract(status);
            }
            catch (error) {
    ...
    listStacks() {
        return implement(orchestrationContract.listStacks).handler(async ({ input }) => {
            ...
            try {
                ...
                return this.orchestrationAdapter.adaptListStacksToContract(stacks);
            }
            catch (error) {
    ...
    setResourceQuotas() {
        return implement(orchestrationContract.setResourceQuotas).handler(({ input }) => {
            this.logger.log('Setting resource quotas for project:', input.projectId, input.environment);
            try {
                this.resourceService.setResourceQuotas(input.projectId, input.environment, input.quotas);
                return {
                    success: true,
    ...
    getResourceAllocation() {
        return implement(orchestrationContract.getResourceAllocation).handler(({ input }) => {
            this.logger.log('Getting resource allocation for project:', input.projectId, input.environment);
            try {
                const allocation = this.resourceService.getResourceAllocation(input.projectId, input.environment);
                return {
                    success: true,
                    data: allocation
                };
    ...
    getSystemResourceSummary() {
        return implement(orchestrationContract.getSystemResourceSummary).handler(() => {
