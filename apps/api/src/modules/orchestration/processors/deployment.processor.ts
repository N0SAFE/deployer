...
import { ResourceAllocationService } from "@/core/modules/orchestration/services/resource-allocation.service";
...
import {
  DockerfileBuilderService,
  NixpackBuilderService,
  BuildpackBuilderService,
  StaticBuilderService,
  DockerComposeBuilderService,
} from "@/core/modules/builders";
...
import { StaticFileServingService } from "@/core/modules/providers/static/services/static-file-serving.service";
...
import type { IDeploymentProvider } from "@/core/modules/providers/interfaces/provider.interface";
...
export class DeploymentProcessor {
  ...
  constructor(
    ...
    private readonly nixpackBuilder: NixpackBuilderService,
    private readonly buildpackBuilder: BuildpackBuilderService,
    private readonly staticBuilder: StaticBuilderService,
    private readonly dockerComposeBuilder: DockerComposeBuilderService,
  ...
  async handleStandardDeployment(
    ...
    try {
      ...
      const buildType = service.builderId;
      ...
      if (buildType === "static") {
        ...
        // Fetch Traefik config to get domain and subdomain for static deployments
        let domain = project.baseDomain || process.env.TRAEFIK_DOMAIN || 'localhost';
        let subdomain: string | undefined;
        
        try {
          const traefikConfig = await this.databaseService.db.query.traefikServiceConfigs.findFirst({
            where: (configs, { eq }) => eq(configs.serviceId, service.id),
          });
          
          if (traefikConfig) {
            domain = traefikConfig.domain;
            subdomain = traefikConfig.subdomain || undefined;
            this.logger.log(`Using Traefik config for ${service.name}: domain=${domain}, subdomain=${subdomain || 'none'}`);
          } else {
            this.logger.warn(`No Traefik config found for service ${service.name}, using defaults`);
          }
        } catch (traefikErr) {
          this.logger.warn(`Failed to fetch Traefik config for ${service.name}:`, traefikErr);
        }
        
        deploymentResult = await this.staticBuilder.deploy({
          deploymentId,
          ...
          domain,
          subdomain,
          staticProviderService: this.staticProviderService,
          onPhaseUpdate: async (phase, progress, metadata) => {
          ...
          onLog: async (log) => {
            const level = log.level === 'debug' ? 'info' : log.level;
            await this.logDeployment(deploymentId, level, log.message, {
              phase: log.phase,
              step: log.step,
              service: log.service,
            });
        ...
        deploymentResult = await this.dockerfileBuilder.deploy({
          deploymentId,
          serviceName: service.name,
          sourcePath,
          dockerfilePath:
      ...
      } else if (buildType === "nixpack") {
        await this.logDeployment(
          deploymentId,
          "info",
          "Deploying using Nixpack (Node.js)"
        );
        deploymentResult = await this.nixpackBuilder.deploy({
          deploymentId,
          serviceName: service.name,
          sourcePath,
          nodeVersion: (service.builderConfig as any)?.nodeVersion || "18-alpine",
          installCommand:
      ...
      } else if (buildType === "buildpack") {
        await this.logDeployment(
          deploymentId,
          "info",
          "Deploying using Buildpack (auto-detect language)"
        );
        deploymentResult = await this.buildpackBuilder.deploy({
          deploymentId,
          serviceName: service.name,
          sourcePath,
          language: (service.builderConfig as any)?.language,
          installCommand: service.builderConfig?.installCommand,
          startCommand: service.builderConfig?.startCommand,
          buildCommand: service.builderConfig?.buildCommand,
          environmentVariables: service.environmentVariables || {},
      ...
      // Trigger automatic cleanup of old deployments (runs in background)
      this.cleanupOldDeploymentsAsync(serviceId).catch(err => {
        this.logger.warn(`Failed to cleanup old deployments for service ${serviceId}: ${err.message}`);
  ...
  async handleOrchestrationUploadDeployment(job: Job) {
    ...
    try {
      ...
      const { FileUploadService } = await import(
        "@/core/modules/storage/services/file-upload.service"
      );
      const fileUploadService = new FileUploadService(null as any, this.constantsService); // We'll only use metadata analysis
  ...
  private async registerDomain(
    ...
    // Get provider instance to retrieve Traefik template
    const provider = this.getProviderInstance(service.providerId);
    const traefikTemplate = provider.getTraefikTemplate();
    
    // Get container info for routing
    ...
    // Register with Traefik using template system
    await this.projectServerService.addServiceRouter(
      project.id,
      service.name,
      `${subdomain}.${process.env.TRAEFIK_DOMAIN || "localhost"}`,
   ...
   */
  private getProviderInstance(providerType: string): IDeploymentProvider {
    switch (providerType) {
      case 'github':
        return this.githubProviderService;
      case 'static':
        return this.staticProviderService;
      // Add other providers as they are implemented
      default:
        this.logger.warn(`Unknown provider type: ${providerType}, falling back to static provider`);
        return this.staticProviderService;
    }
  }
...
CMD ["sh", "-c", "${startCommand}"]
  ...
  /**
   * Cleanup old deployments asynchronously (fire-and-forget)
   * Called after a successful deployment to maintain retention policy
   */
  private async cleanupOldDeploymentsAsync(serviceId: string): Promise<void> {
    try {
      this.logger.log(`Starting automatic cleanup for service ${serviceId}`);
      const result = await this.deploymentCleanupService.cleanupOldDeployments(serviceId);
      
      if (result.deletedCount > 0) {
        this.logger.log(
          `Automatic cleanup completed for service ${serviceId}: ` +
          `deleted ${result.deletedCount} old deployments, kept ${result.keptCount}`
        );
      }
    } catch (error) {
      const err = error as Error;
      this.logger.error(
        `Automatic cleanup failed for service ${serviceId}: ${err.message}`,
        err.stack
      );
      // Don't throw - cleanup failure shouldn't affect the deployment
    }
  }
}
