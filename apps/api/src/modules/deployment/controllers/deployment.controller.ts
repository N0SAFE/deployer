...
export class DeploymentController {
  ...
  jobStatus() {
    return implement(deploymentContract.jobStatus).handler(
      async ({ input }) => {
        this.logger.log(`Getting job status for job: ${input.jobId}`);
        try {
          const job = await this.deploymentQueueService.getJobStatus(
            input.jobId
          );
          ...
          const processedOn = job.data?.processedOn
            ? new Date(job.data.processedOn)
            : undefined;
          const finishedOn = job.data?.finishedOn
            ? new Date(job.data.finishedOn)
            : undefined;
  ...
  getStatus() {
    return implement(deploymentContract.getStatus).handler(
      async ({ input }) => {
  ...
  trigger() {
    return implement(deploymentContract.trigger).handler(async ({ input }) => {
      ...
      try {
        ...
        if (!dockerConnected) {
          throw new Error(
            "Docker service is not available. Cannot trigger deployment."
          );
        }
        ...
        try {
          if (service.providerConfig) {
            ...
            const config =
              typeof service.providerConfig === "object"
              ...
              if (value !== null && value !== undefined) {
                ...
                if (
                  ...
                  try {
                    ...
                    this.logger.warn(
                      `Skipping non-serializable property ${key} in provider config`
                    );
                  }
                  ...
                  try {
                    ...
                    this.logger.warn(
                      `Skipping non-serializable array property ${key} in provider config`
                    );
                  }
                }
              }
            ...
            this.logger.warn(
              `Service ${input.serviceId} has no provider config`
            );
            console.log(
              `WARNING: Service ${input.serviceId} has no provider config`
            );
          }
        } catch (configError) {
          this.logger.warn(
            `Failed to parse provider config for service ${input.serviceId}:`,
            configError
          );
          safeProviderConfig = {};
        }
        ...
        // Add deployment log
        await this.serviceService.addServiceLog(
          input.serviceId,
          deployment.id,
          {
            level: "info",
        ...
        try {
          ...
          this.logger.warn(
            `Failed to parse environment variables for service ${input.serviceId}:`,
            envError
          );
          safeEnvironmentVariables = {};
        }
        ...
        const sourceType =
          input.sourceType ??
          this.mapServiceProviderToSourceType(service.providerId);
        const requestSourceConfig = input.sourceConfig as DeploymentSourceConfig | undefined;
        let sourceConfig = requestSourceConfig
          ? ({ ...requestSourceConfig } as Record<string, unknown>)
          : ({ type: sourceType, ...safeProviderConfig } as Record<
              string,
              unknown
            >);
        ...
        try {
          ...
          this.logger.error(
            `Validation failed for service ${input.serviceId}:`,
            validationError
          );
          ...
          if (
            ...
            validationError.message.includes("Repository URL is required")
          ) {
        ...
        try {
          ...
          this.logger.error(
            `Job data is not serializable:`,
            serializationError
          );
          throw new Error(
            "Failed to prepare deployment job data - contains non-serializable properties"
          );
        }

        const jobId =
          await this.deploymentQueueService.addDeploymentJob(jobData);
        this.logger.log(
          `Queued deployment job: ${jobId} for deployment: ${deployment.id}`
        );
        return {
          deploymentId: deployment.id,
          jobId,
        ...
        this.logger.error(
          `Failed to trigger deployment for service ${input.serviceId}:`,
          error
        );
      ...
      try {
        ...
        if (deployment.status === "cancelled") {
          return {
            ...
            cancelledAt:
              (deployment.metadata as DeploymentMetadata)?.cancelledAt ||
        ...
        this.logger.log(
          `Stopped containers for deployment: ${input.deploymentId}`
        );
        // Try to cancel the job if it's still queued/running
        try {
          const jobs = await this.deploymentQueueService.getDeploymentJobs(
            input.deploymentId
          );
          for (const job of jobs) {
            if (
              ...
              ["waiting", "active", "delayed"].includes(
                job.opts?.jobId ? await job.getState() : "completed"
              )
            ) {
              await this.deploymentQueueService.cancelJob(
          ...
          this.logger.warn(
            `Failed to cancel job for deployment ${input.deploymentId}: ${jobError}`
          );
        ...
        const [cancelDeployment] = await this.databaseService.db
        ...
        this.logger.error(
          `Failed to cancel deployment ${input.deploymentId}:`,
          error
        );
        throw error;
      }
  ...
  rollback() {
    return implement(deploymentContract.rollback).handler(async ({ input }) => {
      this.logger.log(
        `Rolling back deployment: ${input.deploymentId} to target: ${input.targetDeploymentId}`
      );
      try {
        ...
        if (!targetDeployment) {
          throw new Error(
            `Target deployment ${input.targetDeploymentId} not found`
          );
        }
        ...
        if (targetDeployment.status !== "success") {
          throw new Error(
            `Target deployment status is ${targetDeployment.status}, must be 'success'`
          );
        }
  ...
  getLogs() {
    return implement(deploymentContract.getLogs).handler(async ({ input }) => {
      ...
      try {
        ...
        this.logger.error(
          `Error getting logs for deployment ${input.deploymentId}:`,
          error
        );
        throw error;
      }
      ...
      try {
        ...
        const formattedDeployments = paginatedDeployments.map((deployment) => ({
          ...
          status: deployment.status as
            ...
            | "failed"
            | "cancelled",
          environment: deployment.environment as
            | "production"
            | "staging"
            | "preview"
            | "development",
  ...
  health() {
    return implement(deploymentContract.health).handler(async ({ input }) => {
      this.logger.log(
        `Getting health status for deployment: ${input.deploymentId}`
      );

      try {
        const healthStatus =
          await this.deploymentService.monitorDeploymentHealth(
            input.deploymentId
          );
        return healthStatus;
      } catch (error) {
        this.logger.error(
          `Error getting health status for deployment ${input.deploymentId}:`,
          error
        );
        throw error;
      }
  ...
  detailedStatus() {
    return implement(deploymentContract.detailedStatus).handler(
      async ({ input }) => {
        this.logger.log(
          `Getting detailed status for deployment: ${input.deploymentId}`
        );

        try {
          const deploymentStatus =
            await this.deploymentService.getDeploymentStatus(
              input.deploymentId
            );
          return deploymentStatus;
        } catch (error) {
          this.logger.error(
            `Error getting detailed status for deployment ${input.deploymentId}:`,
            error
          );
          throw error;
        }
  ...
  restartUnhealthy() {
    return implement(deploymentContract.restartUnhealthy).handler(
      async ({ input }) => {
        this.logger.log(
          `Restarting unhealthy containers for deployment: ${input.deploymentId}`
        );

        try {
          const restartResult =
            await this.deploymentService.restartUnhealthyContainers(
              input.deploymentId
            );
          return restartResult;
        } catch (error) {
          this.logger.error(
            `Error restarting unhealthy containers for deployment ${input.deploymentId}:`,
            error
          );
          throw error;
        }
   ...
   */
  private mapServiceProviderToSourceType(
    provider: string
  ): "github" | "gitlab" | "git" | "upload" {
    ...
    switch (provider) {
      ...
      case "docker_registry":
        ...
        if (!sourceConfig.registryUrl) {
          throw new Error(
            `Registry URL is required for Docker registry deployments.`
          );
        }
        if (!sourceConfig.imageName) {
          throw new Error(
            `Image name is required for Docker registry deployments.`
          );
        }
        if (!sourceConfig.tag) {
          throw new Error(
            `Image tag is required for Docker registry deployments.`
          );
        }
        break;
      ...
      case "s3_bucket":
        ...
        if (!sourceConfig.accessKeyId || !sourceConfig.secretAccessKey) {
          throw new Error(
            `AWS credentials (accessKeyId and secretAccessKey) are required for S3 bucket deployments.`
          );
        }
        break;
      ...
      case "manual":
        ...
        this.logger.log(
          `Manual deployment detected for ${builder} builder - expecting file upload workflow`
        );
        break;
   ...
   */
  private validateBuilderConfiguration(
    builder: string,
    _sourceConfig: any
  ): void {
    switch (builder) {
  ...
  listContainers() {
    return implement(deploymentContract.listContainers).handler(
      async ({ input }) => {
        const {
          status,
          service,
        ...
        for (const deployment of allDeployments) {
          ...
          try {
            ...
            this.logger.error(
              `Health check failed for ${deployment.containerName}:`,
              error
            );
            containerHealth = {
              isHealthy: false,
          ...
          containers.push({
            ...
            health: {
              ...
              resources: {
                ...
                memoryUsage:
                  containerHealth.containerHealth.resources?.memoryUsage,
                memoryLimit:
                  containerHealth.containerHealth.resources?.memoryLimit,
              },
            },
            metadata: {
              ...
              triggerType: deployment.sourceConfig?.pullRequestNumber
                ? "github"
                : "webhook",
              triggerSource:
                deployment.sourceConfig?.branch ||
                deployment.sourceConfig?.repositoryUrl ||
                undefined,
            },
          });
        }
        ...
        const summary = {
          ...
          stoppedContainers: containers.filter((c) => c.status === "stopped")
            .length,
          failedContainers: containers.filter((c) => c.status === "failed")
            .length,
          healthyContainers: containers.filter((c) => c.health.isHealthy)
            .length,
        };
  ...
  containerAction() {
    return implement(deploymentContract.containerAction).handler(
      async ({ input }) => {
        ...
        try {
          switch (action) {
            case "start":
              ...
              const startContainer = this.dockerService
                .getDockerClient()
  ...
  getRollbackHistory() {
    return implement(deploymentContract.getRollbackHistory).handler(
      async ({ input }) => {
        this.logger.log(
          `Getting rollback history for service: ${input.serviceId}`
        );
        ...
        try {
          ...
          const maxRetention =
            service.deploymentRetention?.maxSuccessfulDeployments || 5;
            ...
            .where(
              and(
                ...
                or(
                  ...
                  eq(deployments.status, "cancelled")
                )
              )
            )
            .orderBy(desc(deployments.createdAt));
          ...
          const availableDeployments = successfulDeployments.slice(
            0,
            maxRetention
          );
  ...
  previewCleanup() {
    return implement(deploymentContract.previewCleanup).handler(
      async ({ input }) => {
        this.logger.log(`Previewing cleanup for service: ${input.serviceId}`);

        try {
          const preview = await this.deploymentCleanupService.previewCleanup(
            input.serviceId
          );
          ...
          this.logger.error(`Failed to preview cleanup: ${error}`);
          throw error;
        }
      }
    );
  ...
  triggerCleanup() {
    return implement(deploymentContract.triggerCleanup).handler(
      async ({ input }) => {
        this.logger.log(`Triggering cleanup for service: ${input.serviceId}`);

        try {
          const result =
            await this.deploymentCleanupService.cleanupOldDeployments(
              input.serviceId
            );
          ...
          this.logger.error(
            `Failed to trigger cleanup: ${err.message}`,
            err.stack
          );
          throw error;
        }
  ...
  updateRetentionPolicy() {
    return implement(deploymentContract.updateRetentionPolicy).handler(
      async ({ input }) => {
        this.logger.log(
          `Updating retention policy for service: ${input.serviceId}`
        );
        ...
        try {
          ...
          const newPolicy = {
            maxSuccessfulDeployments:
              input.maxSuccessfulDeployments ??
              currentPolicy.maxSuccessfulDeployments ??
              5,
            keepArtifacts:
              input.keepArtifacts ?? currentPolicy.keepArtifacts ?? true,
          ...
          this.logger.log(
            `Updated retention policy for service ${input.serviceId}: ${JSON.stringify(newPolicy)}`
          );
          ...
          const err = error as Error;
          this.logger.error(
            `Failed to update retention policy: ${err.message}`,
            err.stack
          );
          throw error;
        }
