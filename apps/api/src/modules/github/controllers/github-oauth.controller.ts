...
export class GitHubOAuthController {
   ...
   */
  @ApiResponse({ status: 201, description: 'GitHub App setup initiated' })
  @Post('installations/setup')
  setupGitHubApp(@Body() body: GitHubAppCredentialsDto) {
    ...
    const setupToken = `setup_${body.appId}_${String(Date.now())}_${Math.random().toString(36).substring(7)}`
   ...
   */
  @ApiResponse({ status: 302, description: 'Redirect to GitHub App installation URL' })
  @Get('install/:appSlug')
  installApp(
   ...
   */
  @ApiResponse({ status: 302, description: 'Redirect to dashboard after GitHub App installation' })
  @Get('callback')
  async handleCallback(
    ...
    try {
      ...
      const organizationLogin =
        'login' in account
          ? account.login
          : organizationLoginHint ?? `org_${String(installation.id)}`
      ...
      if (registered?.appId && registered.privateKey) {
      ...
      this.logger.log(`Synced ${String(syncedCount)} repositories for ${organizationLogin}`)
      ...
      const redirectTarget = `${dashboardUrl}/dashboard?installation=${organizationLogin}&status=success&synced=${String(syncedCount)}`

      this.logger.log(`Installation complete. Redirecting to: ${redirectTarget}`)
      res.redirect(redirectTarget); return;
    } catch (error) {
   ...
   */
  @ApiResponse({ status: 200, description: 'List of GitHub installations' })
  @Get('installations')
   ...
   */
  @ApiResponse({ status: 200, description: 'GitHub installation details' })
   ...
   * @returns Sync result
   */
  @ApiResponse({ status: 200, description: 'GitHub repositories synced successfully' })
  ...
  private async syncRepositories(
    ...
    try {
      ...
      this.logger.log(`Found ${String(repositories.length)} repositories for ${organizationLogin}`)
      ...
      this.logger.log(`Successfully synced ${String(repositories.length)} repositories for ${organizationLogin}`)
