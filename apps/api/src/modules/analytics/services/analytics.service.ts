...
export class AnalyticsService {
    ...
    getResourceUsage(timeRange: string, resource: string, aggregation: string) {
    ...
    getUserActivity(timeRange: string, userId?: string, action?: string, resource?: string, limit = 100, offset = 0) {
        this.logger.log(`Getting user activity for timeRange: ${timeRange}, userId: ${String(userId)}, action: ${action}, resource: ${resource}`);
        
        const actions = ['login', 'deploy', 'view', 'update', 'delete'] as const;
        const resources = ['project', 'service', 'deployment', 'environment'] as const;
        
        // Helper to get random element with guaranteed return
        const getRandomAction = (): string => actions[Math.floor(Math.random() * actions.length)] ?? 'view';
        const getRandomResource = (): string => resources[Math.floor(Math.random() * resources.length)] ?? 'project';
        
        // Mock user activity data
        const mockData = Array.from({ length: Math.min(limit, 50) }, () => {
            // Compute final values with guaranteed string type
            const finalAction: string = action ?? getRandomAction();
            const finalResource: string = resource ?? getRandomResource();
    ...
    getActivitySummary(period: string, granularity: string, limit: number) {
        this.logger.log(`Getting activity summary for period: ${period}, granularity: ${granularity}, limit: ${String(limit)}`);
    ...
    getApiUsage(timeRange: string, groupBy: string, limit: number) {
        // TODO: Implement API usage analytics
        this.logger.log(`Getting API usage for timeRange: ${timeRange}, groupBy: ${groupBy}, limit: ${String(limit)}`);
        return {
            data: [],
    ...
    getDeploymentUsage(timeRange: string, projectId?: string, userId?: string) {
        // TODO: Implement deployment usage analytics
        this.logger.log(`Getting deployment usage for timeRange: ${timeRange}, projectId: ${String(projectId)}, userId: ${String(userId)}`);
        return {
            data: [],
            summary: {
                totalDeployments: 0,
    ...
    getStorageUsage(timeRange: string, breakdown: string) {
