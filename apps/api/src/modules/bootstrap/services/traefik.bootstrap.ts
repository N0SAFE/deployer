  ...
  constructor(
    ...
    private async performInitialSync(): Promise<void> {
        ...
        try {
            ...
            this.logger.log(`Initial sync completed: ${String(syncResult.successful)}/${String(syncResult.total)} configurations synced successfully`);
            
            if (syncResult.failed > 0) {
                this.logger.warn(`${String(syncResult.failed)} configurations failed to sync during startup`);
            ...
            if (cleanupOnStartup) {
                ...
                if (deletedCount > 0) {
                    this.logger.log(`Removed ${String(deletedCount)} orphaned configuration files during startup`);
                } else {
    ...
    async healthCheck(): Promise<{ 
        ...
        try {
            ...
            return {
                ...
                configCount,
                message: healthy 
                    ? `Traefik is healthy with ${String(configCount)} configuration files`
