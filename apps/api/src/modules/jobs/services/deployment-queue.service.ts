...
import { DeploymentJobData, RollbackJobData, HealthCheckJobData, CleanupJobData } from '../types/deployment-job.types';
...
export class DeploymentQueueService {
  ...
  async addCleanupJob(jobData: CleanupJobData, delay: number = 0): Promise<string> {
  ...
  async getDeploymentJobs(deploymentId: string): Promise<any[]> {
    ...
    return allJobs.filter(job => 
      job.data && job.data.deploymentId === deploymentId
    );
  }
}