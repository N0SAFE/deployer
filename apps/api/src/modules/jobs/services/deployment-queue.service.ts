...
export class DeploymentQueueService {
    ...
    async addDeploymentJob(jobData: DeploymentJobData, priority = 0): Promise<string> {
        ...
        try {
            ...
            this.logger.log(`Deployment job ${String(job.id)} added to queue`);
    ...
    test() {
        void this.deploymentQueue.add("test", { foo: 'bar' })
    }

    async addRollbackJob(jobData: RollbackJobData, priority = 10): Promise<string> {
        ...
        this.logger.log(`Rollback job ${String(job.id)} added to queue`);
        return job.id.toString();
    }
    async addHealthCheckJob(jobData: HealthCheckJobData): Promise<string> {
        this.logger.log(`Adding health check job for deployment ${jobData.deploymentId}`);
    ...
    async addCleanupJob(jobData: CleanupJobData, delay = 0): Promise<string> {
        this.logger.log(`Adding cleanup job for deployment ${jobData.deploymentId}`);
    ...
    async retryFailedJobs(): Promise<number> {
        ...
        this.logger.log(`Retried ${String(failedJobs.length)} failed jobs`);
    ...
    async getDeploymentJobs(deploymentId: string): Promise<any[]> {
        ...
        return allJobs.filter(job => job.data?.deploymentId === deploymentId);
    }
}
