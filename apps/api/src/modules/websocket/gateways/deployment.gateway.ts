...
export class DeploymentWebSocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    broadcastDeploymentProgress(event: DeploymentProgressEvent) {
        ...
        this.logger.debug(`Broadcasted progress update for deployment ${deploymentId}: ${event.stage} ${String(event.progress)}%`);
    }
    ...
    async getRoomStats(): Promise<Record<string, number>> {
        const sockets = await this.server.in('/deployments').fetchSockets();
        const roomStats: Record<string, number> = {};
        for (const remoteSocket of sockets) {
            const socket = this.connectedClients.get(remoteSocket.id);
            if (socket) {
                for (const room of socket.rooms) {
                    if (room !== remoteSocket.id && room !== '/deployments') {
