import { WebSocketGateway, WebSocketServer, SubscribeMessage, MessageBody, ConnectedSocket, type OnGatewayConnection, type OnGatewayDisconnect, type OnGatewayInit } from '@nestjs/websockets';
...
export class DeploymentORPCGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
    ...
    client: Socket) {
        ...
        this.logger.log(`Client ${client.id} subscribed to: ${JSON.stringify({ deploymentId, projectId, serviceId })}`);
    ...
    client: Socket) {
        ...
        // Clear subscriptions
        this.clientSubscriptions.delete(client.id);
        this.logger.log(`Client ${client.id} unsubscribed from: ${JSON.stringify({ deploymentId, projectId, serviceId })}`);
     ...
     */
    broadcastDeploymentStatus(deploymentId: string, projectId: string, serviceId: string, data: unknown) {
        const event = {
     ...
     */
    broadcastDeploymentLogs(deploymentId: string, logs: unknown[]) {
        const event = {
     ...
     */
    sendToClient(clientId: string, event: string, data: Record<string, unknown>) {
