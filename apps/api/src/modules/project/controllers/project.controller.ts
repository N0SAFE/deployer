...
export class ProjectController {
  ...
  private transformEnvironmentToContract(dbEnv: DatabaseEnvironment) {
    ...
    const deploymentConfig = dbEnv.deploymentConfig ? {
      strategy: mapDeploymentStrategy((dbEnv.deploymentConfig as { deploymentStrategy?: string }).deploymentStrategy || 'rolling'),
      healthCheckPath: '/health',
      healthCheckTimeout: 30,
      deploymentTimeout: (dbEnv.deploymentConfig as { deployTimeoutMinutes?: number }).deployTimeoutMinutes ? 
        (dbEnv.deploymentConfig as { deployTimeoutMinutes: number }).deployTimeoutMinutes * 60 : 600,
      replicas: (dbEnv.deploymentConfig as { maxInstances?: number }).maxInstances || 1,
      resources: {
        cpu: (dbEnv.resourceLimits as { cpu?: string } | null)?.cpu || '0.5',
        memory: (dbEnv.resourceLimits as { memory?: string } | null)?.memory || '512MB',
        storage: (dbEnv.resourceLimits as { storage?: string } | null)?.storage,
      },
    ...
    return {
      ...
      branch: (dbEnv.previewSettings as { sourceBranch?: string } | null)?.sourceBranch,
      isActive: dbEnv.isActive,
      autoDeloy: (dbEnv.deploymentConfig as { autoDeployEnabled?: boolean } | null)?.autoDeployEnabled || false,
      ...
      tags: ((dbEnv.metadata as { tags?: string[] } | null)?.tags) || [],
  ...
  createEnvironment(@Session() session?: UserSession) {
  ...
  updateEnvironment() {
    return implement(projectContract.updateEnvironment).handler(async ({ input: inputData }) => {
      ...
      if (inputData.type) updateData.type = inputData.type;
  ...
  cloneEnvironment(@Session() session?: UserSession) {
  ...
  createVariableTemplate(@Session() session?: UserSession) {
  ...
  updateVariableTemplate() {
    return implement(projectContract.updateVariableTemplate).handler(async ({ input: inputData }) => {
      ...
      if (inputData.variables) updateData.variables = inputData.variables;
    ...
    return implement(projectContract.getEnvironmentStatus).handler(async ({ input }) => {
      ...
      const environment = await db
        ...
        .where(and(
          ...
          eq(environments.projectId, input.id)
        ))
        .limit(1);
