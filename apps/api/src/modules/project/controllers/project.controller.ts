...
import type { EnvironmentDeploymentConfig } from '../interfaces/project.types';
...
export class ProjectController {
    ...
    update() {
        return implement(projectContract.update).handler(async ({ input }) => {
            ...
            return this.projectAdapter.adaptProjectUpdateToContract(updatedProject);
        });
    }
    ...
    getCollaborators() {
        return implement(projectContract.getCollaborators).handler(async ({ input }) => {
            ...
            // Use adapter to transform to contract schema
            return this.projectAdapter.adaptCollaboratorsToContract(collaborators);
        });
    }
    ...
    getEnvironmentConfig() {
        return implement(projectContract.getEnvironmentConfig).handler(({ input }) => {
    ...
    updateEnvironmentConfig() {
        return implement(projectContract.updateEnvironmentConfig).handler(({ input }) => {
    ...
    getDeploymentConfig() {
        return implement(projectContract.getDeploymentConfig).handler(({ input }) => {
    ...
    updateDeploymentConfig() {
        return implement(projectContract.updateDeploymentConfig).handler(({ input }) => {
            this.logger.log(`Updating deployment config for project: ${input.id}`);
            
            // Use adapter to return updated deployment config
            // TODO: Update actual config in database when available
            return this.projectAdapter.adaptDeploymentConfigToContract(input);
        });
    }
    @Implement(projectContract.getSecurityConfig)
    getSecurityConfig() {
        return implement(projectContract.getSecurityConfig).handler(({ input }) => {
    ...
    updateSecurityConfig() {
        return implement(projectContract.updateSecurityConfig).handler(({ input }) => {
            this.logger.log(`Updating security config for project: ${input.id}`);
            
            // Use adapter to return updated security config
            // TODO: Update actual config in database when available
            return this.projectAdapter.adaptSecurityConfigToContract(input);
        });
    }
    @Implement(projectContract.getResourceConfig)
    getResourceConfig() {
        return implement(projectContract.getResourceConfig).handler(({ input }) => {
    ...
    updateResourceConfig() {
        return implement(projectContract.updateResourceConfig).handler(({ input }) => {
            this.logger.log(`Updating resource config for project: ${input.id}`);
            
            // Use adapter to return updated resource config
            // TODO: Update actual config in database when available
            return this.projectAdapter.adaptResourceLimitsToContract(input);
        });
    }
    @Implement(projectContract.getNotificationConfig)
    getNotificationConfig() {
        return implement(projectContract.getNotificationConfig).handler(({ input }) => {
    ...
    updateNotificationConfig() {
        return implement(projectContract.updateNotificationConfig).handler(({ input }) => {
            this.logger.log(`Updating notification config for project: ${input.id}`);
            
            // Use adapter to return updated notification config
            // TODO: Update actual config in database when available
            return this.projectAdapter.adaptNotificationConfigToContract(input);
        });
    }
    ...
    session?: UserSession) {
        return implement(projectContract.createEnvironment).handler(async ({ input: inputData }) => {
            ...
            const newEnvironment = await this.projectService.createEnvironment({
                ...
                description: '',
                status: 'active',
                deploymentConfig: inputData.deploymentConfig as EnvironmentDeploymentConfig | undefined,
    ...
    resolveVariables() {
        return implement(projectContract.resolveVariables).handler(({ input }) => {
    ...
    getAvailableVariables() {
        return implement(projectContract.getAvailableVariables).handler(({ input }) => {
