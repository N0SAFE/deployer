...
export class ServiceController {
    ...
    addDependency() {
        return implement(serviceContract.addDependency).handler(async ({ input }) => {
            ...
            const dependency = await this.serviceService.addServiceDependency(input.id, input.dependsOnServiceId, input.isRequired);
            if (!dependency) {
                throw new Error('Failed to create service dependency');
            }
            return dependency;
        });
    }
    ...
    toggleActive() {
        return implement(serviceContract.toggleActive).handler(async ({ input }) => {
            this.logger.log(`Toggling active status for service: ${input.id} to ${String(input.isActive)}`);
