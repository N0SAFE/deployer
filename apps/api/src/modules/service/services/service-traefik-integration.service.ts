...
import type { CreateServiceData, UpdateServiceData } from '@/core/modules/service/repositories/service.repository';

// Internal type for service data with common fields
...
export class ServiceTraefikIntegrationService {
    ...
    async createServiceWithTraefik(data: CreateServiceData) {
        ...
        this.createTraefikConfigForService(service);
        
        return service;
    }

    async updateServiceWithTraefik(id: string, data: UpdateServiceData) {
        ...
        if (data.port && data.port !== existingService.port) {
            this.updateTraefikConfigForService(updatedService);
        }
    ...
    async deleteServiceWithTraefik(id: string) {
        ...
        this.cleanupTraefikConfigForService(service);
        
        // Delete the service
        return await this.serviceService.deleteService(id);
    }

    private createTraefikConfigForService(service: ServiceData): void {
    ...
    private updateTraefikConfigForService(service: ServiceData): void {
    ...
    private cleanupTraefikConfigForService(service: ServiceData): void {
