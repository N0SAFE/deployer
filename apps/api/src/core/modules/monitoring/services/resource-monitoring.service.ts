...
export class ResourceMonitoringService {
    ...
    async collectResourceMetrics() {
        try {
            ...
            const totalSystemMetrics: SystemMetrics = {
                totalCpuUsage: 0,
                totalMemoryUsage: 0,
                totalMemoryLimit: 0,
                totalStorageUsage: 0,
            ...
            this.logger.log(`Resource metrics collection completed. Collected ${String(allMetrics.length)} metrics`);
        }
        catch (error) {
    ...
    private async collectStackMetrics(stackId: string, stackServices: any[]): Promise<ResourceMetrics[]> {
        ...
        try {
            ...
            for (const containerInfo of containers) {
                try {
                    const stats = await this.dockerService.getContainerStats(containerInfo.Id);
    ...
    private async storeMetrics(metrics: ResourceMetrics[]): Promise<void> {
        try {
            ...
            this.logger.log(`Stored ${String(dbMetrics.length)} metrics in database`);
        }
        catch (error) {
        ...
        for (const metric of metrics) {
            ...
            if (metric.memoryLimit > 0) {
                ...
                if (memoryPercentage >= this.thresholds.memory.critical) {
                    alerts.push({
                        ...
                        alertType: 'memory',
                        severity: 'critical',
                        message: `Critical memory usage: ${memoryPercentage.toFixed(2)}% (${String(metric.memoryUsage)}MB/${String(metric.memoryLimit)}MB)`,
                    ...
                    alerts.push({
                        ...
                        alertType: 'memory',
                        severity: 'warning',
                        message: `High memory usage: ${memoryPercentage.toFixed(2)}% (${String(metric.memoryUsage)}MB/${String(metric.memoryLimit)}MB)`,
            ...
            if (metric.storageUsage >= this.thresholds.storage.critical) {
                alerts.push({
                    ...
                    alertType: 'storage',
                    severity: 'critical',
                    message: `Critical storage usage: ${String(metric.storageUsage)}MB`,
    ...
    private async storeAlerts(alerts: ResourceAlert[]): Promise<void> {
        try {
            ...
            this.logger.log(`Stored ${String(dbAlerts.length)} resource alerts`);
        }
        catch (error) {
     ...
     */
    async getStackMetrics(stackId: string, hours = 24): Promise<any[]> {
        try {
     ...
     * Get system metrics
     */
    async getSystemMetrics(serviceId: string, hours = 24): Promise<any[]> {
        try {
