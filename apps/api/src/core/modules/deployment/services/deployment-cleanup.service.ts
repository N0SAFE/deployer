...
export class DeploymentCleanupService {
    ...
    async cleanupOldDeployments(serviceId: string): Promise<CleanupResult> {
        ...
        try {
            ...
            if (successfulDeployments.length <= maxDeployments) {
                this.logger.log(
                    `Service ${serviceId} has ${String(successfulDeployments.length)} successful deployments ` +
                    `(max: ${String(maxDeployments)}). No cleanup needed.`
                );
                return {
                    serviceId,
                    deletedCount: 0,
                    ...
                    message: `Keeping all ${String(successfulDeployments.length)} deployments (within retention limit)`,
                };
            }
            ...
            this.logger.log(
                `Service ${serviceId}: Keeping ${String(deploymentsToKeep.length)} deployments, ` +
                `deleting ${String(deploymentsToDelete.length)} old deployments`
            );
            ...
            const result: CleanupResult = {
                ...
                message: `Cleaned up ${String(deletedIds.length)} old deployments, kept ${String(deploymentsToKeep.length)} recent deployments`,
            };
    ...
    async cleanupAllServices(): Promise<CleanupResult[]> {
        ...
        try {
            ...
            this.logger.log(`Cleanup completed: ${String(totalDeleted)} deployments deleted across ${String(results.length)} services`);
    ...
    /**
     * Clean up a single deployment by ID
     * Removes containers, images, files, and database record
     */
    async cleanupDeployment(deploymentId: string): Promise<{
        success: boolean;
        message: string;
    }> {
        this.logger.log(`Cleaning up deployment: ${deploymentId}`);
        ...
        try {
            ...
            // Get service to determine builder type
            const service = await this.serviceService.getService(deployment.serviceId);
            const keepArtifacts = service.deploymentRetention?.keepArtifacts !== false;

            // Clean up artifacts (containers, images, files)
            await this.cleanupDeploymentArtifacts(
                deploymentId,
                deployment.containerName,
                deployment.containerImage,
            ...
            this.logger.log(`Successfully cleaned up deployment: ${deploymentId}`);
            return {
                success: true,
                message: `Deployment ${deploymentId} cleaned up successfully`,
            };
        } catch (error) {
            const err = error as Error;
            this.logger.error(
                `Failed to cleanup deployment ${deploymentId}: ${err.message}`,
                err.stack
            );
            return {
                success: false,
                message: `Failed to cleanup deployment: ${err.message}`,
            };
        }
    }
}
