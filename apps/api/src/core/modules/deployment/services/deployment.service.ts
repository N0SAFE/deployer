...
import { DeploymentPhase, type PhaseMetadata } from '@/core/interfaces/deployment-phase';
...
import type { ProviderConfig, DeploymentTrigger, SourceFiles } from '@/core/modules/providers/interfaces/provider.interface';
import type { BuilderConfig } from '@/core/modules/builders/common/services/base-builder.service';
import * as fs from 'fs/promises';
import * as path from 'path';
import type { deploymentStatusEnum, logLevelEnum, deployments, deploymentLogs, services, projects } from '@/config/drizzle/schema';
...
/** Provider configuration from service record */
...
/** Builder configuration from service record */
...
/** Deployment log entry from build process */
interface BuildLog {
    level: string;
    message: string;
    phase?: string;
...
/** Source configuration stored in deployment records */
type SourceConfigData = CreateDeploymentData['sourceConfig'];
...
interface DeploymentConfig {
    ...
    buildArgs?: Record<string, string>;
    port?: number;
    buildConfig?: DockerfileBuildConfig;
    // Node.js specific
    buildCommand?: string;
    startCommand?: string;
    installCommand?: string;
...
/** Build config for Dockerfile generation */
interface DockerfileBuildConfig {
    buildContext?: string;
    dockerfilePath?: string;
    buildArgs?: Record<string, string>;
    installCommand?: string;
...
/** Container health information returned by DockerService */
...
export class DeploymentService {
    ...
    async deployService(
        ...
        try {
            ...
            const typedProviderConfig = providerConfig as StoredProviderConfig | null;
            const typedBuilderConfig = builderConfig as StoredBuilderConfig | null;

            const providerCfg: ProviderConfig = {
                type: provider.type as ProviderConfig['type'],
                config: providerConfig ?? {},
                repository: typedProviderConfig?.repository,
                monorepo: typedProviderConfig?.monorepo,
                cache: typedProviderConfig?.cache,
            };
            ...
            const sourceFiles: SourceFiles = await provider.fetchSource(providerCfg, trigger) as SourceFiles;
            ...
            const builderCfg: BuilderConfig = {
                deploymentId,
                serviceName,
                sourcePath: sourceFiles.localPath,
                environmentVariables: config.environmentVariables,
                port: typedBuilderConfig?.port != null 
                    ? (typeof typedBuilderConfig.port === 'string' 
                        ? parseInt(typedBuilderConfig.port, 10) 
                        : typedBuilderConfig.port)
                    : undefined,
            ...
            // IBuilder interface deploy accepts BuilderConfig & Record<string, unknown>
            const builderResult = await builder.deploy(builderCfg as BuilderConfig & Record<string, unknown>);

            // Cleanup source files
            try {
                await sourceFiles.cleanup();
            } catch (cleanupError) {
                this.logger.warn(`Failed to cleanup source files: ${String(cleanupError)}`);
            }
    ...
    public async deployStaticSite(
        ...
        if (!finalSubdomain) {
            ...
            try {
                // Get service with project info - row.proj is typed from the join
                const row = await this.serviceRepository.findByNameWithProject(serviceName);
                if (row) {
                    // row.proj is SelectProject type from inner join
                    const typedProject = row.proj as SelectProject;
                    const projectName: string = typedProject.name;
    ...
    public async deployDockerService(config: DeploymentConfig): Promise<DeploymentResult> {
        ...
        const healthCheckUrl = config.port ? 
            `http://localhost:${String(config.port)}${healthCheckPath}` : 
     ...
     */
    private async verifyContainerHealth(
        containerId: string,
        healthCheckUrl?: string,
        options?: {
            maxAttempts?: number;
        ...
        while (attempt < maxAttempts) {
            ...
            try {
                const containerHealthy = await this.dockerService.checkContainerHealth(containerId);
                let httpHealthy = !healthCheckUrl;
                ...
                if (healthCheckUrl) {
                    try {
                        ...
                        if (!httpHealthy) {
                            lastError = `HTTP ${String(response.status)} ${response.statusText}`;
                        }
                    } catch (error) {
                        lastError = error;
                        httpHealthy = false;
                        this.logger.warn(
                            `Health check request failed (attempt ${String(attempt)}/${String(maxAttempts)}) for ${healthCheckUrl}: ${error instanceof Error ? error.message : String(error)}`
                        );
                    }
                }

                if (containerHealthy && httpHealthy) {
                    if (attempt > 1) {
                        this.logger.log(
                            `Container ${containerId} became healthy after ${String(attempt)} attempts${healthCheckUrl ? ` (${healthCheckUrl})` : ''}`
                        );
                    }
                    return true;
                }

                const reason = lastError instanceof Error ? lastError.message : (typeof lastError === 'string' ? lastError : undefined);
                this.logger.warn(
                    `Health check attempt ${String(attempt)}/${String(maxAttempts)} failed for container ${containerId}${healthCheckUrl ? ` (${healthCheckUrl})` : ''}: containerHealthy=${String(containerHealthy)}, httpHealthy=${String(httpHealthy)}${reason ? `, lastError=${reason}` : ''}`
                );
            } catch (error) {
                lastError = error;
                this.logger.error(`Error verifying container health ${containerId} on attempt ${String(attempt)}/${String(maxAttempts)}:`, error);
            }
        ...
        this.logger.error(
            `Container ${containerId} failed health verification after ${String(maxAttempts)} attempts${healthCheckUrl ? ` (${healthCheckUrl})` : ''}${finalErrorMessage ? ` - last error: ${finalErrorMessage}` : ''}`
        );
        return false;
    }
    ...
    private generateNodejsDockerfile(buildConfig?: DeploymentConfig['buildConfig']): string {
        ...
        const portValue = buildConfig?.port ?? 3000;
        const port = typeof portValue === 'string' ? portValue : String(portValue);
        const healthCheckPath = buildConfig?.healthCheckPath ?? '/health';
...
CMD ["sh", "-c", "${startCommand}"]
    ...
    private generatePythonDockerfile(buildConfig?: DeploymentConfig['buildConfig']): string {
        ...
        const portValue = buildConfig?.port ?? 8000;
        const port = typeof portValue === 'string' ? portValue : String(portValue);
        const healthCheckPath = buildConfig?.healthCheckPath ?? '/health';
...
CMD ["sh", "-c", "${startCommand}"]
    ...
    async createDeployment(data: CreateDeploymentData): Promise<string> {
        ...
        const deploymentId = await this.databaseService.transaction(async (tx) => {
            const deployment = await this.deploymentRepository.create(insertData, tx);
    ...
    async updateDeploymentPhase(
        ...
        this.logger.log(`Updating deployment ${deploymentId} phase to ${phase} (${String(progress)}%)`);
        ...
        await this.addDeploymentLog(deploymentId, {
            level: 'info',
            message: `Phase transition: ${phase} (${String(progress)}%)`,
            phase,
            timestamp: new Date(),
            metadata,
        });
    ...
    async addDeploymentLog(deploymentId: string, logData: DeploymentLogData, db?: Database): Promise<void> {
    ...
    async cleanupOldDeployments(olderThanDays = 30): Promise<number> {
        ...
        this.logger.log(`Cleaned up ${String(deletedCount)} old deployments`);
        return deletedCount;
    }
    ...
    private async updateDeploymentHealthStatus(
        deploymentId: string, 
        status: 'healthy' | 'degraded' | 'unhealthy' | 'unknown', 
        containers: { containerId: string; health: ContainerHealthInfo }[]
    ): Promise<void> {
        try {
    ...
    async restartUnhealthyContainers(deploymentId: string): Promise<{
        ...
        try {
            ...
            const errors: { containerId: string; error: string }[] = [];
    ...
    async getPreviewDeploymentsByTrigger(
        branchName?: string, 
        pullRequestNumber?: number
    ): Promise<(SelectDeployment & { previewEnvironment?: Record<string, unknown> | null })[]> {
        ...
        return allDeployments.deployments
            .filter(deployment => {
                const sourceConfig = deployment.sourceConfig as SourceConfigData | null;
    ...
    private async cleanupDeploymentResources(deploymentId: string, status: DeploymentStatus): Promise<void> {
        try {
            ...
            this.logger.log(`Found ${String(containers.length)} container(s) for deployment ${deploymentId}`);
    ...
    private async resumeHealthCheck(
        ...
        try {
            ...
            const phaseMetadata = (deployment.phaseMetadata ?? {}) as PhaseMetadata;
    ...
    async startRollback(
        ...
        try {
            ...
            await this.deploymentRepository.updateStatus(fromDeploymentId, 'deploying', {
                metadata: { rollbackInProgress: true, rollbackTo: toDeploymentId } as Record<string, unknown>
            });
     ...
     */
    completeRollback(rollbackId: string, _metadata?: Record<string, unknown>): void {
        try {
     ...
     */
    failRollback(rollbackId: string, errorMessage: string): void {
        try {
     ...
     */
    async findStuckDeployments(thresholdMinutes = 30) {
     ...
     */
    async findRecentByServiceId(serviceId: string, limit = 10) {
