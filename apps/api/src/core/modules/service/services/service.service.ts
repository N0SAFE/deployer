...
// Interface for deployment records returned from getAllServiceDeployments
...
export class ServiceService {
    ...
    async listServices(projectId: string, activeOnly = false) {
        this.logger.log(`Listing services for project: ${projectId}, activeOnly: ${String(activeOnly)}`);
    ...
    async updateService(id: string, data: UpdateServiceData) {
        ...
        if (!updatedService) {
            throw new NotFoundException(`Service not found after update: ${id}`);
        }
    ...
    async getServiceLogs(
        ...
        this.logger.log(`Getting logs for service: ${serviceId}, deployment: ${String(deploymentId)}`);
        ...
        if (!targetDeploymentId) {
            ...
            if (activeDeployments.length > 0 && activeDeployments[0]) {
    ...
    async listServicesByProject(input: {
        ...
        const servicesWithStats = result.services.map(({ service, healthCheckConfig, project }) => {
            ...
            // Cast Map.get() results with proper type assertions
            const latestDeployment = (result.latestDeployments.get(service.id) ?? null) as {
                id: string;
                status: string;
                environment: string;
                createdAt: Date;
            ...
            const traefikConfig = (traefikConfigs.get(service.id) ?? null) as TraefikConfig;
    ...
    async toggleServiceActive(id: string, isActive: boolean) {
        this.logger.log(`Toggling service active state: ${id} -> ${String(isActive)}`);
    ...
    async getServiceHealthStatus(id: string) {
        ...
        if (allDeployments.length > 0) {
            ...
            const deploymentsByEnv = new Map<string, DeploymentRecord[]>();
