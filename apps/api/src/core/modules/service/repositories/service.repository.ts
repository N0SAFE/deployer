...
export class ServiceRepository {
    ...
    async findDeploymentLogs(deploymentId: string, filter: ServiceLogsFilter = {}, limit = 100, offset = 0) {
        ...
        if (filter.level) {
            conditions.push(eq(deploymentLogs.level, filter.level));
        }
        if (filter.phase) {
    ...
    async findServicesByProject(projectId: string, activeOnly = false) {
        this.logger.log(`Finding services for project: ${projectId}, activeOnly: ${String(activeOnly)}`);
    ...
    async getProjectDependencyGraph(projectId: string) {
        ...
        const latestDeploymentByService = latestDeployments.reduce<Record<string, typeof latestDeployments[0]>>((acc, deployment) => {
    ...
    async listServicesByProjectWithStats(
        ...
        if (serviceIds.length > 0) {
            ...
            deploymentCountMap = new Map(deploymentCounts.map(dc => [dc.serviceId, dc.count]));
            dependencyCountMap = new Map(dependencyCounts.map(dc => [dc.serviceId, dc.count]));
        }
    ...
    async getServiceWithDetails(serviceId: string) {
        ...
        const deploymentCount = deploymentCountResult[0]?.count ?? 0;
        ...
        const dependencyCount = dependencyCountResult[0]?.count ?? 0;
    ...
    async getServiceDeployments(
        ...
        if (filters.environment) {
            conditions.push(eq(deployments.environment, filters.environment as 'production' | 'staging' | 'preview' | 'development'));
        }
        if (filters.status) {
            conditions.push(eq(deployments.status, filters.status as 'pending' | 'queued' | 'building' | 'deploying' | 'success' | 'failed' | 'cancelled'));
        }
    ...
    async updateTraefikConfig(serviceId: string, data: Partial<{
        domain: string;
        subdomain: string | null;
        fullDomain: string;
        sslEnabled: boolean;
        sslProvider: 'letsencrypt' | 'selfsigned' | 'custom' | null;
        pathPrefix: string | null;
        port: number;
        middleware: unknown;
        healthCheck: { enabled: boolean; path: string; interval?: number; timeout?: number };
        isActive: boolean;
        ...
        const updateData: typeof data & { updatedAt: Date; fullDomain?: string } = {
            updatedAt: new Date(),
            ...data,
        };
        ...
        if (data.domain !== undefined || data.subdomain !== undefined) {
            ...
            if (existing) {
                const domain: string = data.domain ?? existing.domain;
                const subdomain: string | null = data.subdomain ?? existing.subdomain;
