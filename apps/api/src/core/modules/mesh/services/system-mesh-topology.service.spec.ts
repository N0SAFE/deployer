...
describe("SystemMeshTopologyService", () => {
    ...
    beforeEach(() => {
        ...
        meshEventService = {
            ...
            emitTopology: vi.fn((_clusterId, event) => {
        ...
        const result = service.reconcileMembership({
            ...
            snapshot: {
                ...
                nodes: [
                    {
                        nodeId: remoteNodeId,
                        clusterId: localSnapshot.localNode.clusterId,
                        region: "eu-central",
                        roles: ["relay"],
                        lifecycleState: "healthy",
                        routingMode: "balanced",
    ...
    describe("six-node integration", () => {
        const createNodeService = (clusterId: string) => {
            process.env.MESH_NODE_ID = randomUUID();
            process.env.MESH_CLUSTER_ID = clusterId;
        ...
        it("plans direct owner routes from a six-instance discovered mesh", () => {
            ...
            const nodes = Array.from({ length: 6 }, () => createNodeService(clusterId));
        ...
        it("recovers best route after primary owner disconnects then resumes", () => {
            const clusterId = randomUUID();
            const requester = createNodeService(clusterId);
            const ownerA = createNodeService(clusterId);
            const ownerB = createNodeService(clusterId);
            const streamId = randomUUID();
