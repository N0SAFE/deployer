...
export class StorageService {
    ...
    async listFiles(path = '/', recursive = false, includeHidden = false): Promise<FileMetadata[]> {
    ...
    async uploadFile(filePath: string, content: string, overwrite = false, permissions?: string): Promise<FileMetadata> {
        const fullPath = this.resolvePath(filePath);
        try {
    ...
    async moveFile(sourcePath: string, destinationPath: string, overwrite = false): Promise<FileMetadata> {
    ...
    async copyFile(sourcePath: string, destinationPath: string, overwrite = false): Promise<FileMetadata> {
        const sourceFullPath = this.resolvePath(sourcePath);
        const destFullPath = this.resolvePath(destinationPath);
        try {
            // Check if destination exists and overwrite is false
    ...
    async createBackup(databaseName: string, backupType: 'full' | 'incremental' | 'differential' = 'full', compression = true, _encryption = false, metadata?: Record<string, any>): Promise<DatabaseBackup> {
        const backupId = crypto.randomUUID();
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const fileName = `${databaseName}_${backupType}_${timestamp}.sql${compression ? '.gz' : ''}`;
        const filePath = path.join(this.constantsService.constants.FOLDER_LOCATION.BACKUP_BASE_PATH, fileName);
        ...
        try {
            ...
            await fs.mkdir(this.constantsService.constants.FOLDER_LOCATION.BACKUP_BASE_PATH, { recursive: true });
    ...
    listBackups(databaseName?: string, status?: 'in-progress' | 'completed' | 'failed' | 'cancelled', limit = 20, offset = 0): {
    ...
    getStorageUsage(pathToCheck?: string): StorageUsage[] {
    ...
    private resolvePath(relativePath: string): string {
        const basePath = path.resolve(this.constantsService.constants.FOLDER_LOCATION.STORAGE_BASE_PATH);
        const providedPath = relativePath.trim();

        // Remove leading slashes to treat input as relative (unless intentionally empty/root)
        const relativeCleanPath = providedPath.replace(/^\/+/, '');

        // Resolve against the base path to collapse any `..` segments or absolute overrides
        const resolvedPath = path.resolve(basePath, relativeCleanPath);

        // Enforce containment: resolved path must stay within the configured storage base path
        const isBasePath = resolvedPath === basePath;
        const isChildPath = resolvedPath.startsWith(`${basePath}${path.sep}`);

        if (!isBasePath && !isChildPath) {
            throw new Error(`Access to path outside storage base is denied: ${relativePath}`);
        }
