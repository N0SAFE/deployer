...
export class FileUploadService {
    ...
    async processUploadedFile(file: UploadedFile): Promise<FileUploadResult> {
        try {
            ...
            const uploadPath = path.join(this.constantsService.constants.FOLDER_LOCATION.UPLOAD_DIR, `${uploadId}_${file.originalname}`);
            const extractPath = path.join(this.constantsService.constants.FOLDER_LOCATION.EXTRACT_DIR, uploadId);
            this.logger.log(`Processing uploaded file: ${file.originalname} (${String(file.size)} bytes)`);
            // Save uploaded file
            ...
            this.logger.log(`File processed successfully: ${uploadId} (${String(fileCount)} files extracted)`);
            return result;
        }
        catch (error) {
    ...
    private validateFile(file: UploadedFile): void {
        ...
        if (file.size > this.constantsService.constants.UPLOAD_FILE.MAX_FILE_SIZE) {
            throw new BadRequestException(`File too large. Maximum size is ${String(this.constantsService.constants.UPLOAD_FILE.MAX_FILE_SIZE / 1024 / 1024)}MB`);
        }
        if (!this.constantsService.constants.UPLOAD_FILE.ALLOWED_MIME_TYPES.includes(file.mimetype)) {
            throw new BadRequestException(`Invalid file type. Allowed types: ${this.constantsService.constants.UPLOAD_FILE.ALLOWED_MIME_TYPES.join(', ')}`);
        }
    ...
    private async extractArchive(archivePath: string, extractPath: string, mimeType: string): Promise<number> {
        ...
        try {
            ...
            this.logger.log(`Extracted ${String(fileCount)} files from ${archivePath}`);
            return fileCount;
        }
        catch (error) {
        ...
        return new Promise((resolve, reject) => {
            ...
            yauzl.open(archivePath, { lazyEntries: true }, (err, zipfile) => {
                if (err)
                    { reject(err); return; }
                if (!zipfile)
                    { reject(new Error('Failed to open ZIP file')); return; }
                zipfile.readEntry();
                zipfile.on('entry', (entry) => {
                    ...
                    zipfile.openReadStream(entry, (err, readStream) => {
                        if (err)
                            { reject(err); return; }
                        if (!readStream)
                            { reject(new Error('Failed to create read stream')); return; }
    ...
    private async analyzeExtractedFiles(extractPath: string): Promise<FileUploadResult['metadata']> {
        ...
        try {
            ...
            this.logger.log(`File analysis complete: type=${metadata.detectedType}, files=${String(fileCount)}`);
            return metadata;
        }
        catch (error) {
     ...
     */
    async cleanupOldUploads(olderThanDays = 7): Promise<number> {
        let cleanedCount = 0;
        try {
            const cutoffTime = Date.now() - (olderThanDays * 24 * 60 * 60 * 1000);
            const uploads = await fs.readdir(this.constantsService.constants.FOLDER_LOCATION.EXTRACT_DIR);
            for (const uploadId of uploads) {
                const uploadPath = path.join(this.constantsService.constants.FOLDER_LOCATION.EXTRACT_DIR, uploadId);
            ...
            this.logger.log(`Cleaned up ${String(cleanedCount)} old uploads`);
            return cleanedCount;
        }
        catch (error) {
    ...
    async createDeploymentBundle(uploadId: string): Promise<Buffer> {
        try {
            const extractPath = path.join(this.constantsService.constants.FOLDER_LOCATION.EXTRACT_DIR, uploadId);
            const bundlePath = path.join(this.constantsService.constants.FOLDER_LOCATION.UPLOAD_DIR, `${uploadId}-bundle.tar.gz`);
