...
import type { orchestrationStacks, serviceInstances } from "@/config/drizzle/schema/index";
...
export class HealthCheckService {
  ...
  async performHealthChecks() {
    try {
      ...
      this.logger.log(
        `Health checks completed. Total: ${String(totalChecks)}, Healthy: ${String(healthyServices)}, Unhealthy: ${String(unhealthyServices)}`
      );
    } catch (error) {
  ...
  private async checkServiceHealth(
    stack: InferSelectModel<typeof orchestrationStacks>,
    service: InferSelectModel<typeof serviceInstances>,
    ...
    try {
      ...
      try {
        ...
        result.metadata = {
          ...
          contentLength: typeof response.headers["Content-Length"] === "string"
        ...
        if (error.code === "ECONNABORTED") {
          ...
          result.errorMessage = `Request timeout after ${String(config.timeout)}s`;
        } else if (error.response) {
          result.status = "unhealthy";
          result.statusCode = error.response.status;
          result.errorMessage = `HTTP ${String(error.response.status)}: ${error.response.statusText}`;
        } else {
          result.status = "error";
      ...
      result.status = "error";
      result.errorMessage = error instanceof Error ? error.message : "Health check failed";
      this.logger.error(
   ...
   */
  private buildServiceEndpoint(service: InferSelectModel<typeof serviceInstances>, healthCheckPath: string): string {
    try {
      ...
      if (service.domainAssignments?.external) {
        ...
        return `${protocol}://${external.domain}:${String(external.port)}${healthCheckPath}`;
      }
      // Try internal hostname
      if (service.domainAssignments?.internal) {
        const internal = service.domainAssignments.internal;
        return `http://${internal.hostname}:${String(internal.port)}${healthCheckPath}`;
      }
      // Try service name with default port
      if (service.metadata?.ports.length && service.metadata.ports.length > 0) {
        const port = service.metadata.ports[0];
        return `http://${service.serviceName}:${String(port)}${healthCheckPath}`;
      }
      return "";
    } catch (error) {
      this.logger.error(
   ...
   */
  private extractHealthCheckConfig(service: InferSelectModel<typeof serviceInstances>): HealthCheckConfig {
  ...
  private async generateHealthAlert(result: HealthCheckResult): Promise<void> {
    try {
      ...
      if (!result.serviceId) {
        return; // Skip stack-level results without serviceId
      }
  ...
  private async updateStackHealthStatus(stackId: string): Promise<void> {
    try {
      ...
      const serviceHealthMap = new Map<string, typeof recentHealthChecks[0]>();
      recentHealthChecks.forEach((check) => {
        if (check.metadata?.type === "health-check") {
          const serviceId = check.serviceId;
          if (!serviceId) return; // Skip checks without serviceId
          const existing = serviceHealthMap.get(serviceId);
          if (
            !existing ||
            check.timestamp > existing.timestamp
          ) {
      ...
      serviceHealthMap.forEach((check) => {
        totalCount++;
        if ((check.metadata as Record<string, unknown>).status === "healthy") {
          healthyCount++;
        }
      });
      ...
      // Update stack health status
      await this.healthCheckRepository.updateStackHealth(stackId, {
        stackHealthStatus,
    ...
    try {
      ...
      if (lastHourChecks.length > 0) {
        const totalResponseTime = lastHourChecks.reduce(
          (sum, check) => {
            const metadata = check.metadata as Record<string, unknown> | null;
            return sum + (Number(metadata?.responseTime) || 0);
          },
          0
        );
      ...
      if (!healthCheckResults[0]) {
        throw new Error("No health check results found");
      }
      return {
        stackId: service.stackId,
        serviceId: service.id,
  ...
  async resolveRecoveredHealthAlerts() {
    try {
      ...
      for (const alert of activeHealthAlerts) {
        try {
          // Skip stack-level alerts without serviceId
          if (!alert.serviceId) {
            continue;
          }
          ...
          const healthyChecks = recentHealthChecks.filter(
            (check) =>
              ...
              (check.metadata as Record<string, unknown>).status === "healthy"
          );
      ...
      if (resolvedCount > 0) {
        this.logger.log(
          `Resolved ${String(resolvedCount)} health alerts for recovered services`
        );
      }
    } catch (error) {
