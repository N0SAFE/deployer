...
export class SslCertificateService {
    ...
    async monitorCertificateExpiry() {
        try {
            ...
            for (const cert of expiringCertificates) {
                ...
                const daysUntilExpiry = Math.ceil((new Date(expiresAt).getTime() - Date.now()) / (24 * 60 * 60 * 1000));
                if (daysUntilExpiry <= 7) {
                    // Renew certificates expiring in 7 days or less
                    this.logger.log(`Certificate for ${cert.domain} expires in ${String(daysUntilExpiry)} days, initiating renewal`);
                    await this.renewCertificate(cert.domain);
                } else if (daysUntilExpiry <= 30) {
                    // Log warning for certificates expiring within 30 days
                    this.logger.warn(`Certificate for ${cert.domain} expires in ${String(daysUntilExpiry)} days`);
                }
            }
            this.logger.log(`Certificate monitoring completed. Found ${String(expiringCertificates.length)} certificates to monitor`);
        } catch (error) {
    ...
    async validateCertificateFiles() {
        try {
            ...
            this.logger.log(`Certificate validation completed. Validated: ${String(validatedCount)}, Errors: ${String(errorCount)}`);
        } catch (error) {
    ...
    async parseCertificateFile(certificatePath: string | null): Promise<CertificateInfo | null> {
        ...
        try {
            ...
            const sanExtension = cert.getExtension("subjectAltName") as { altNames: { type: number; value: string }[] };
     ...
     */
    async createCertificateRecord(config: { domain: string; projectId: string; issuer: string; autoRenew: boolean }): Promise<void> {
        try {
     ...
     */
    async getCertificatesExpiringSoon(days = 30): Promise<any[]> {
        try {
            ...
            return expiringCertificates.map((cert) => {
                const expiresIn = cert.expiresAt ? Math.ceil((new Date(cert.expiresAt).getTime() - Date.now()) / (24 * 60 * 60 * 1000)) : null;
                return {
                    ...cert,
