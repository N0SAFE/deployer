...
export abstract class BaseProviderService implements IDeploymentProvider {
    ...
    protected async retryWithBackoff<T>(
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                this.debug(`${operationName} (attempt ${String(attempt)}/${String(maxRetries)})`);
                return await operation();
            } catch (error) {
                lastError = error as Error;
                this.warn(`${operationName} failed (attempt ${String(attempt)}/${String(maxRetries)})`, error as Error);
        ...
        this.error(`${operationName} failed after ${String(maxRetries)} attempts`, lastError);
        throw lastError;
    }
