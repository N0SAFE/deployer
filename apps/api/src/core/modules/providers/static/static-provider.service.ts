...
export class StaticProviderService extends BaseProviderService implements IDeploymentProvider, IProvider {
    ...
    async fetchSource(config: ProviderConfig, trigger: DeploymentTrigger): Promise<SourceFiles> {
        return this.executeWithLogging(
            'fetchSource',
            async () => {
                const sourcePath: string = (config.repository?.url ?? trigger.data?.sourcePath) as string;
                ...
                const sourceId = `static-${String(Date.now())}-${this.generateId()}`;
                ...
                const version = trigger.data?.version;
                return {
                    sourceId,
                    localPath: sourcePath,
                    metadata: {
                        provider: 'static' as const,
                        version: typeof version === 'string' ? version : new Date().toISOString(),
     ...
     */
    async validateConfig(config: ProviderConfig | Record<string, any>): Promise<{ valid: boolean; errors: string[] }> {
        const errors: string[] = [];

        // Support both ProviderConfig and raw config format
        const configAsProviderConfig = config as ProviderConfig;
        const configAsRaw = config as Record<string, any>;
        const providerConfigObj = configAsProviderConfig.config as Record<string, any> | undefined;
        const sourcePath: unknown = configAsProviderConfig.repository?.url ?? configAsRaw.sourcePath ?? providerConfigObj?.defaultPath;
    ...
    getConfigSchema(): ConfigSchema {
        return {
            ...
            validate: (config: Record<string, unknown>) => {
                ...
                if (config.customHeaders && typeof config.customHeaders === 'string') {
                    try {
            ...
            transform: (config: Record<string, unknown>) => {
                const sourcePath = typeof config.sourcePath === 'string' ? config.sourcePath : '';
                return {
                    ...
                    config: {
                        ...
                        indexFile: typeof config.indexFile === 'string' ? config.indexFile : 'index.html',
                        spaMode: Boolean(config.enableSPA),
                        customHeaders: typeof config.customHeaders === 'string' && config.customHeaders ? JSON.parse(config.customHeaders) as Record<string, string> : {},
                        compression: typeof config.compressionEnabled === 'boolean' ? config.compressionEnabled : true,
                    },
                };
            },
        };
    ...
    async deployStaticFiles(options: StaticFileDeploymentOptions & { image?: string; imagePullPolicy?: 'IfNotPresent' | 'Always' | 'Never'; registryAuth?: any }): Promise<NginxContainerInfo & { imageUsed?: string }> {
        ...
        // Always use project-level HTTP server model
        {
            ...
            if (sourcePath) {
                const copyMeta = await this.copyFilesDirectly(effectiveProjectId, sourcePath, serviceName, deploymentId);
                this.debug(`Copy completed, probe result: ${copyMeta.probe ? 'available' : 'none'}`);
            } else {
            ...
            try {
                ...
                const errorMsg = symlinkErr instanceof Error ? symlinkErr.message : String(symlinkErr);
                this.error(`Failed to set current symlink for ${serviceName}/${deploymentId}: ${errorMsg}`);
                throw symlinkErr;
            }
            ...
            try {
                ...
                const vhostErrMsg = vhostErr instanceof Error ? vhostErr.message : String(vhostErr);
                this.error(`CRITICAL: Failed to ensure vhost for ${serviceName}@${effectiveProjectId}: ${vhostErrMsg}`);
                throw new Error(`Failed to configure web server for ${serviceName}: ${vhostErrMsg}`);
            }
            ...
            try {
                ...
                const pruneErrMsg = pruneErr instanceof Error ? pruneErr.message : String(pruneErr);
                this.warn(`Prune operation failed for ${serviceName}: ${pruneErrMsg}`);
            }
            ...
            try {
                ...
                const portKeys = Object.keys(exposedPorts);
                const portString = portKeys.length > 0 && portKeys[0] ? portKeys[0].split('/')[0] : '80';
                const containerPort = portString ?? '80';
                ...
                const traefikErrMsg = traefikErr instanceof Error ? traefikErr.message : String(traefikErr);
                this.warn(`Failed to configure Traefik routing for ${serviceName}: ${traefikErrMsg}`);
            }
            ...
            try {
                ...
                this.warn(`Deployment verification failed for ${serviceName}: ${verifyErrMsg}`);
    ...
    async updateStaticFiles(projectId: string, serviceName: string, deploymentId: string, sourcePath?: string): Promise<void> {
        ...
        const projectServer = await this.projectServerService.getProjectServerStatus(projectId);
        if (!projectServer) {
     ...
     */
    async listStaticFileDeployments(projectId: string): Promise<{ serviceName: string; deployments: string[]; current?: string }[]> {
        const projectServer = await this.projectServerService.getProjectServerStatus(projectId);
        ...
        try {
            ...
            const result: { serviceName: string; deployments: string[]; current?: string }[] = [];
            for (const svc of services) {
                ...
                try {
                    ...
                    const innerErrMsg = inner instanceof Error ? inner.message : String(inner);
                    this.warn(`Failed to list deployments for ${svc}: ${innerErrMsg}`);
                }
            }
            return result;
        }
    ...
    private async copyFilesIntoProjectVolume(projectId: string, sourcePath: string, serviceName: string, deploymentId: string): Promise<{ probe?: { exitCode: number; output: string } | null }> {
        ...
        try {
            ...
            try {
                ...
                // Intentionally ignore removal errors - container may already be removed
            }
        }
        ...
        try {
            ...
            this.log(`Probe for copied files in ${volumeName}${targetDirInVolume}: exit=${String(probeResult.exitCode)}`);
            try {
                ...
                this.warn(`Failed to write probe summary into volume (non-fatal): ${writeErrMsg}`);
            }
            return { probe: probeResult };
        } catch (probeErr) {
            const probeErrMsg = probeErr instanceof Error ? probeErr.message : String(probeErr);
            this.warn(`Volume probe failed (non-fatal): ${probeErrMsg}`);
            return { probe: null };
        }
    }
    /**
    ...
    private async verifyDeploymentAccessible(containerName: string, host: string): Promise<void> {
        ...
        try {
            ...
            this.warn(`HTTP verification failed: ${errMsg}`);
    ...
    private async copyFilesDirectly(
        ...
        try {
            ...
            this.log(`✅ Successfully copied ${String(files.length)} files/directories to ${targetDir}`);
            ...
            this.error(`Failed to copy files directly: ${errorMsg}`);
            throw error;
        }
    }
    ...
    private async generateProbeInfo(targetDir: string): Promise<{ exitCode: number; output: string }> {
        try {
            ...
            const output = `files=${String(fileCount)};bytes=${String(totalBytes)};sample=${sample}`;
            return { exitCode: 0, output };
            
        } catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            return { exitCode: 1, output: `Probe failed: ${errorMsg}` };
        }
    }
    ...
    private async pruneOldDeployments(projectId: string, projectContainerName: string, serviceName: string, keep = 5): Promise<void> {
        ...
        const cmd = ['sh', '-c', `cd ${serviceDir} && ls -1dt */ 2>/dev/null | sed 's#/##' | tail -n +${String(keep + 1)} | xargs -r rm -rf`];
        try {
            await this.dockerService.execInContainer(projectContainerName, cmd);
            this.log(`Pruned older deployments for ${serviceName}, keeping ${String(keep)}`);
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.warn(`Failed to prune deployments for ${serviceName}: ${errorMsg}`);
        }
    }
    ...
    private async pruneOldDeploymentsDirect(projectId: string, serviceName: string, keep = 5): Promise<void> {
        ...
        try {
            ...
            const deploymentDirs: { name: string; mtime: Date }[] = [];
            ...
            if (toRemove.length > 0) {
                this.log(`Pruning ${String(toRemove.length)} old deployments for ${serviceName}, keeping ${String(keep)} newest`);
                ...
                for (const { name } of toRemove) {
                    ...
                    try {
                        ...
                        const removeErrMsg = removeError instanceof Error ? removeError.message : String(removeError);
                        this.warn(`Failed to remove old deployment ${deploymentPath}: ${removeErrMsg}`);
                    }
                }
            } else {
                this.debug(`No old deployments to prune for ${serviceName} (${String(deploymentDirs.length)} total, keeping ${String(keep)})`);
            }
