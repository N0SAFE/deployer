...
export class StaticFileServingService {
  ...
  async setupStaticServing(
    deploymentId: string,
    sourcePath: string,
    staticProviderService: IStaticProviderService,
    serviceId?: string,
    projectId?: string,
    domain?: string
  ): Promise<void> {
    try {
      ...
      await staticProviderService.deployStaticFiles({
        serviceName: svcId,
        deploymentId,
        projectId,
      ...
      await fs.copy(sourceDir, targetPath, {
        ...
        filter: (src) => {
          ...
          return !skipPatterns.some((pattern) =>
            pattern.includes("*")
              ? (new RegExp(pattern.replace("*", ".*")).exec(basename))
              : basename === pattern
          );
        },
      });
  ...
  private async generateManifest(
    ...
    try {
      ...
      this.logger.log(
        `Generated manifest for deployment ${deploymentId} (${String(files.length)} files)`
      );
    } catch (error) {
      this.logger.error(
  ...
  private async resolveFilePath(
    ...
    const staticDir = path.join(this.constantsService.constants.FOLDER_LOCATION.STATIC_FILES_DIR, deploymentId);
  ...
  private generateETag(stats: fs.Stats): string {
    return `"${String(stats.size)}-${stats.mtime.getTime().toString(16)}"`;
  }
  /**
  ...
  async getDeploymentManifest(deploymentId: string): Promise<any> {
    try {
      const manifestPath = path.join(
        this.constantsService.constants.FOLDER_LOCATION.STATIC_FILES_DIR,
        deploymentId,
        ".manifest.json"
      );
  ...
  async getServingStats(serviceId: string): Promise<{
    ...
    try {
      ...
      const files = Object.values(manifest as Record<string, { size?: number }>);
