...
export class GithubDeploymentRulesService {
   ...
   */
  matchRule(rule: DeploymentRule, context: RuleMatchContext): RuleMatchResult {
  ...
  private matchPathConditions(
    ...
    if (include && include.length > 0) {
      ...
      return {
        matches: true,
        reason: `${String(includedFiles.length)} file(s) match path conditions`,
      };
    }
  ...
  private evaluateCustomCondition(
    ...
    try {
      ...
      const result = (eval(`(function(ctx) { with(ctx) { return ${condition}; } })`) as (ctx: typeof safeContext) => unknown)(safeContext);
