...
import { GithubDeploymentCacheRepository } from "@/core/modules/git/github";
...
export class GithubDeploymentCacheService {
    ...
    constructor(private readonly cacheRepository: GithubDeploymentCacheRepository) {}

    /**
     * Create a cache entry for a deployment
     */
    async createCacheEntry(input: CreateCacheEntryInput): Promise<CacheEntry> {
        this.logger.log(`Creating cache entry for ${input.repositoryId}@${input.branch} - ${input.commitSha}`);
     ...
     */
    async getLatestCacheEntry(projectId: string, repositoryId: string, branch: string): Promise<CacheEntry | null> {
        const entry = await this.cacheRepository.findLatestByBranch(projectId, repositoryId, branch);
     ...
     */
    async getCacheEntryByCommit(projectId: string, repositoryId: string, commitSha: string): Promise<CacheEntry | null> {
        const entry = await this.cacheRepository.findByCommitSha(projectId, repositoryId, commitSha);
    ...
    async shouldSkipDeployment(
        ...
        switch (cacheStrategy) {
            case "loose": {
                ...
                const hasNewChanges = newWatchedFiles.some((file) => !cachedWatchedFiles.includes(file));
                ...
                return {
                    shouldSkip: false,
                    reason: hasNewChanges ? "New watched files detected" : "First deployment or all watched files changed",
                    cachedEntry: latestCache,
                };
            }
        }
    ...
    async getCacheStats(
        ...
        const entries = await this.cacheRepository.findByRepository(projectId, repositoryId);

        const branches = [...new Set(entries.map((e) => e.branch))];
        const firstEntry = entries[0];
        const lastEntry = entries[entries.length - 1];
        const latestCommit = firstEntry?.commitSha ?? null;
        const oldestEntry = lastEntry?.createdAt ?? null;
        const newestEntry = firstEntry?.createdAt ?? null;
     ...
     */
    async cleanupOldEntries(projectId: string, repositoryId: string, keepPerBranch = 10): Promise<number> {
        const entries = await this.cacheRepository.findByRepository(projectId, repositoryId);

        // Group by branch
        const entriesByBranch = entries.reduce<Record<string, typeof entries>>((acc, entry) => {
            acc[entry.branch] ??= [];
        ...
        for (const branch in entriesByBranch) {
            ...
            if (branchEntries && branchEntries.length > keepPerBranch) {
        ...
        if (toDelete.length > 0) {
            ...
            this.logger.log(`Cleaned up ${String(toDelete.length)} old cache entries for ${repositoryId}`);
        }
     ...
     */
    async deleteBranchCache(projectId: string, repositoryId: string, branch: string): Promise<void> {
