...
export class GithubChangeDetectionService {
  ...
  async detectChanges(
    ...
    this.logger.debug(`Total changed files: ${String(allChangedFiles.length)}`);
    ...
    this.logger.debug(`Watched files after filtering: ${String(watchedFiles.length)}`);
    ...
    return {
      shouldDeploy: true,
      reason: `${String(watchedFiles.length)} watched file(s) changed`,
      changedFiles: allChangedFiles,
      watchedFiles,
