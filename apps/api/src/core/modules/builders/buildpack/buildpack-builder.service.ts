...
export class BuildpackBuilderService extends BaseBuilderService {
  ...
  async deploy(config: BuildpackBuilderConfig): Promise<BuilderResult> {
    ...
    try {
      ...
      const dockerfile = this.generateDockerfileForLanguage(detectedLanguage, {
        sourcePath,
        installCommand,
        startCommand,
        buildCommand,
   ...
   * Generate Dockerfile based on detected language
   */
  private generateDockerfileForLanguage(
    language: string,
    options: {
      sourcePath: string;
      installCommand?: string;
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \\
  CMD curl -f http://localhost:${String(port)}${options.healthCheckPath} || exit 1
