...
/** UI form input config - all values are strings/numbers from form */
interface StaticFormConfig {
  indexFile?: string;
  errorPage?: string;
  serverImage?: string;
  port?: number | string;
...
interface StaticTransformedConfig {
  indexFile: string;
  errorPage?: string;
  serverImage: string;
  port: number;
...
export class StaticBuilderService extends BaseBuilderService implements IBuilder {
  ...
  getConfigSchema(): ConfigSchema {
    return {
      ...
      validate: (config: StaticFormConfig): Promise<{ valid: boolean; errors: string[] }> => {
        const errors: string[] = [];
      ...
      transform: (config: StaticFormConfig): StaticTransformedConfig => {
        const port = typeof config.port === 'string' ? parseInt(config.port, 10) : config.port;
        return {
   ...
   */
  getDefaultConfig(): StaticFormConfig {
    return {
   ...
   */
  validateConfig(config: StaticFormConfig): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = [];
    const port = typeof config.port === 'string' ? parseInt(config.port, 10) : config.port;
