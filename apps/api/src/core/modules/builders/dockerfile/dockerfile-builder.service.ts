...
/** UI form input config - all values are strings from form */
interface DockerfileFormConfig {
  dockerfilePath?: string;
  buildContext?: string;
  buildArgs?: string;  // JSON string
  target?: string;
  cacheFrom?: string;  // Comma-separated
  platform?: string;
}

/** Transformed config after validation */
interface DockerfileTransformedConfig {
  dockerfilePath: string;
  buildContext: string;
  buildArgs?: Record<string, string>;
  target?: string;
...
export class DockerfileBuilderService extends BaseBuilderService implements IBuilder {
  ...
  getConfigSchema(): ConfigSchema {
    return {
      ...
      validate: (config: DockerfileFormConfig): Promise<{ valid: boolean; errors: string[] }> => {
        const errors: string[] = [];
      ...
      transform: (config: DockerfileFormConfig): DockerfileTransformedConfig => {
        const transformed: DockerfileTransformedConfig = {
          dockerfilePath: config.dockerfilePath ?? 'Dockerfile',
          buildContext: config.buildContext ?? '.',
        };
        
        if (config.buildArgs) {
          transformed.buildArgs = JSON.parse(config.buildArgs) as Record<string, string>;
        }
        ...
        if (config.cacheFrom) {
          transformed.cacheFrom = config.cacheFrom.split(',').map((img) => img.trim());
        }
   ...
   */
  getDefaultConfig(): DockerfileFormConfig {
    return {
   ...
   */
  validateConfig(config: DockerfileFormConfig): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = [];
