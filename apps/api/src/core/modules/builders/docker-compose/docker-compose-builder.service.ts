...
export class DockerComposeBuilderService extends BaseBuilderService {
  ...
  async deploy(config: DockerComposeBuilderConfig): Promise<BuilderResult> {
    ...
    try {
      ...
      if (port) {
        healthCheckUrl = `http://localhost:${String(port)}${healthCheckPath}`;
      ...
      return {
        ...
        healthCheckUrl,
        message: isHealthy
          ? `Docker Compose deployment successful with ${String(containers.length)} containers`
          : `Docker Compose deployment created ${String(containers.length)} containers but health check failed`,
        metadata: {
          composeFile,
  ...
  async teardown(config: {
    ...
    try {
      const composeCmd = `docker-compose -f ${composeFile} -p ${String(projectName)} down -v`;
      await execAsync(composeCmd, { cwd: sourcePath });
      this.logger.log(`Docker Compose deployment ${String(projectName)} torn down successfully`);
    } catch (error) {
      const err = error as Error;
