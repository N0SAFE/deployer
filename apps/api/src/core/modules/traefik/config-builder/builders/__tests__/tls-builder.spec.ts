...
describe('TLSBuilder', () => {
  describe('Certificates', () => {
    ...
    it('should support variables in certificate paths', () => {
      ...
      expect(config.certificates?.at(0)?.certFile).toBe('~##certPath##~');
      expect(config.certificates?.at(0)?.keyFile).toBe('~##keyPath##~');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should support all variable types', () => {
      ...
      expect(config.certificates?.at(0)?.certFile).toBe('~##cert##~');
      expect(config.stores?.default?.defaultCertificate?.certFile).toBe('~##defaultCert##~');
