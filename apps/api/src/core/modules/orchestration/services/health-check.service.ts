...
export class HealthCheckService {
  ...
  async getServiceHealthSummary(serviceId: string): Promise<ServiceHealthSummary | null> {
    try {
      ...
      const healthCheckResults = healthChecks.filter(check => 
      ...
      const healthyChecks = healthCheckResults.filter(check => 
      ...
      const lastHourChecks = healthCheckResults.filter(check => 
        check.timestamp >= lastHour
      );
      ...
      if (lastHourChecks.length > 0) {
        const totalResponseTime = lastHourChecks.reduce((sum, check) => 
          sum + (check.metadata?.responseTime || 0), 0
        );
        avgResponseTime = totalResponseTime / lastHourChecks.length;

        const errorCount = lastHourChecks.filter(check => 
          check.metadata?.status !== 'healthy'
        ).length;
    ...
    try {
      ...
      for (const alert of activeHealthAlerts) {
        try {
          ...
          const healthyChecks = recentHealthChecks.filter(check => 
            check.metadata?.type === 'health-check' && 
            check.metadata?.status === 'healthy'
          );
