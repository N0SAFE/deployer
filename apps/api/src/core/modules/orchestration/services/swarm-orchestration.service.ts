...
import { SwarmStackConfig, StackStatus } from '@repo/api-contracts/modules/orchestration';
...
export class SwarmOrchestrationService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectQueue('deployment-queue') private readonly deploymentQueue: Queue,
  ) {
    ...
    try {
      ...
      for (const [serviceName, replicas] of Object.entries(request.services || request)) {
        if (typeof replicas === 'number') {
          ...
          try {
            const service = this.docker.getService(fullServiceName);
    ...
    for (const [serviceName, serviceConfig] of Object.entries(composeConfig.services)) {
      ...
      try {
        ...
        if (existingServices.length > 0) {
          ...
          const service = this.docker.getService(existingServices[0].ID);
          const serviceInfo = await service.inspect();
  ...
  private async removeStackFromSwarm(stackName: string): Promise<void> {
    try {
      ...
      // Remove all services
      await Promise.all(
        services.map(async (serviceInfo) => {
