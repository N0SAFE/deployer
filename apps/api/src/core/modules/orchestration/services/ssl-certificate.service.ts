    ...
    try {
      ...
      const expiringCertificates = await this.db.select()
        ...
        .where(and(
          ...
          lt(sslCertificates.expiresAt, new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)) // 30 days from now
        ));
    ...
    try {
      const expiringCertificates = await this.db.select()
        ...
        .where(and(
          ...
          isNotNull(sslCertificates.expiresAt),
          lt(sslCertificates.expiresAt, new Date(Date.now() + days * 24 * 60 * 60 * 1000))
        ));
