import { Controller, Post, Get, Put, Delete, Body, Param, HttpException, HttpStatus, Logger, Inject, Query } from '@nestjs/common';
import { SwarmOrchestrationService } from '../services/swarm-orchestration.service';
import type { SwarmStackConfig, ResourceUsage } from '@repo/api-contracts/modules/orchestration';
import { TraefikService, type TraefikConfig, type DomainMapping } from '../services/traefik.service';
import { ResourceAllocationService, type ResourceQuota } from '../services/resource-allocation.service';
...
import { JobTrackingService, type JobHistoryQuery } from '../services/job-tracking.service';
import { DATABASE_CONNECTION } from '../../db/database-connection';
import type { Database } from '../../db/drizzle/index';
import { orchestrationStacks } from '../../db/drizzle/schema/orchestration';
...
export class OrchestrationController {
    ...
    request: CreateStackRequest) {
        try {
            ...
            // Validate request
            if (!request.projectId || !request.environment || !request.stackName || !request.composeConfig) {
                throw new HttpException('Missing required fields', HttpStatus.BAD_REQUEST);
            }
            // Check resource capacity if quotas are set
            if (request.resourceQuotas) {
                const requestedResources = this.extractResourceUsageFromConfig(request.composeConfig);
                const capacityCheck = await this.resourceService.checkResourceCapacity(request.projectId, request.environment, requestedResources);
                if (!capacityCheck.allowed) {
                    throw new HttpException(`Resource capacity exceeded: ${capacityCheck.violations.join(', ')}`, HttpStatus.BAD_REQUEST);
                }
            }
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to create stack', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    stackId: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Stack not found', HttpStatus.NOT_FOUND);
        }
    }
    /**
    ...
    request: UpdateStackRequest) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to update stack', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
     * Remove a stack
    ...
    stackId: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to remove stack', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    request: ScaleServicesRequest) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to scale services', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    mappings: DomainMapping[]) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to update domain mappings', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    domain: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to get certificate status', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    domain: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to renew certificate', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    days?: string) {
        try {
            ...
            const certificates = await this.sslService.getCertificatesExpiringSoon(daysNum);
            return { success: true, data: certificates };
        }
        catch (error) {
            this.logger.error(`Failed to get expiring certificates:`, error);
            throw new HttpException(error instanceof Error ? error.message : 'Failed to get expiring certificates', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    request: SetQuotasRequest) {
        try {
            this.logger.log(`Setting resource quotas for ${projectId} in ${environment}`);
            await this.resourceService.setResourceQuotas(projectId, environment, request.quotas);
            return {
                success: true,
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to set resource quotas', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    environment: string) {
        try {
            const allocation = await this.resourceService.getResourceAllocation(projectId, environment);
            if (!allocation) {
                return {
                    success: true,
                    data: null,
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to get resource allocation', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    async getSystemResourceSummary() {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to get system resource summary', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    async getResourceAlerts() {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to get resource alerts', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    config: TraefikConfig) {
        try {
            const traefikConfig = await this.traefikService.generateTraefikConfig(config);
            return { success: true, data: traefikConfig };
        }
        catch (error) {
            this.logger.error('Failed to generate Traefik preview:', error);
            throw new HttpException(error instanceof Error ? error.message : 'Failed to generate Traefik preview', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
     * Extract resource usage from Docker Compose configuration
     */
    private extractResourceUsageFromConfig(composeConfig: any): Partial<ResourceUsage> {
        ...
        if (composeConfig?.services) {
            for (const [, serviceConfig] of Object.entries(composeConfig.services)) {
                ...
                if (config?.deploy?.resources?.limits) {
                    ...
                    if (limits.memory) {
                        usage.memory!.allocated += this.parseMemoryString(limits.memory) * replicas;
                    }
                }
            }
        }
    ...
    stackId: string) {
        try {
            const metrics = await this.resourceMonitoringService.getStackMetrics(stackId, 24);
            return metrics;
        }
        catch (error) {
            this.logger.error(`Failed to get metrics for stack ${stackId}:`, error);
            throw new HttpException('Failed to get stack metrics', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    async getSystemMetrics() {
        try {
            ...
            throw new HttpException('Failed to get system metrics', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
     * Get active alerts
    ...
    stackId: string) {
        try {
            const alerts = await this.resourceMonitoringService.getActiveAlerts(stackId);
            return alerts;
        }
        catch (error) {
            this.logger.error(`Failed to get alerts for stack ${stackId}:`, error);
            throw new HttpException('Failed to get stack alerts', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
     * Resolve an alert
    ...
    stackId: string) {
        try {
            const summary = await this.resourceMonitoringService.getStackResourceSummary(stackId);
            return summary;
        }
        catch (error) {
            this.logger.error(`Failed to get resource summary for stack ${stackId}:`, error);
            throw new HttpException('Failed to get stack resource summary', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getHealthSummary() {
        try {
            ...
            const stacks = await this.db.select().from(orchestrationStacks);
            const summaries = await Promise.all(stacks.map(stack => this.healthCheckService.getStackHealthOverview(stack.id)));
            return summaries;
        }
        catch (error) {
            this.logger.error('Failed to get health summary:', error);
            throw new HttpException('Failed to get health summary', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    jobId: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to retry job', HttpStatus.BAD_REQUEST);
        }
    }
    @Delete('jobs/:jobId')
    async cancelJob(
    ...
    jobId: string) {
        try {
            ...
            throw new HttpException(error instanceof Error ? error.message : 'Failed to cancel job', HttpStatus.BAD_REQUEST);
        }
    }
