...
import { SwarmStackConfig, ResourceUsage } from '@repo/api-contracts/modules/orchestration';
import { TraefikService, TraefikConfig, DomainMapping } from '../services/traefik.service';
import { ResourceAllocationService, ResourceQuota } from '../services/resource-allocation.service';
...
import { JobTrackingService, JobHistoryQuery } from '../services/job-tracking.service';
...
export interface CreateStackRequest {
  ...
  domainMappings?: { [service: string]: string[] };
  sslConfig?: {
    email: string;
...
export class OrchestrationController {
   ...
   */
  @Post('stacks')
  async createStack(@Body() request: CreateStackRequest) {
    try {
  ...
  async removeStack(@Param('stackId') stackId: string) {
    try {
  ...
  async scaleServices(
    @Param('stackId') stackId: string,
    @Body() request: ScaleServicesRequest
  ) {
    try {
  ...
  async updateDomainMappings(
    ...
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to update domain mappings',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getCertificateStatus(@Param('domain') domain: string) {
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to get certificate status',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getCertificatesExpiringSoon(@Param('days') days?: string) {
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to get expiring certificates',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async setResourceQuotas(
    ...
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to set resource quotas',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getResourceAllocation(
    ...
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to get resource allocation',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getSystemResourceSummary() {
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to get system resource summary',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getResourceAlerts() {
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to get resource alerts',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async generateTraefikPreview(@Body() config: TraefikConfig) {
    try {
      ...
      throw new HttpException(
        error instanceof Error ? error.message : 'Failed to generate Traefik preview',
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  async getStackMetrics(@Param('stackId') stackId: string) {
    try {
  ...
  async getStackAlerts(@Param('stackId') stackId: string) {
    try {
  ...
  async getStackResourceSummary(@Param('stackId') stackId: string) {
    try {
  ...
  async getStackHealth(@Param('stackName') stackName: string) {
    return await this.healthCheckService.getStackHealthOverview(stackName);
  }

  @Post('health/check/:stackName')
  async triggerHealthCheck(@Param('stackName') stackName: string) {
  ...
  async getHealthSummary() {
    try {
      ...
      const summaries = await Promise.all(
        stacks.map(stack => this.healthCheckService.getStackHealthOverview(stack.id))
      );
      return summaries;
    } catch (error) {
  ...
  @Get('jobs')
  async getJobHistory(@Query() query: JobHistoryQuery) {
  ...
  async getStackJobStatistics(@Param('stackId') stackId: string) {
