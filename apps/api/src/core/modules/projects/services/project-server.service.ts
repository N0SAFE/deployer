...
export class ProjectServerService {
  ...
  private sanitizeHost(maybeUrl: string): string {
    try {
      ...
      const sanitized = maybeUrl.replace(/^https?:\/\//, '').split('/')[0];
      return sanitized ?? maybeUrl;
    }
  }
  ...
  async ensureProjectServerForProject(projectId: string, host?: string): Promise<ProjectServerInfo | undefined> {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        this.logger.log(`Creating project server ${containerName} (attempt ${String(attempt)}/3)`);
        ...
        this.logger.error(`Attempt ${String(attempt)} failed to create project server ${containerName}:`, error);
        try {
          await this.dockerService.removeContainer(containerName);
        } catch (cleanupError) {
   ...
   */
  private async verifyContainerHealth(containerName: string, timeoutMs = 30000): Promise<{healthy: boolean, reason: string}> {
...
`;
      ...
      try {
        ...
        if (!healthCheck.healthy) {
          if (attempt === maxRetries) {
            throw new Error(`Container is not healthy after ${String(maxRetries)} attempts: ${healthCheck.reason}`);
          }
          this.logger.warn(`Container not healthy on attempt ${String(attempt)}, retrying...`);
          continue;
        }
        ...
        this.logger.warn(`Vhost setup attempt ${String(attempt)}/${String(maxRetries)} failed: ${(error as any).message}`);
        if (attempt === maxRetries) {
          throw new Error(`Failed to configure web server for ${serviceName} after ${String(maxRetries)} attempts: ${(error as any).message}`);
        }
      }
    }
  }
   ...
   */
  async ensureProjectServerHealth(projectId: string, serviceName: string, host = 'localhost'): Promise<boolean> {
    const containerName = `project-http-${projectId}`;
