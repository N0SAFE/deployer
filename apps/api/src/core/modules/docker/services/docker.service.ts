...
CMD ["npm", "start"]
    ...
    async createAndStartContainer(options: {
        ...
        try {
            ...
            const errMsg = (error && (error.json?.message ?? error.message ?? String(error))).toLowerCase();
                ...
                for (const imgName of imagesToTry) {
                    try {
                        ...
                        if (imgName !== requestedImage) {
                            (options as { image?: string }).image = imgName;
                        }
                        break;
                    } catch (pullErr) {
    ...
    async pullImage(image: string, registryAuth?: Docker.AuthConfig, retries = 3, backoffMs = 2000): Promise<void> {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                this.logger.warn(`Pull attempt ${String(attempt)} failed for ${image}: ${(err as Error)?.message || String(err)}`);
                if (attempt < retries) {
        ...
        throw new Error(`Failed to pull image ${image}: ${lastErr?.message ?? String(lastErr)}`);
    }
        ...
        try {
            ...
            if (msg && /http code\s*101|\(http code 101\)/i.test(msg)) {
                ...
                try {
                    ...
                    if (input) {
                        try {
                            (streamRetry as NodeJS.WritableStream).write(input);
                        } catch (w) {
                            this.logger.warn("Failed to write input to retry exec stdin", w);
                        }
                        try {
                            (streamRetry as NodeJS.WritableStream).end();
                        } catch {}
                    }
    ...
    async putStringIntoVolume(volumeName: string, destPathInVolume: string, filename: string, content: string, retries = 3, backoffMs = 1000): Promise<void> {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            ...
            try {
                ...
                const status = waitRes?.StatusCode ?? 0;
                ...
                if (status !== 0) {
                    ...
                    this.logger.warn(`putString attempt ${String(attempt)} failed for ${volumeName}${safeDest}/${filename}: ${lastErr.message}`);
                } else {
        ...
        throw new Error(`Failed to write file ${filename} into volume ${volumeName} after ${String(retries)} attempts: ${lastErr?.message ?? String(lastErr)}`);
    }
     ...
     */
    private async waitForStreamEnd(stream: NodeJS.ReadableStream, timeoutMs = 5000): Promise<void> {
        return new Promise((resolve, reject) => {
     ...
     */
    private async captureStreamOutput(stream: NodeJS.ReadableStream, timeoutMs = 5000): Promise<string> {
        return new Promise((resolve, reject) => {
            let output = "";
            const timeout = setTimeout(() => {
