import { pgTable, text, timestamp, boolean, integer, uuid, jsonb, pgEnum, decimal } from "drizzle-orm/pg-core";
...
export const orchestrationStacks = pgTable("orchestration_stacks", {
    ...
    domainMappings: jsonb("domain_mappings").$type<Record<string, {
            subdomain: string;
            fullDomain: string;
            sslEnabled: boolean;
...
export const orchestrationStacksRelations = relations(orchestrationStacks, ({ one, many }) => ({
    project: one(projects, {
        fields: [orchestrationStacks.projectId],
        references: [projects.id],
    }),
    serviceInstances: many(serviceInstances),
}));

export const serviceInstancesRelations = relations(serviceInstances, ({ one }) => ({
    stack: one(orchestrationStacks, {
        fields: [serviceInstances.stackId],
        references: [orchestrationStacks.id],
    }),
    service: one(services, {
        fields: [serviceInstances.serviceId],
        references: [services.id],
    }),
}));

export const networkAssignmentsRelations = relations(networkAssignments, ({ one }) => ({
    project: one(projects, {
        fields: [networkAssignments.projectId],
        references: [projects.id],
    }),
}));
...
export const sslCertificatesRelations = relations(sslCertificates, ({ one }) => ({
    project: one(projects, {
        fields: [sslCertificates.projectId],
        references: [projects.id],
    }),
}));
...
export const jobTrackingRelations = relations(jobTracking, ({ one }) => ({
    stack: one(orchestrationStacks, {
        fields: [jobTracking.stackId],
        references: [orchestrationStacks.id],
    }),
    serviceInstance: one(serviceInstances, {
        fields: [jobTracking.serviceId],
