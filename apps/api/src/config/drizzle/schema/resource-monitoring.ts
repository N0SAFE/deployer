...
export const stackMetricsRelations = relations(stackMetrics, ({ one }) => ({
    stack: one(orchestrationStacks, {
        fields: [stackMetrics.stackId],
        references: [orchestrationStacks.id],
    }),
    service: one(serviceInstances, {
        fields: [stackMetrics.serviceId],
        references: [serviceInstances.id],
    }),
}));

export const resourceAlertsRelations = relations(resourceAlerts, ({ one }) => ({
    stack: one(orchestrationStacks, {
        fields: [resourceAlerts.stackId],
        references: [orchestrationStacks.id],
    }),
    service: one(serviceInstances, {
        fields: [resourceAlerts.serviceId],
        references: [serviceInstances.id],
    }),
}));

export const metricAggregatesRelations = relations(metricAggregates, ({ one }) => ({
    stack: one(orchestrationStacks, {
        fields: [metricAggregates.stackId],
        references: [orchestrationStacks.id],
    }),
}));
