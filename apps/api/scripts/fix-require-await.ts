#!/usr/bin/env bun
/**
 * Fix @typescript-eslint/require-await errors
 *
 * This script removes the `async` keyword from functions that have no `await` expression.
 *
 * Usage:
 *   Dry run (preview changes):
 *     bun run lint 2>&1 | bun run scripts/fix-require-await.ts --dry-run
 *
 *   Apply changes:
 *     bun run lint 2>&1 | bun run scripts/fix-require-await.ts --apply
 */
...
const RULE = '@typescript-eslint/require-await';
...
function findAsyncToRemove(content: string, lineNum: number): Fix | null {
    ...
    // Search for 'async' keyword on this line or previous lines (for multiline declarations)
    for (let i = lineIndex; i >= Math.max(0, lineIndex - 5); i--) {
        const line = lines[i];

        // Match async keyword in various contexts
        // async function foo()
        // async method()
        // async () =>
        // private async method()
        // public async method()
        const asyncMatch = line.match(/\basync\s+/);

        if (asyncMatch) {
            const asyncIndex = asyncMatch.index!;
            const oldText = line;
            const newText = line.slice(0, asyncIndex) + line.slice(asyncIndex + asyncMatch[0].length);

            // Get context (the function/method name)
            const contextMatch = line.match(/(?:async\s+)?(?:function\s+)?(\w+)\s*[(<]/);
            const context = contextMatch ? contextMatch[1] : line.trim().substring(0, 50);
...
async function main() {
    ...
    if (!dryRun && !apply) {
        ...
        console.log('  bun run lint 2>&1 | bun run scripts/fix-require-await.ts --dry-run');
        console.log('  bun run lint 2>&1 | bun run scripts/fix-require-await.ts --apply');
        process.exit(1);
    }
    ...
    for (const [file, fileErrors] of errorsByFile) {
        ...
        // Sort errors by line (descending) so we can fix from bottom to top
        fileErrors.sort((a, b) => b.line - a.line);

        for (const error of fileErrors) {
            const fix = findAsyncToRemove(content, error.line);
            ...
            if (fix) {
                ...
                skipped.push({
                    file,
                    line: error.line,
                    reason: 'Could not find async keyword',
                });
            }
        }
    }
