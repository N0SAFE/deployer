...
| **Toast** | Background action failure (save, fetch) | `toast.error("Failed to save", { description: getErrorMessage(error) })` |
...
### ORPC Typed Error Handling (mandatory)

Never render `error.message` / `error instanceof Error ? error.message : ...`
directly. The API surfaces every domain failure as an ORPC builtin error with a
typed payload (`{ statusCode, code, message, orpcCode }`) — use the canonical
helpers in `apps/web/src/lib/orpc/typed-errors.ts` so the UI branches on the
CONTRACT error type, not on ad-hoc strings:

```ts
import { getErrorMessage, isDomainError } from "@/lib/orpc/typed-errors";

catch (error) {
  // Type-safe message — prefers the API payload.message, falls back to
  // error.message, then the provided fallback.
  toast.error("Failed to add", { description: getErrorMessage(error) })

  // Branch on the canonical code (declared via .errors(...) on the contract):
  if (isDomainError(error, "CONFLICT")) {
    // error.data is typed: { statusCode, code, message, orpcCode }
    toast.error("Duplicate name", { description: error.data.message })
  }
}
```

Rules:
- `getErrorMessage(error, fallback)` — the ONLY way to build a display message.
- `isDomainError(error, code)` — type guard; narrows `error.data` to the typed
  standard payload when it returns true. Used to branch per error code.
- `isDefinedError(error)` / `getErrorType(error)` — detect defined vs unknown
  errors and read the code as a value (keys, analytics).
- NEVER guess messages from `error.message` for non-ORPC errors — an unknown
  (network / 500) error must render the fallback, not "undefined".
