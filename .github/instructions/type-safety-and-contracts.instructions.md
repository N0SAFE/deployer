...
#### Contract Errors — Mandatory Pattern

Every endpoint-bearing contract MUST declare its errors so the typed client
gets a typed catch path (`isDefinedError`, `error.code`, typed `error.data`):
...
export const xxxContract = xxxOps
  .read()
  .path("/xxx")
  .input((b) => b.body(xxxInputSchema))
  .output((b) => b.body(xxxResponseSchema))
  .errors((e) => [
    ...standardDomainErrorContracts(e),          // NOT_FOUND/BAD_REQUEST/CONFLICT/UNAUTHORIZED/FORBIDDEN/FAILED_DEPENDENCY
    // Domain-specific or gateway codes go AFTER the spread:
    e().code("BAD_GATEWAY").message("Upstream failed").status(502).data(standardDomainErrorPayloadSchema),
  ])
  .build();
```

> **Why the data schema matters**: oRPC marks a thrown error as "defined" on
> the client ONLY when its data validates against this schema. Every throw
> site must therefore use the matching payload builders — see the API-side
> rules in `.github/instructions/backend-nestjs.instructions.md`
> (`standardErrorOptions` / `domainErrorOptions`).
...
### Client Error Handling (Typed)

Handle errors with the canonical helpers in
`apps/web/src/lib/orpc/typed-errors.ts` — never raw `error.message`:

- `getErrorMessage(error, fallback)` — display-safe message (prefers the
  contract payload's `message`).
- `isDomainError(error, code?)` — type guard that narrows `error.data` to the
  typed `StandardDomainErrorPayload`; branch on `NOT_FOUND`, `CONFLICT`, ... as
  declared by the contract's `.errors(...)`.
- `isDefinedError(error)` / `getErrorType(error)` — defined-vs-unknown detection
  and code-as-value.

See `.github/instructions/frontend-react-uiux.instructions.md` → "ORPC Typed Error Handling".
