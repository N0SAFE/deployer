...
6. **Maintain documentation inventory** - Always know what docs are available
7. **Read relevant docs before implementation** - Load context selectively based on task needs
...
### Phase 1: Documentation Awareness & Selective Loading
**📚 CRITICAL - UNDERSTAND WHAT'S AVAILABLE**

1. **Read `docs/README.md`** - Understand documentation structure and available topics
2. **Read `docs/core-concepts/README.md`** - Get index of all core concepts
3. **Identify relevant documentation** - Based on the task at hand, determine which docs to load
4. **Load task-specific documentation** - Read only the relevant core concepts, guides, and references needed

**Why This Matters:**
- Documentation inventory prevents missing critical guidance
- Selective reading is more efficient than loading everything
- Core concepts are NON-NEGOTIABLE when relevant to the task
- Task-specific reading ensures you have necessary context without information overload

**Examples of Selective Reading:**
- **API endpoint work**: Load ORPC pattern, Service-Adapter pattern, Repository Ownership
- **Frontend routing**: Load declarative routing docs, frontend patterns
- **Database changes**: Load Service-Adapter, Repository Ownership, database encryption docs
- **Authentication**: Load Better Auth integration pattern
- **Documentation updates**: Load Documentation Maintenance Protocol, README-First Discovery
...
### 1. Documentation Awareness Is Essential
- **ALWAYS** understand what documentation exists (read docs/README.md and docs/core-concepts/README.md)
- **LOAD** only relevant documentation for the task at hand
- **NEVER** bypass applicable core concepts without explicit override
- **ALWAYS** check loaded concepts for conflicts before implementing

### 2. Documentation-First (Selective & Smart)
- **READ** docs/README.md FIRST (understand structure and available topics)
- **READ** docs/core-concepts/README.md (inventory all core concepts)
- **IDENTIFY** which docs are relevant to current task
- **NAVIGATE** via subdirectory structure to find needed documentation
- **BATCH** relevant file reads (parallel, silent)
- **VERIFY** task-specific context complete before implementing
- **USE** `docs/bin/generate-doc-diagram.sh` to understand doc relationships when needed
